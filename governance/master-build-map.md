...
| **P1** | **After P0**, before or during **runtime preparation** (documentation) | Schema Registry; Run ID / Correlation ID Model; Event Model / Event Log Contract; Context Budget Policy; Prompt / Policy Version Registry; Cost / Token Budgeting Policy; Human Review Queue Contract; Tool Lifecycle Management; Agent Runtime Status. |
...
| **Status** | **planned-docs** (placeholder README), **partial-docs** (capability map + legacy pack) |
| **What exists now** | `tools/README.md` placeholder; capability map mentions tools logically. |
| **What is missing** | Dedicated **tool registry** artifact (markdown or future machine format); explicit permission matrix beyond legacy doc. |
| **Next required action** | Author `tools/` **contract** **v0** aligned with **agents** **registry** and **security** **layer** **(Stage** **8.5** **P0** **Runtime** **Readiness** **is** **documented** **—** **2026**-**04**-**27**). **Prerequisite (documentation):** [Self-Heal v0](../interfaces/self-heal-v0.md) and [Recovery Playbooks v0](../interfaces/recovery-playbooks-v0.md) (**plan**-**only**); extend [dependency-map.md](dependency-map.md) for new system-level **tool** / **permission** entities (§5); **review** or **update** [risk-register.md](risk-register.md) (**§7**). |
...
| Field | Content |
|-------|---------|
| **Purpose** | **LLM** and **embedding** routing, model identity, version policy. |
| **Related folders** | [../models/](../models/), [../web-gpt-sources/07_tools_models.md](../web-gpt-sources/07_tools_models.md) |
| **Status** | **planned-docs** (placeholder), **partial-docs** (legacy architecture) |
| **What exists now** | `models/README.md`; imported design in `web-gpt-sources`. |
| **What is missing** | MARS **model contract** v0 (routing rules, **SAFE UNKNOWN** for live endpoints). |
| **Next required action** | Add model-layer contract file(s) under `models/` when governance requests it. **Prerequisite (documentation):** [Self-Heal v0](../interfaces/self-heal-v0.md) (**plan-only** recovery discipline); extend [dependency-map.md](dependency-map.md) for new **Model**-layer entities (§5); **review** or **update** [risk-register.md](risk-register.md) (**§7**). |
| **Dependencies** | Stages 8, 8.5, 9 (permissions and **runtime** **readiness** **contracts** affect model/tool use); [Self-Heal v0](../interfaces/self-heal-v0.md) (documentation — **no** execution or auto-fix). |
...
| Field | Content |
|-------|---------|
| **Purpose** | **Durable** stores, **memory** services, **RAG** data paths — documented only until adapters exist. |
...
| **Status** | **planned-docs** (folder READMEs), **partial-docs** (legacy pack) |
| **What exists now** | Placeholder READMEs; architecture and migration discussion in `web-gpt-sources`. |
| **What is missing** | Consolidated MARS **storage+RAG contract** referencing **governance** boundaries; **planned-implementation** for all adapters. |
| **Next required action** | Draft cross-layer contract after **models** and **security** v0 sketches exist; ensure **Memory** **Write** **Policy** (**Stage 8.5**) is **referenced** for **mutable** **memory**/**vector** **paths**. **Prerequisite (documentation):** **review** or **update** [risk-register.md](risk-register.md) for **data**/**compliance**/**integration** rows when **storage** or **PII** **paths** are **named** (**§7**). |
| **Dependencies** | Stages 8, 8.5 (memory write policy for durable memory surfaces), 10; **registry** for project-scoped data ownership (Stage 3). |
...
| Field | Content |
|-------|---------|
| **Purpose** | **Traces**, **metrics**, **run history**, **evals** — distinct from **documented** `lifecycle-log` (design events). |
| **Related folders** | [../observability/](../observability/), [../evaluation/](../evaluation/), [../web-gpt-sources/10_observability_eval.md](../web-gpt-sources/10_observability_eval.md) |
| **Status** | **planned-docs** (README placeholders), **partial-docs** (legacy pack + governance execution-model distinction) |
| **What exists now** | Folder READMEs; imported observability/eval architecture. |
| **What is missing** | Operational **contract** tying **observability** to **workflow** signals and **control plane** without conflating **lifecycle-log**. |
| **Next required action** | Author `observability/` and `evaluation/` v0 contracts before runtime instrumentation. |
| **Dependencies** | Stages 6–7, 13 (execution story). |

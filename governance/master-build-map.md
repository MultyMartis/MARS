...
| **What exists now** | Roadmap markdown; example `mars-core` row in project registry; **documentation pack** for **`seo-content-agent`** (SEO Content Agent) under [`../projects/seo-content-agent/`](../projects/seo-content-agent/) — **documentation / planned MVP only**; **no** in-pack runtime or n8n workflows. **Documentation pack** for **`metabot-seo-content-agent`** (**MetaBOT — SEO Content Agent**) under [`../projects/metabot-seo-content-agent/`](../projects/metabot-seo-content-agent/) — **external multi-workflow AI system** (Intake / Worker / Admin; **n8n** owns execution); **in-repo knowledge only**, **not** a MARS runtime adapter or single-tool shorthand. |
| **What is missing** | Named pilot **charter**, success metrics, and **lifecycle** entries for pilot **phase transitions**; **SAFE UNKNOWN** whether `seo-content-agent` becomes the Stage 16 pilot or a parallel internal app. |

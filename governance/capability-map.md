...
| C16 Website Factory (strategic planned) | **Planned** specialist agents (intake, strategy, IA, blueprint, design, **Gulp Frontend**, QA lanes) as **documentation**; **Validator** for cross-cutting checks; **agent identity normalization** via §4.1 **`agent_id`** rows + [dependency-map.md](dependency-map.md) §4 **`entity_id`** s (**not** cards, **not** runtime) | Factory workflow in [../projects/mars-website-factory/workflow-map.md](../projects/mars-website-factory/workflow-map.md); align with `execution-flow` stages | **Cursor** / human execution today per `execution-model.md`; future **Execution Bridge** — **SAFE UNKNOWN** wire format for factory |

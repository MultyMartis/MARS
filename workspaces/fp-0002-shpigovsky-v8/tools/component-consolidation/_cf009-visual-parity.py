#!/usr/bin/env python3
"""FP-0002 V8 CF-009 before/after visual parity comparison."""
...
STORAGE = Path(r"C:\MARS Phenix\AI MARS STORAGE\website-factory\fp-0002-shpigovsky-v8\cf-009-evidence")
AUDIT = Path(__file__).resolve().parents[2] / "audits" / "cf-009-final-form"

PAGES = ["home", "services", "services-v2", "service-subdivision", "service-leaf"]
VIEWPORTS = ["desktop", "mobile"]
KINDS = ["final_form_crop", "final_form_context"]
...
def main() -> None:
    ...
    for page in PAGES:
        for viewport in VIEWPORTS:
            for kind in KINDS:
                base = f"{page}-{viewport}-initial"
                suffix = "-final-form-crop.png" if kind == "final_form_crop" else "-final-form-context.png"
    ...
    payload = {
        "manifest_id": "CF-009-VISUAL-PARITY-MATRIX",
        "overall": overall,
        "matrix": matrix,
    }
    (AUDIT / "CF-009-VISUAL-PARITY-MATRIX.json").write_text(

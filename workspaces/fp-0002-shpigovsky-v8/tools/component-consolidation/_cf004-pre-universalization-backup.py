#!/usr/bin/env python3
"""FP-0002 V8 CF-004 pre-universalization backup + SHA-256 guard (pre-change)."""
...
ZIP_NAME = "FP-0002-V8-BEFORE-CF-004-FOUNDER-QUOTE-UNIVERSALIZATION.zip"
MANIFEST_LINE = "FP-0002 V8 CF-004 FOUNDER QUOTE PRE-UNIVERSALIZATION STATE PRESERVED"

TARGET_FILES = [
    "src/partials/sections/home-founder-quote.html",
...
REGISTRY_FILES = [
    ...
    "audits/component-family-audit-v8-bootstrap-01/FP-0002-V8-COMPONENT-FAMILY-AUDIT-DATA.json",
    ...
    "audits/shared-component-universalization/FP-0002-V8-UNIVERSAL-COMPONENT-NAMING-RULES-v1.md",
...
def main() -> None:
    ...
    staging = OUT_DIR / "_cf004_backup_staging"
    if staging.exists():
        import shutil
    ...
    for rel in TARGET_FILES + REGISTRY_FILES:
        src = ROOT / rel
        if not src.is_file():
            raise SystemExit(f"FP0002_V8_CF004_BACKUP_FAILED: missing {rel}")
        dest = staging / rel
    ...
    audit_dir = ROOT / "audits" / "cf-004-founder-quote" / "data"
    audit_dir.mkdir(parents=True, exist_ok=True)
    (audit_dir / "cf-004-source-hash-guard-pre.json").write_text(
        json.dumps(pre_hashes, indent=2) + "\n", encoding="utf-8"
    )

    print(
        json.dumps(
            {"zip": str(zip_path), "zip_sha256": sha256_file(zip_path), "pre_hashes": pre_hashes},
            indent=2,
        )
    )

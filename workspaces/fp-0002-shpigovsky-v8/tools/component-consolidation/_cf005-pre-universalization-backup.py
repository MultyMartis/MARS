#!/usr/bin/env python3
"""FP-0002 V8 CF-005 pre-universalization backup + SHA-256 guard (pre-change)."""
...
ZIP_NAME = "FP-0002-V8-BEFORE-CF-005-SPECIALISTS-UNIVERSALIZATION.zip"
MANIFEST_LINE = "FP-0002 V8 CF-005 SPECIALISTS PRE-UNIVERSALIZATION STATE PRESERVED"
...
PROTECTED_FILES = [
    ...
    "src/partials/sections/home-comfort.html",
]
...
def main() -> None:
    ...
    staging = OUT_DIR / "_cf005_backup_staging"
    if staging.exists():
        import shutil
    ...
    for rel in TARGET_FILES + REGISTRY_FILES:
        src = ROOT / rel
        if not src.is_file():
            raise SystemExit(f"FP0002_V8_CF005_BACKUP_FAILED: missing {rel}")
        dest = staging / rel
    ...
    audit_dir = ROOT / "audits" / "cf-005-specialists" / "data"
    audit_dir.mkdir(parents=True, exist_ok=True)
    (audit_dir / "cf-005-source-hash-guard-pre.json").write_text(

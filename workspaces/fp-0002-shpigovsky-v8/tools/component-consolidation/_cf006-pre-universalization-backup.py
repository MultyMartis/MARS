#!/usr/bin/env python3
"""FP-0002 V8 CF-006 pre-universalization backup + SHA-256 guard (pre-change)."""
...
ZIP_NAME = "FP-0002-V8-BEFORE-CF-006-COMFORT-UNIVERSALIZATION.zip"
MANIFEST_LINE = "FP-0002 V8 CF-006 COMFORT PRE-UNIVERSALIZATION STATE PRESERVED"
...
def main() -> None:
    ...
    staging = OUT_DIR / "_cf006_backup_staging"
    if staging.exists():
        import shutil
    ...
    for rel in TARGET_FILES + REGISTRY_FILES:
        src = ROOT / rel
        if not src.is_file():
            raise SystemExit(f"FP0002_V8_CF006_BACKUP_FAILED: missing {rel}")
        dest = staging / rel
    ...
    audit_dir = ROOT / "audits" / "cf-006-comfort" / "data"
    audit_dir.mkdir(parents=True, exist_ok=True)
    (audit_dir / "cf-006-source-hash-guard-pre.json").write_text(

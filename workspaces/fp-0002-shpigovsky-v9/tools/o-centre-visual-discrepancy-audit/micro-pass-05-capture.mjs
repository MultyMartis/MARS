...
const storageRoot =
  "X:/AI MARS STORAGE/website-factory/fp-0002-shpigovsky-v8/o-centre-micro-pass-05-clinic-landscape";
...
const PORT = Number(process.env.MICRO_PASS_05_PORT || 4728);
...
if (approvedHash !== EXPECTED_HASH) {
  console.error("FP0002_OCENTRE_CLINIC_LANDSCAPE_PNG_AUTHORITY_MISMATCH");
  process.exit(2);
}

const DESIGN_CROP = { x: 0, y: 4180, width: 1437, height: 720 };
const RUNTIME_CONTEXT_ABOVE = 100;
const RUNTIME_CONTEXT_BELOW = 140;
...
async function getRuntimeCropBox(page) {
  return page.evaluate(
    ({ above, below }) => {
      ...
      if (!approach || !clinic || !program) return null;
      const py = (el) => {
        const r = el.getBoundingClientRect();
        return { top: r.top + window.scrollY, bottom: r.bottom + window.scrollY, height: r.height, width: r.width };
      };
      const ap = py(approach);
      const cp = py(clinic);
      const pp = py(program);
      const img = clinic.querySelector(".clinic-landscape__image");
      const bleed = clinic.querySelector(".clinic-landscape__bleed");
      const container = clinic.querySelector(".container");
      const ip = img ? py(img) : null;
      const bp = bleed ? py(bleed) : null;
      const ctp = container ? py(container) : null;
      const intro = approach.querySelector(".program-approach-band__intro");
      const introBox = intro ? py(intro) : null;
      const programHead = program.querySelector(".services-program-v2__head, h2");
      const programHeadBox = programHead ? py(programHead) : null;
      const cs = img ? getComputedStyle(img) : null;
      const top = Math.max(introBox ? introBox.bottom - above : ap.bottom - above, ap.top);
      const bottom = Math.min(pp.top + below, programHeadBox ? programHeadBox.bottom + below : pp.top + below);
      return {
        x: 0,
        ...
        clinic_image: ip,
        clinic_bleed: bp,
        clinic_container: ctp,
        program: pp,
        intro: introBox,
        program_head: programHeadBox,
        spacing_intro_to_clinic_section: cp.top - (introBox ? introBox.bottom : ap.bottom),
        spacing_clinic_section_to_program: pp.top - cp.bottom,
        spacing_intro_to_clinic_image: ip && introBox ? ip.top - introBox.bottom : null,
        spacing_clinic_image_to_program: pp.top - (ip ? ip.bottom : cp.bottom),
        image_natural: img ? { w: img.naturalWidth, h: img.naturalHeight } : null,
        image_computed: cs
          ? {
              width: cs.width,
              height: cs.height,
...
async function captureRuntimeCrop(page, outPath, fullPath) {
  ...
  const py = join(tempDir, "micro-pass-05-runtime-crop.py");
...
async function diagnose(page) {
  ...
  return page.evaluate(() => {
    ...
    const img = document.querySelector(".clinic-landscape__image");
    return {
      clinicLandscapeCount: document.querySelectorAll(".clinic-landscape").length,
...
const designCropPath = join(designDir, "o-centre-micro-pass-05-design-crop.png");
const designCropMeta = await cropDesignPng(page, designCropPath);

const meta = {
  task: "FP-0002 O-Centre Clinic Landscape Desktop Micro-Pass 05",
  approved_png_path: approvedPngPath,
  approved_png_sha256: approvedHash,
  figma_frame: "1:2185",
  landscape_frame_node: "1:2341",
  landscape_image_node: "1:2391",
  asset_path: "assets/img/content/pre-reviews/shpigovsky-clinic-landscape.webp",
  asset_natural_dimensions: [1139, 584],
  figma_rendered_dimensions: [1170, 603],
...
try {
  if (phase === "before") {
    const beforeCrop = join(beforeDir, "o-centre-micro-pass-05-runtime-before.png");
    const beforeFull = join(beforeDir, "o-centre-micro-pass-05-runtime-before-full.png");
    ...
    meta.diagnosis = diag;
    writeFileSync(join(evidenceDir, "micro-pass-05-before-meta.json"), JSON.stringify(meta, null, 2));
    console.log(JSON.stringify({ phase: "before", ...meta }, null, 2));
  } else {
    const beforeCrop = join(beforeDir, "o-centre-micro-pass-05-runtime-before.png");
    const afterCrop = join(afterDir, "o-centre-micro-pass-05-runtime-after.png");
    const afterFull = join(afterDir, "o-centre-micro-pass-05-runtime-after-full.png");
    const runtimeBox = await captureRuntimeCrop(page, afterCrop, afterFull);
    const diag = await diagnose(page);
    diag.consoleErrors = consoleErrors;
    const sideBySide = join(evidenceDir, "o-centre-micro-pass-05-side-by-side.png");
    ...
    writeFileSync(join(evidenceDir, "micro-pass-05-after-meta.json"), JSON.stringify(meta, null, 2));

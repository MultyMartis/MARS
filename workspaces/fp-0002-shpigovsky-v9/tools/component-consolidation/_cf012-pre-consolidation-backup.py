#!/usr/bin/env python3
"""FP-0002 V8 CF-012 pre-consolidation backup from git HEAD + SHA-256 guard."""
...
ZIP_NAME = "FP-0002-V8-BEFORE-CF-012-PROGRAM-MODIFIER-CONSOLIDATION.zip"
MANIFEST_LINE = "FP-0002 V8 CF-012 PROGRAM MODIFIER PRE-CONSOLIDATION STATE PRESERVED"

TARGET_FILES = [
    "src/partials/sections/services-program-v2.html",
    "src/partials/components/services-program-v2-item.html",
...
REGISTRY_FILES = [
    ...
    "audits/cf-012-program-modifiers/CF-012-PRE-IMPLEMENTATION-INVENTORY.md",
    "audits/cf-012-program-modifiers/data/CF-012-PRE-IMPLEMENTATION-INVENTORY.json",
]
...
def main() -> None:
    ...
    staging = OUT_DIR / "_cf012_backup_staging"
    if staging.exists():
        import shutil
    ...
    for rel in TARGET_FILES + REGISTRY_FILES:
        try:
            ...
            local = ROOT / rel
            if not local.exists():
                raise SystemExit(f"FP0002_V8_CF012_BACKUP_FAILED: missing {rel}") from None
    ...
    git_diff_stat = git_cmd(["diff", "--stat", "--", "workspaces/fp-0002-shpigovsky-v8"])
    ...
    manifest = {
        ...
        "source": "git HEAD snapshot (pre-CF-012 implementation)",
        "files": bundle_files,
        "sha256": checksums,
    ...
    (OUT_DIR / "FP-0002-V8-BEFORE-CF-012-PROGRAM-MODIFIER-CONSOLIDATION.sha256").write_text(
        f"{zip_sha}  {ZIP_NAME}\n", encoding="utf-8"
    )

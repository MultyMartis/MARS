#!/usr/bin/env python3
"""FP-0002 V8 CF-007 slider functional smoke QA."""
...
INTERACT_JS = """() => {
  const root = document.querySelector('.reviews, .home-reviews');
  const slider = root ? root.querySelector('[data-reviews-slider]') : null;
  ...
  if (bullets.length > 1) {
    const target = Math.min(2, bullets.length - 1);
    sw.slideTo(target, 0);
    paginationWorks = sw.activeIndex === target;
  }
  sw.slideTo(0, 0);
  return {
    ok: true,
...
def main() -> None:
    ...
    with sync_playwright() as p:
        ...
        for page_id, page_file in PAGES:
            for viewport_id, width, height in VIEWPORTS:
                ...
                page.locator(".reviews, .home-reviews").first.scroll_into_view_if_needed()
    ...
    payload = {
        "validation_id": "CF-007-SLIDER-FUNCTIONAL-QA",
        "rows": rows,
        "overall": "PASS" if all(r["result"] == "PASS" for r in rows) else "FAIL",
    }
    (AUDIT / "CF-007-SLIDER-FUNCTIONAL-QA.json").write_text(

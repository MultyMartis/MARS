#!/usr/bin/env python3
"""FP-0002 V8 CF-006 post-implementation hash guard."""
...
def main() -> None:
    pre = json.loads((AUDIT / "cf-006-source-hash-guard-pre.json").read_text(encoding="utf-8"))
    post = {
    ...
    protected_changes = [
        rel for rel, h in pre["protected"].items() if post["protected"].get(rel) != h
    ]
    payload = {
        **post,
    ...
    (AUDIT / "cf-006-source-hash-guard-post.json").write_text(
        json.dumps(payload, indent=2) + "\n", encoding="utf-8"
    )
    print(json.dumps({"protected_unchanged": payload["protected_unchanged"], "changes": protected_changes}, indent=2))

# FP-0002 V6 RESPONSIVE SHELL STABLE 01 — Restore Instructions

**Release ID:** `FP-0002-V6-RESPONSIVE-SHELL-STABLE-01`  
**Archive:** `FP-0002-V6-RESPONSIVE-SHELL-STABLE-01-SOURCE.zip`  
...
| Working folder | Any empty directory; example `C:\restore\fp-0002-v6-responsive-shell-stable-01\` |
...
1. Obtain `FP-0002-V6-RESPONSIVE-SHELL-STABLE-01-SOURCE.zip` from MARS Storage (see release manifest).
2. Extract to a clean directory. Root folder inside archive: `FP-0002-V6-RESPONSIVE-SHELL-STABLE-01/`.
...
```powershell
cd FP-0002-V6-RESPONSIVE-SHELL-STABLE-01
npm ci
```
...
```powershell
git fetch origin tag fp-0002-v6-responsive-shell-stable-01
git checkout fp-0002-v6-responsive-shell-stable-01
```
...
## 10. Responsive shell verification

After build, confirm:

- Desktop Header, Hero, Footer render at ≥1025px
- Mobile Header (logo, primary phone, menu trigger) at ≤1024px
- Off-canvas opens from **left** (`transform: translateX(-100%)` closed state)
- Off-canvas closes via button, overlay, Escape; body scroll lock active when open
- Mobile Footer layout without horizontal overflow at 320–430px

---

## 11. Do not substitute `dist` for `src`
...
- [ ] Responsive shell renders from local build
- [ ] Off-canvas JS present in `dist/assets/js/main.js`

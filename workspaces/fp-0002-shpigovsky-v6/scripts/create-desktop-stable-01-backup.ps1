# FP-0002 V6 DESKTOP STABLE 01 — source backup, checksums, verify, restore test
$ErrorActionPreference = 'Stop'

$Workspace = 'C:\AI MARS\workspaces\fp-0002-shpigovsky-v6'
$ReleaseId = 'FP-0002-V6-DESKTOP-STABLE-01'
...
$RestoreTestDir = Join-Path 'C:\AI MARS\workspaces' 'fp-0002-shpigovsky-v6-restore-test'
...
$backupManifest = @"
# FP-0002 V6 DESKTOP STABLE 01 — Backup Manifest

**Release ID:** $ReleaseId
**Created:** $(Get-Date -Format 'yyyy-MM-ddTHH:mm:ssK')
**Scope:** Canonical operator-approved desktop source snapshot
...
# Restore test — extract at workspaces/ depth so ../../shared resolves to repo shared/
$RestoreTestDir = Join-Path 'C:\AI MARS\workspaces' 'fp-0002-shpigovsky-v6-restore-test'
if (Test-Path $RestoreTestDir) {
  try { Remove-Item $RestoreTestDir -Recurse -Force -ErrorAction Stop } catch {
    $RestoreTestDir = Join-Path 'C:\AI MARS\workspaces' ('fp-0002-shpigovsky-v6-restore-test-' + (Get-Date -Format 'yyyyMMddHHmmss'))
  }
}
New-Item -ItemType Directory -Path $RestoreTestDir -Force | Out-Null
$ExpandRoot = Join-Path $env:TEMP 'fp0002-restore-expand'

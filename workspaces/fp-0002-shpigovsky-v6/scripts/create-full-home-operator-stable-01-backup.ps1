# FP-0002 V6 FULL HOME OPERATOR STABLE 01 — backup, checksums, verify, restore test
$ErrorActionPreference = 'Stop'

$Workspace = 'C:\AI MARS\workspaces\fp-0002-shpigovsky-v6'
$ReleaseId = 'FP-0002-V6-FULL-HOME-OPERATOR-STABLE-01'
...
$RestoreTestDir = Join-Path 'C:\AI MARS\workspaces' ('fp-0002-shpigovsky-v6-fullhome-restore-test-' + (Get-Date -Format 'yyyyMMddHHmmss'))
New-Item -ItemType Directory -Path $RestoreTestDir -Force | Out-Null
$ExpandRoot = Join-Path $env:TEMP 'fp0002-fullhome-restore-expand'
...
if ($distHtml -notmatch 'home-final-form') { Pop-Location; throw 'Restore test: Final form must be present' }
if ($distHtml -notmatch 'home-faq') { Pop-Location; throw 'Restore test: FAQ must be present' }
...
if ($distHtml -match 'data-safe-unknown') { Pop-Location; throw 'Restore test: data-safe-unknown must be 0' }

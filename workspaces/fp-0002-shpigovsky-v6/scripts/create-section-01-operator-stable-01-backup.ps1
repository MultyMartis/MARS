# FP-0002 V6 HOME SECTION 01 OPERATOR STABLE 01 — source backup, checksums, verify, restore test
$ErrorActionPreference = 'Stop'

$Workspace = 'C:\AI MARS\workspaces\fp-0002-shpigovsky-v6'
$ReleaseId = 'FP-0002-V6-HOME-SECTION-01-OPERATOR-STABLE-01'
...
$backupManifest = @"
# FP-0002 V6 HOME SECTION 01 OPERATOR STABLE 01 — Backup Manifest

**Release ID:** $ReleaseId
**Created:** $(Get-Date -Format 'yyyy-MM-ddTHH:mm:ssK')
**Scope:** Operator-polished Section 01 + responsive shell (Header, Hero, Footer, local Inter, foundation)
**NOT CANONICAL:** dist/ — regenerate via npm run build
**NOT IN SCOPE:** Section 02+, main content beyond Section 01
...
$RestoreTestDir = Join-Path 'C:\AI MARS\workspaces' 'fp-0002-shpigovsky-v6-section-01-restore-test'
if (Test-Path $RestoreTestDir) {
  try { Remove-Item $RestoreTestDir -Recurse -Force -ErrorAction Stop } catch {
    $RestoreTestDir = Join-Path 'C:\AI MARS\workspaces' ('fp-0002-shpigovsky-v6-section-01-restore-test-' + (Get-Date -Format 'yyyyMMddHHmmss'))
  }
}
New-Item -ItemType Directory -Path $RestoreTestDir -Force | Out-Null
$ExpandRoot = Join-Path $env:TEMP 'fp0002-section-01-restore-expand'
...
if ($distHtml -notmatch 'home-recovery-intro') { throw 'Restore test: Section 01 missing in dist HTML' }
if ($distHtml -match 'home-founder-quote|home-expert-opinion') { throw 'Restore test: Section 02 must be absent in Section 01 freeze' }
...
$srcHtml = Get-ChildItem 'src' -Recurse -Filter '*.html' | ForEach-Object { Get-Content $_.FullName -Raw }
if (($srcHtml -join '') -match 'data-safe-unknown') { throw 'Restore test: data-safe-unknown must be 0' }
Pop-Location

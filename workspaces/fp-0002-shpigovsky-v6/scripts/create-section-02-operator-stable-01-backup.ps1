# FP-0002 V6 HOME SECTION 02 OPERATOR STABLE 01 — source backup, checksums, verify, restore test
$ErrorActionPreference = 'Stop'

$Workspace = 'C:\AI MARS\workspaces\fp-0002-shpigovsky-v6'
$ReleaseId = 'FP-0002-V6-HOME-SECTION-02-OPERATOR-STABLE-01'
...
$backupManifest = @"
# FP-0002 V6 HOME SECTION 02 OPERATOR STABLE 01 — Backup Manifest

**Release ID:** $ReleaseId
**Created:** $(Get-Date -Format 'yyyy-MM-ddTHH:mm:ssK')
**Scope:** Section 02 with exact Figma portrait + operator shell through Section 02
**NOT CANONICAL:** dist/ — regenerate via npm run build
**NOT IN SCOPE:** Section 03+
...
$RestoreTestDir = Join-Path 'C:\AI MARS\workspaces' ('fp-0002-shpigovsky-v6-section-02-restore-test-' + (Get-Date -Format 'yyyyMMddHHmmss'))
New-Item -ItemType Directory -Path $RestoreTestDir -Force | Out-Null
$ExpandRoot = Join-Path $env:TEMP 'fp0002-section-02-restore-expand'
...
if ($distHtml -notmatch 'home-founder-quote') { throw 'Restore test: Section 02 missing in dist HTML' }
if ($distHtml -match 'home-treatment-prevention') { throw 'Restore test: Section 03 must be absent in Section 02 freeze' }
$portrait = 'dist\assets\img\content\founder-sergey-shpigovsky.png'
if (-not (Test-Path $portrait)) { throw 'Restore test: founder portrait missing in dist' }

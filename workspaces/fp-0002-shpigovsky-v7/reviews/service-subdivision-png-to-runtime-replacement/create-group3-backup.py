"""Create GROUP 3 operator checkpoint ZIP."""
import hashlib
import subprocess
import zipfile
...
zip_path = storage / "FP-0002-V7-SERVICE-SUBDIVISION-PNG-GROUP-3-BEFORE-SOURCE.zip"

head = subprocess.check_output(["git", "rev-parse", "--short", "HEAD"], cwd=ws.parents[1], text=True).strip()
...
manifest_lines = [
    "# BACKUP-MANIFEST — FP-0002 GROUP 3",
    "",
    ...
    "- scope: GROUP 3 Rehabilitation requirements / stages / support block",
...
restore = """# RESTORE-INSTRUCTIONS — FP-0002 GROUP 3

1. Stop preview server if running.
2. Extract `FP-0002-V7-SERVICE-SUBDIVISION-PNG-GROUP-3-BEFORE-SOURCE.zip` into `workspaces/fp-0002-shpigovsky-v7/` preserving relative paths.
3. Verify `src/pages/usluga-podrazdel-v1.html` matches pre-GROUP-3 state.
4. Run `npm run build` from workspace root.
5. Confirm HEAD was `41777d4a` at backup time (GROUP 2 baseline).
"""

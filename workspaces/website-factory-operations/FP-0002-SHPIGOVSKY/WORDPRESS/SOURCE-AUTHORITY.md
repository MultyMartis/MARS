...
## V9-06E59 layout polish, contacts maps, footer links, comfort CTA admin parity (2026-07-17)

E59 **PASS** (local; no commit/push/freeze): Operator-owned runtime drift canonized before wave — `v9-style.css` (`106D5BEB…` promoted) and operator-rebuilt `rehabilitation-requirements.php` (`68BE0867…` promoted). Implemented confirmed E58-VA-001 only (`no-top-padding` utilities on Home staff/feature/landscape/why-us). Contacts page `#20` gained ACF repeater `contacts_locations` (`group_fp02_page_contacts`) with validated Yandex Constructor embed rendering via `inc/yandex-map-embed.php`. Footer nav headings «Услуги» / «О центре» link internally. Comfort Requirements block (`fp02-block-comfort` storage; admin `fp02-block-comfort-requirements`) gained `cta_lead_text` for `.home-rehabilitation-requirements__cta-lead-txt`. Bounded exact-file delivery: 15 paths; source↔runtime hash match YES on all delivered files. Post-wave protected operator CSS hash prefix `1AA1AAC8…`. DB writes: `contacts_locations` (2 rows on page 20) + `cta_lead_text` seed only. Backup: `v9-06e59-before-layout-polish-maps-footer-comfort-admin-20260717-001046`. Report: `REPORTS/REPORT-FP-0002-V9-06E59-layout-polish-maps-footer-comfort-admin.md`. Evidence: `REPORTS/evidence/v9-06e59-layout-polish-maps-footer-comfort-admin/`.

## V9-06E59-FIX01 comfort decor, contacts ACF cleanup, footer hover (2026-07-17)

E59-FIX01 **PASS** (local; no commit/push/freeze): Preflight confirmed full theme source↔runtime MATCH (operator CSS/HTML already canon; no promote). Comfort decor separated from gallery semantics (`.comfort__gallery-stage` + `.comfort__gallery-decor`; real items only inside `.comfort__gallery` with `display:contents`). Obsolete Contacts page ACF fields removed from PHP/JSON registration (`contacts_address`, `contacts_map_url`, `contacts_blocks`); `contacts_locations` retained (2 rows); legacy postmeta retained dormant; no postmeta deletion. Footer heading-link hover/focus replaced with exact `color: var(--color-accent-hover)`. Future Site Settings admin IA audit registered in `DOCS/FUTURE-TASK-SITE-SETTINGS-ADMIN-IA-AUDIT-AND-RU-UX-REBUILD-v1.md`. Exact-file delivery 8 paths; source↔runtime MATCH. Post-wave CSS hash prefix `361CB364…`. DB writes: **0**. Backup: `v9-06e59-fix01-before-comfort-contacts-footer-corrections-20260717-013408`. Report: `REPORTS/REPORT-FP-0002-V9-06E59-FIX01-comfort-contacts-footer-corrections.md`. Evidence: `REPORTS/evidence/v9-06e59-fix01-comfort-contacts-footer-corrections/`.

## V9-06E60 nav/breadcrumb audit, program CTA unify, service links (2026-07-17)

E60 **PASS** (local; no commit/push/freeze): Operator runtime `v9-style.css` drift promoted (`C8AE0923299B…` → source; post-wave `B60D01A404FA…`). Task 00 audit: no unwanted E58/E59 nav/breadcrumb typography changes (nav 16/20; crumbs 14/18; blog mobile 8/12 Figma EXACT_GEOMETRY retained) — no typography restore. `.program-cta-band` rebuilt to Comfort CTA structure (`wrap01`/`wrap02`, lead/lead-txt); `group_fp02_block_cta_bands` labels aligned (field keys reused); optional `cta_button_target` / blog CTA URL wired. `.services-category-section-v2__service-name` becomes permalink `<a>` on hub/section lists. Title/nav-like hovers use `var(--color-accent-hover)` without underline where classified. Exact-file delivery 9 paths + ACF JSON to runtime; theme drift 0. DB writes: **0**. Backup: `v9-06e60-before-nav-breadcrumb-cta-service-links-20260717-015352`. Report: `REPORTS/REPORT-FP-0002-V9-06E60-nav-breadcrumb-cta-service-links.md`. Evidence: `REPORTS/evidence/v9-06e60-nav-breadcrumb-cta-service-links/`.

## V9-06E61 admin controls, contacts/blog/reviews/ocentre/home (2026-07-17)

E61 **PARTIAL PASS** (local; no commit/push/freeze): Operator runtime CSS/JS canonized first (`v9-style` `5E4378756424`, shell `A38931640BDD`, lifebuoy `FD8B5551813D`) then additive CSS only → `DCB0C163…` (operator breadcrumb 16/22 preserved). Global toggles `show_breadcrumbs_pages` / `show_breadcrumbs_services`; Contacts multi-phone + header-parity messengers + empty crumb shell; Blog archive admin simplify + `blog_archive_posts_per_page` + 10 demo posts `#1745–1754`; Reviews options `review_service` + expand clamp + `reviews_per_page`; O-centre remove `#o-centre-cta-1` / who-we-treat gallery, insert program CTA, reuse Home staff/feature/landscape, red-line infrastructure lead. Exact-file source→runtime delivery (no broad sync). Backup: `v9-06e61-before-admin-controls-contacts-blog-reviews-ocentre-home-20260717-141747`. Report: `REPORTS/REPORT-FP-0002-V9-06E61-admin-controls-contacts-blog-reviews-ocentre-home.md`. Evidence: `REPORTS/evidence/v9-06e61-admin-controls-contacts-blog-reviews-ocentre-home/`.

## V9-06E62A 404, breadcrumb wrapper, phone mask (2026-07-17)

E62A **PASS** (local; no commit/push/freeze): Operator runtime `v9-style.css` promoted (`DCB0C163…` → **`E11627B9…`**; crumbs 16/22 preserved). Figma 404 from approved PNG pair (`Страница 404 - десктоп/мобильная`); theme `404.php` + `fp02-404.css` + local visual asset. Generic/Specialist/Legal breadcrumb shell restored via `shpigovsky_render_breadcrumbs` wrap (Contacts/Reviews/Blog/Service excluded). Phone mask replaced non-functional Inputmask path with Triumph v6 vanilla `bindPhoneMask` (`+7 (XXX) XXX-XX-XX`). Exact-file delivery 10 theme paths; source↔runtime MATCH; DB writes **0**. E61 tails retained open. Backup: `v9-06e62a-before-404-breadcrumb-wrapper-phone-mask-20260717-160948`. Report: `REPORTS/REPORT-FP-0002-V9-06E62A-404-breadcrumb-wrapper-phone-mask.md`. Evidence: `REPORTS/evidence/v9-06e62a-404-breadcrumb-wrapper-phone-mask/`.

## V9-06E62B Blog/Reviews pagination, SEO, slider links (2026-07-17)

E62B **PASS** (local; no commit/push/freeze): Pre-wave theme/plugin drift **0**. Additive `v9-style` `E11627B9…` → **`18114D3C…`**. Demo Blog featured images on `#1745–1754` (existing ML attachments only). +20 idempotent demo Reviews in ACF `reviews_items` (total 30; `reviews_per_page=10` → 3 pages; out-of-range 404). Theme pagination SEO self-canonicals for Blog/Reviews (no SEO plugin). Slider five-line overflow → `<a>Читать весь отзыв</a>` with page-aware `#review-{id}` targets. Founder Quote reusable ACF group registered + seeded (`fp02-block-founder-quote`). Exact-file delivery 11 paths (+ founder JSON); **no** broad ACF sync of 8 source-only groups. Backup: `v9-06e62b-before-blog-reviews-pagination-seo-demo-content-20260717-162925`. Report: `REPORTS/REPORT-FP-0002-V9-06E62B-blog-reviews-pagination-seo-slider-links.md`. Evidence: `REPORTS/evidence/v9-06e62b-blog-reviews-pagination-seo-slider-links/`.

## V9-06E62C O-centre cleanup, Service admin hide, stable review UIDs, final regression (2026-07-17)

E62C **PASS** (local; no commit/push/freeze): Pre-wave theme/plugin drift **0**; operator `v9-style` **`18114D3C…`** preserved (no CSS overwrite). O-centre infrastructure lead without inner `<span>`; new ACF `infrastructure_narrative_bullet_intro` seeded if empty; who-we-treat nested CTA fixed to `<div class="program-cta-band">`. Service groups Structured Sections + Relationships hidden (`active:false` + always-hide filter; postmeta retained). Reviews stable `review_uid` anchors (30 unique); slider/archive page-aware links; reversible reorder test restored. Exact-file delivery (theme/plugin + reviews ACF JSON only); **no** broad sync of remaining source-only ACF groups. Viewport 64/64 screenshots PASS. DB writes: bullet seed + UIDs + reorder test. Backup: `v9-06e62c-before-ocentre-service-admin-review-anchor-final-regression-20260717-164734`. Report: `REPORTS/REPORT-FP-0002-V9-06E62C-ocentre-service-admin-review-anchor-final-regression.md`. Evidence: `REPORTS/evidence/v9-06e62c-ocentre-service-admin-review-anchor-final-regression/`.

## V9-06E62D Treatment Program mini-descriptions + 404 Figma correction (2026-07-17)

E62D **PASS** (local; no commit/push/freeze): Pre-wave theme/plugin drift **0**; operator `v9-style` **`18114D3C…`** preserved. New ACF group `group_fp02_treatment_program_child` (location `page_parent == 13`) field `treatment_program_short_description` / «Мини-описание»; seeded pages **#1053–#1056** from former hardcoded Home direction texts; `program-direction-helpers.php` reads page ACF for Home cards and drops permanent hardcoded body copy. 404 `fp02-404.css` retuned to approved Figma PNG metrics (Inter/`#475371`, desktop 32/20 typography, logo 246px, button 262×53, PNG-owned cutout — CSS radius removed). Exact-file delivery of theme/plugin + **one** new ACF JSON; **no** broad sync of source-only ACF groups. DB writes: seed + reversible edit/empty tests (restored). Backup: `v9-06e62d-before-program-mini-descriptions-404-figma-correction-20260717-170730`. Report: `REPORTS/REPORT-FP-0002-V9-06E62D-program-mini-descriptions-404-figma-correction.md`. Evidence: `REPORTS/evidence/v9-06e62d-program-mini-descriptions-404-figma-correction/`.

## V9-06E62E 404 decor replacement + WordPress site search (2026-07-17)

E62E **PASS** (local; no commit/push/freeze): Operator runtime CSS canonized first (`v9-style` **`C2246240…`** preserved; `fp02-404` runtime→source then decor-width edit). Operator asset `INCOMING/OPERATOR-ASSETS/404/404-decor.png` → `assets/img/404/404-decor.png` (670×425); `404-visual.png` removed from source/runtime. Header search dropdown (`search-panel` + `searchform.php` + `fp02-search.css` + `v9-shell.js`); mobile + floating triggers added. Native frontend search via `inc/search-helpers.php` (`post`/`page`/`service`, 12/page; legal pages excluded). `search.php` results + cards + pagination; SEO `noindex,follow` + title with query/page; core canonical removed on search. Exact-file delivery; **no** broad sync; DB writes **0**. Backup: `v9-06e62e-before-404-decor-wordpress-search-20260717-173256`. Report: `REPORTS/REPORT-FP-0002-V9-06E62E-404-decor-wordpress-search.md`. Evidence: `REPORTS/evidence/v9-06e62e-404-decor-wordpress-search/`.

## V9-06E63 Stable v1 closeout (2026-07-18)

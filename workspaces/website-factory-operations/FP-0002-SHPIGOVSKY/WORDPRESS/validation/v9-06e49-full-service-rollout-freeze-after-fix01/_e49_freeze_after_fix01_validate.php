<?php
/**
 * V9-06E49 Full Service Rollout Freeze AFTER FIX01 — validation + evidence exports.
 *
 * Read-only freeze retry after #315 restore: no lasting product mutation.
 *
 * @package FP0002
 */
...
$bak_path_file = 'X:/AI MARS/workspaces/website-factory-operations/FP-0002-SHPIGOVSKY/REPORTS/evidence/v9-06e49-freeze-after-fix01-backup-path.txt';
...
function e49fz_http( string $url ): array {
	...
	curl_setopt_array(
		...
		array(
			...
			CURLOPT_USERAGENT     => 'FP0002-E49-Freeze-After-FIX01/1.0',
		)
	);
...
foreach ( $q->posts as $p ) {
	...
	if ( 'publish' === $p->post_status ) {
		...
		if ( 315 === $id ) {
			$notes = trim( $notes . ' E49-FIX01 restored to Услуга; must remain service/service_general' );
		}
	...
	if ( 'publish' === $p->post_status ) {
		...
		if ( 'section_excluded' !== $cat ) {
			...
			if ( 315 === $id ) {
				$admin_ok = $admin_ok && ( 'service' === $role ) && ( 'service_general' === $layout || 'alcohol_special' === $layout );
			}
			$admin_rows[] = array(
				$id,
				$p->post_title,
...
e49fz_csv(
	$evidence . '/v9-06e49-freeze-after-fix01-full-service-inventory.csv',
...
copy( $evidence . '/v9-06e49-freeze-after-fix01-full-service-inventory.csv', $bak . '/exports/full-service-inventory.csv' );

e49fz_csv(
	$evidence . '/v9-06e49-freeze-after-fix01-postmeta-inventory.csv',
	array( 'post_id', 'title', 'post_type', 'status', 'meta_rows', 'editor_role', 'layout_variant', 'page_layout_mode', 'url' ),
	$postmeta_inv
);
copy( $evidence . '/v9-06e49-freeze-after-fix01-postmeta-inventory.csv', $bak . '/exports/postmeta/postmeta-inventory.csv' );

e49fz_csv(
	$evidence . '/v9-06e49-freeze-after-fix01-acf-inventory.csv',
	array( 'post_id', 'title', 'freeze_category', 'editor_role', 'layout', 'populated_required_text', 'images_ok', 'repeaters_ok', 'url' ),
	$acf_inv_rows
);
copy( $evidence . '/v9-06e49-freeze-after-fix01-acf-inventory.csv', $bak . '/exports/admin-layout/acf-inventory.csv' );

e49fz_csv(
	$evidence . '/v9-06e49-freeze-after-fix01-admin-validation.csv',
...
copy( $evidence . '/v9-06e49-freeze-after-fix01-admin-validation.csv', $bak . '/exports/admin-layout/admin-validation.csv' );

e49fz_csv(
	$evidence . '/v9-06e49-freeze-after-fix01-service-content-validation.csv',
...
copy( $evidence . '/v9-06e49-freeze-after-fix01-service-content-validation.csv', $bak . '/exports/admin-layout/service-content-validation.csv' );

e49fz_csv(
	$evidence . '/v9-06e49-freeze-after-fix01-no-alcohol-copy-paste.csv',
	array( 'post_id', 'title', 'alcohol_terms_expected', 'alcohol_terms_found', 'checked_sources', 'result', 'notes' ),
	$alcohol_rows
);
copy( $evidence . '/v9-06e49-freeze-after-fix01-no-alcohol-copy-paste.csv', $bak . '/exports/admin-layout/no-alcohol-copy-paste.csv' );
...
foreach ( $routes as $r ) {
	...
	if ( 'service' === $r['kind'] ) {
		...
		// #78 and #315 must NOT be placeholder.
		$sok = $ok && ! $has_ph && $has_h1 && $has_full && $has_img && $child_ok;
		if ( ( 78 === $pid || 315 === $pid ) && $has_ph ) {
			$sok = false;
		}
		$fe_note = '';
		if ( 78 === $pid ) {
			$fe_note = 'must_be_full_service';
		} elseif ( 315 === $pid ) {
			$fe_note = 'fix01_restored_must_be_full_service';
		}
		$fe_rows[] = array(
			$pid,
			get_the_title( $pid ),
...
$role315   = (string) get_post_meta( 315, 'service_editor_role', true );
$layout315 = (string) get_post_meta( 315, 'service_layout_variant', true );
$accepted_rows[] = array(
	'#315 final state (E49-FIX01)',
	'service/service_general',
	$role315 . '/' . $layout315,
	( 'service' === $role315 && in_array( $layout315, array( 'service_general', 'alcohol_special' ), true ) ) ? 'PASS' : 'FAIL',
	'restored by E49-FIX01; freeze retry expects Услуга',
);

file_put_contents( $bak . '/frontend/snapshot-index.tsv', implode( "\n", $snap_index ) );
e49fz_csv(
	$evidence . '/v9-06e49-freeze-after-fix01-frontend-validation.csv',
...
copy( $evidence . '/v9-06e49-freeze-after-fix01-frontend-validation.csv', $bak . '/frontend/frontend-validation.csv' );

e49fz_csv(
	$evidence . '/v9-06e49-freeze-after-fix01-accepted-pages-validation.csv',
	array( 'page_or_route', 'expected', 'actual', 'result', 'notes' ),
	$accepted_rows
);
copy( $evidence . '/v9-06e49-freeze-after-fix01-accepted-pages-validation.csv', $bak . '/frontend/accepted-pages-validation.csv' );

e49fz_csv(
	$evidence . '/v9-06e49-freeze-after-fix01-route-smoke.csv',
	array( 'route', 'http', 'result', 'notes' ),
	$smoke_rows
);
copy( $evidence . '/v9-06e49-freeze-after-fix01-route-smoke.csv', $bak . '/frontend/route-smoke.csv' );
...
e49fz_csv(
	$evidence . '/v9-06e49-freeze-after-fix01-source-runtime-sync.csv',
	array( 'file', 'source_path', 'runtime_path', 'hash_match', 'result', 'notes' ),
	$sync_rows
);
copy( $evidence . '/v9-06e49-freeze-after-fix01-source-runtime-sync.csv', $bak . '/hashes/source-runtime-sync.csv' );
...
$summary = array(
	'phase'              => 'V9-06E49-FULL-SERVICE-ROLLOUT-FREEZE-AFTER-FIX01',
	'backup'             => $bak,
	'db_writes'          => $db_writes,
	'publish_count'      => count( $publish_services ),
	...
	'#78_layout'         => $layout78,
	'#315_role'          => $role315,
	'#315_layout'        => $layout315,
	'timestamp'          => gmdate( 'c' ),
);
file_put_contents( $bak . '/freeze-summary.json', wp_json_encode( $summary, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE ) );
file_put_contents( $evidence . '/v9-06e49-freeze-after-fix01-summary.json', wp_json_encode( $summary, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE ) );

echo 'E49_FREEZE_AFTER_FIX01_VALIDATE_OK'
	. ' db_writes=' . $db_writes

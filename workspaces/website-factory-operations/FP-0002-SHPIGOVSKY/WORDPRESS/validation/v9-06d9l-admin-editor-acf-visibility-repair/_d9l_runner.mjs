/**
 * FP-0002 V9-06D9-L — frontend regression + admin screenshot runner.
...
async function captureFrontend() {
  ...
  const shots = [
    { file: 'runtime-home-full-desktop-after-d9l.png', url: runtimeUrl + '/', size: '1440,9000' },
    { file: 'runtime-home-full-mobile-after-d9l.png', url: runtimeUrl + '/', size: '390,8000' },
    { file: 'runtime-hero-gallery-after-d9l.png', url: runtimeUrl + '/#home-gallery', size: '1440,1200' },
    { file: 'runtime-service-74-after-d9l.png', url: runtimeUrl + '/uslugi/zavisimosti/lechenie-alkogolnoy-zavisimosti/', size: '1440,4000' },
    { file: 'runtime-contacts-after-d9l.png', url: runtimeUrl + '/kontakty/', size: '1440,3000' },
  ];
...
async function main() {
  ...
  if (mode === 'admin-after' || mode === 'all') {
    await captureAdmin('wp-admin-home-edit-screen-after-classic-editor-d9l.png');
    await captureAdmin('wp-admin-home-acf-fields-after-d9l.png');
    await captureAdmin('wp-admin-home-hero-gallery-fields-after-d9l.png');
  }
  visualResult.screenshots = manifest;
  visualResult.result = manifest.filter((m) => m.captured).length >= 5 ? 'PASS' : 'PARTIAL';
  writeFileSync(join(evidenceDir, 'screenshot-manifest.json'), JSON.stringify({ phase: 'V9-06D9-L', capturedAt, items: manifest }, null, 2) + '\n');

...
echo "  password_configured = " . ( MailOps::password_is_configured() ? 'YES' : 'NO' ) . "\n";
echo "  recipients      = " . MailOps::recipient_count() . "\n";
...
if ( 'smtp.beget.com' === $cfg_before['smtp_host']
	...
	&& MailOps::ENCRYPTION_NONE === $cfg_before['smtp_encryption']
) {
...
if ( $mismatch ) {
	...
	$cfg_corrected                    = MailOps::get_config();
	$cfg_corrected['smtp_encryption'] = MailOps::ENCRYPTION_SSL;
	// Save via option directly (MailOps::save_from_post not used — password rewrite avoided).
	update_option( MailOps::OPTION_CONFIG, $cfg_corrected, false );
...
echo "  password_configured = " . ( MailOps::password_is_configured() ? 'YES' : 'NO' ) . "\n";
echo "  is_complete     = " . ( MailOps::is_complete() ? 'YES' : 'NO' ) . "\n";
echo "  state           = " . MailOps::state() . "\n";
echo "\n";
...
if ( $sent ) {
	...
	echo "  MailOps state is now: " . MailOps::state() . "\n";
	echo "\n";

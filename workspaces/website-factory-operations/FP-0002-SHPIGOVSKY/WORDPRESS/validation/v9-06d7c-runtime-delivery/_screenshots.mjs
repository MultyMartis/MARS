/**
 * FP-0002 V9-06D7-C — post-delivery screenshot capture via Chrome headless.
 */
...
for (const vp of viewports) {
  for (const route of routes) {
    const file = `${vp.name}-${route.slug}-after-d7c.png`;
...
const screenshotManifest = {
  ...
  evidence_root: 'WORDPRESS/validation/v9-06d7c-runtime-delivery/screenshots/',
  browser: chrome,
...
const visualResult = {
  ...
  services_hub_d7c_sections_visible: captured > 0,
  service_groups_cards_visible: captured > 0,
  global_shell_styled: captured > 0,
  header_footer_visible: 'ASSUMED_FROM_ROUTE_SMOKE',
  home_stable_after_d7c: captured > 0,
  ...
  deferred_sections_4_of_10: true,

# V9-06E54 pre-write backup — STOP if DB dump fails
$ErrorActionPreference = "Stop"
$ts = Get-Date -Format "yyyyMMdd-HHmmss"
$backupRoot = "X:\MARS-Localhost\backups\wordpress\projects\shpigovsky\v9-06e54-after-web-gpt-chat-migration-before-floating-header-work-$ts"
$siteRoot = "X:\MARS-Localhost\sites\wordpress\projects\shpigovsky"
$rtTheme = Join-Path $siteRoot "wp-content\themes\shpigovsky"
$rtPlugin = Join-Path $siteRoot "wp-content\plugins\shpigovsky-core"
$rtAcf = Join-Path $siteRoot "wp-content\acf-json"
$wpConfig = Join-Path $siteRoot "wp-config.php"
$mysqldump = "X:\MARS-Localhost\laragon\bin\mysql\mysql-8.4.3-winx64\bin\mysqldump.exe"
$evidence = "X:\AI MARS\workspaces\website-factory-operations\FP-0002-SHPIGOVSKY\REPORTS\evidence"

New-Item -ItemType Directory -Force -Path $backupRoot | Out-Null
@("db", "theme", "plugin", "acf-json", "meta", "hashes", "runtime-snapshot") | ForEach-Object {
    New-Item -ItemType Directory -Force -Path (Join-Path $backupRoot $_) | Out-Null
}

Write-Output "BACKUP_ROOT=$backupRoot"
Write-Output "PURPOSE=after-web-gpt-chat-migration-before-floating-header-work"
Write-Output "BASELINE=E53"
Write-Output "WAVE=V9-06E54"

# Git HEAD snapshot
Push-Location "X:\AI MARS"
$gitBranch = git branch --show-current 2>$null
$gitHead = git rev-parse HEAD 2>$null
Pop-Location

$dbFile = Join-Path $backupRoot "db\mars_wp_fp0002.sql"
$stderr = Join-Path $backupRoot "db\mysqldump-stderr.txt"
$prevEap = $ErrorActionPreference
$ErrorActionPreference = "Continue"
cmd /c "`"$mysqldump`" --host=127.0.0.1 --user=mli_shpigovsky_app --password=9st4UPjdkc5MXyuNKEGTQaS0V7AD1ClR --no-tablespaces --single-transaction --routines --triggers mars_wp_fp0002 1>`"$dbFile`" 2>`"$stderr`""
$dumpExit = $LASTEXITCODE
$ErrorActionPreference = $prevEap
$dbSize = 0
if (Test-Path $dbFile) { $dbSize = (Get-Item $dbFile).Length }
if ($dumpExit -ne 0 -or $dbSize -lt 100000) {
    Write-Output "STOP - DB backup failed exit=$dumpExit size=$dbSize"
    if (Test-Path $stderr) { Get-Content $stderr }
    exit 2
}
$dbHash = (Get-FileHash $dbFile -Algorithm SHA256).Hash
Write-Output "DB_OK size=$dbSize sha256=$dbHash"

# Runtime project snapshot (exclude heavy caches if present)
$excludeDirs = @("node_modules", ".git")
robocopy $siteRoot (Join-Path $backupRoot "runtime-snapshot\shpigovsky") /E /XD node_modules .git /NFL /NDL /NJH /NJS /nc /ns /np | Out-Null

robocopy $rtTheme (Join-Path $backupRoot "theme\shpigovsky") /E /NFL /NDL /NJH /NJS /nc /ns /np | Out-Null
robocopy $rtPlugin (Join-Path $backupRoot "plugin\shpigovsky-core") /E /NFL /NDL /NJH /NJS /nc /ns /np | Out-Null
robocopy $rtAcf (Join-Path $backupRoot "acf-json") /E /NFL /NDL /NJH /NJS /nc /ns /np | Out-Null
if (Test-Path $wpConfig) {
    Copy-Item $wpConfig (Join-Path $backupRoot "runtime-snapshot\wp-config.php") -Force
}

function Get-TreeHash([string]$dir) {
    if (-not (Test-Path $dir)) { return "MISSING" }
    $files = Get-ChildItem -Path $dir -Recurse -File | Sort-Object FullName
    $sha = [System.Security.Cryptography.SHA256]::Create()
    $ms = New-Object System.IO.MemoryStream
    foreach ($f in $files) {
        $rel = $f.FullName.Substring($dir.Length).TrimStart('\')
        $hash = (Get-FileHash $f.FullName -Algorithm SHA256).Hash
        $line = "$rel|$hash`n"
        $bytes = [System.Text.Encoding]::UTF8.GetBytes($line)
        $ms.Write($bytes, 0, $bytes.Length)
    }
    $ms.Position = 0
    $hashBytes = $sha.ComputeHash($ms)
    return ([System.BitConverter]::ToString($hashBytes) -replace '-', '').ToLowerInvariant()
}

$themeHash = Get-TreeHash (Join-Path $backupRoot "theme\shpigovsky")
$pluginHash = Get-TreeHash (Join-Path $backupRoot "plugin\shpigovsky-core")
$acfHash = Get-TreeHash (Join-Path $backupRoot "acf-json")
Set-Content (Join-Path $backupRoot "hashes\theme-tree.sha256") $themeHash
Set-Content (Join-Path $backupRoot "hashes\plugin-tree.sha256") $pluginHash
Set-Content (Join-Path $backupRoot "hashes\acf-json-tree.sha256") $acfHash

$keyFiles = @(
    "assets\css\v9-style.css",
    "assets\js\v9-shell.js",
    "template-parts\layout\header.php",
    "template-parts\navigation\offcanvas.php"
)
$keyHashLines = @()
foreach ($rel in $keyFiles) {
    $p = Join-Path $rtTheme $rel
    if (Test-Path $p) {
        $h = (Get-FileHash $p -Algorithm SHA256).Hash
        $keyHashLines += "$rel=$h"
    }
}
Set-Content (Join-Path $backupRoot "hashes\key-files.sha256") ($keyHashLines -join "`n")

$css = Join-Path $rtTheme "assets\css\v9-style.css"
$cssHash = if (Test-Path $css) { (Get-FileHash $css -Algorithm SHA256).Hash } else { "MISSING" }
Set-Content (Join-Path $backupRoot "hashes\operator-css.sha256") "v9-style.css=$cssHash prefix=$($cssHash.Substring(0,8))"

$manifest = @"
DB=$dbHash
DB_SIZE_BYTES=$dbSize
THEME=$themeHash
PLUGIN=$pluginHash
ACF=$acfHash
OPERATOR_CSS=$cssHash
GIT_BRANCH=$gitBranch
GIT_HEAD=$gitHead
RUNTIME_URL=http://shpigovsky.test/
TIMESTAMP=$ts
PURPOSE=after-web-gpt-chat-migration-before-floating-header-work
BASELINE=E53
WAVE=V9-06E54
"@
Set-Content (Join-Path $backupRoot "hashes\source-hash-manifest.txt") $manifest

$backupInfo = @"
# BACKUP-INFO — FP-0002 V9-06E54 Pre-Work

- **Purpose marker:** `after-web-gpt-chat-migration-before-floating-header-work`
- **First backup after Web-GPT chat migration** (WP Forge Proger pt.2)
- **Created before** any V9-06E54 floating-header mutations
- **Accepted baseline:** E53 (admin UX freeze)
- **Runtime URL:** http://shpigovsky.test/
- **Database:** mars_wp_fp0002
- **Timestamp:** $ts
- **Git branch:** $gitBranch
- **Git HEAD:** $gitHead
- **Operator runtime CSS:** v9-style.css hash prefix $($cssHash.Substring(0,8)) — preserved in backup theme snapshot
- **DB dump:** db/mars_wp_fp0002.sql ($dbSize bytes, SHA256 $dbHash)
"@
Set-Content (Join-Path $backupRoot "BACKUP-INFO.md") $backupInfo -Encoding UTF8
Set-Content (Join-Path $backupRoot "BACKUP-OK.txt") "OK $ts DB=$dbHash THEME=$themeHash PLUGIN=$pluginHash ACF=$acfHash CSS=$cssHash" -Encoding UTF8

New-Item -ItemType Directory -Force -Path $evidence | Out-Null
Set-Content -Path (Join-Path $evidence "v9-06e54-backup-path.txt") -Value $backupRoot -Encoding UTF8

Write-Output "BACKUP_COMPLETE=$backupRoot"
Write-Output "DB_SHA256=$dbHash"
Write-Output "THEME_SHA256=$themeHash"
Write-Output "CSS_PREFIX=$($cssHash.Substring(0,8))"

...
const { chromium } = require('X:/AI MARS/workspaces/fp-0002-shpigovsky-v7/node_modules/playwright-core');
...
async function main() {
  ...
  for (const vp of viewports) {
    ...
    const up = scroll.steps.find((s) => s.phase === 'scroll-up');
    if (up && !up.floatingVisible && vp.id === 'desktop-1440') {
      await page.screenshot({ path: path.join(OUT, 'desktop-1440-after-scroll-up.png'), fullPage: false });
    }

/**
 * FP-0002 V9-06D7-E — post-delivery screenshot capture via Chrome headless.
 */
...
for (const vp of viewports) {
  for (const route of routes) {
    const file = `${vp.name}-${route.slug}-after-d7e.png`;
...
const screenshotManifest = {
  ...
  evidence_root: 'WORDPRESS/validation/v9-06d7e-runtime-delivery/screenshots/',
  browser: chrome,
...
const visualResult = {
  ...
  contacts_d7e_sections_visible: manifest.some((m) => m.route === '/kontakty/' && m.result === 'CAPTURED'),
  location_contact_structure_visible: 'ASSUMED_FROM_SECTION_SMOKE',
  map_photo_media_may_be_omitted: true,
  global_shell_styled: captured > 0,
  home_stable_after_d7e: manifest.some((m) => m.route === '/' && m.result === 'CAPTURED'),
  services_hub_stable_after_d7e: manifest.some((m) => m.route === '/uslugi/' && m.result === 'CAPTURED'),
  service_templates_stable_after_d7e: manifest.some((m) => m.route.includes('/uslugi/') && m.route !== '/uslugi/' && m.result === 'CAPTURED'),
  ...
  deferred_media_gaps: ['map PNG assets', 'rehabilitation interior photo'],

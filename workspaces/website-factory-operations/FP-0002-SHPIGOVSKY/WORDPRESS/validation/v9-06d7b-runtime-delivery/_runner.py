...
EVIDENCE = WP_ROOT / "validation" / "v9-06d7b-runtime-delivery"
...
EXPECTED_HEAD = "c006edeb47afd25cc4e3f6dfe459e8d46993472b"

CHANGED_PHP = [
    "theme/shpigovsky/functions.php",
    "theme/shpigovsky/front-page.php",
    "theme/shpigovsky/inc/home-helpers.php",
    "theme/shpigovsky/template-parts/layout/header.php",
    ...
    "theme/shpigovsky/template-parts/home/rehabilitation-program.php",
    "theme/shpigovsky/template-parts/home/gallery.php",
    "theme/shpigovsky/template-parts/home/articles-teaser.php",
    "theme/shpigovsky/template-parts/home/faq.php",
    "theme/shpigovsky/template-parts/components/final-form.php",
]
...
HOME_SECTION_CHECKS = [
    {"key": "front_page_orchestration", "pattern": r"site-main--front", "required": True, "omit_if_empty": False},
    {"key": "site-main--front", "pattern": r'class=["\'][^"\']*site-main--front', "required": True, "omit_if_empty": False},
    {"key": "hero", "pattern": r'class=["\'][^"\']*hero--home', "required": True, "omit_if_empty": False},
    {"key": "feature-grid", "pattern": r'class=["\'][^"\']*home-feature-grid', "required": False, "omit_if_empty": True},
    {"key": "treatment-prevention", "pattern": r'class=["\'][^"\']*home-treatment-prevention', "required": True, "omit_if_empty": False},
    {"key": "rehabilitation-program", "pattern": r'class=["\'][^"\']*home-rehabilitation-program', "required": True, "omit_if_empty": False},
    {"key": "gallery", "pattern": r'class=["\'][^"\']*home-gallery', "required": False, "omit_if_empty": True},
    {"key": "articles-teaser", "pattern": r'class=["\'][^"\']*home-articles', "required": False, "omit_if_empty": True},
    {"key": "faq", "pattern": r'class=["\'][^"\']*faq\b', "required": False, "omit_if_empty": True},
    {"key": "final-form", "pattern": r'class=["\'][^"\']*final-form', "required": True, "omit_if_empty": False},
    {"key": "deferred_sections_documented", "pattern": None, "required": False, "omit_if_empty": False, "static": True},
]
...
def http_fetch(url):
    try:
        req = Request(url, method="GET", headers={"User-Agent": "MARS-V9-06D7B-readonly"})
...
def make_checkpoint(theme_baseline):
    ...
    root = BACKUP_ROOT / f"v9-06d7b-home-runtime-delivery-pre-{stamp}"
    ...
    rollback_text = (
        "D7-B home template runtime delivery rollback — theme only.\n"
...
def apply_delivery(dry_run):
    ...
    for action in dry_run["actions"]:
        ...
        temp = dst.with_name(dst.name + ".tmp-v9-06d7b")
...
def home_section_smoke():
    url = f"{DOMAIN.rstrip('/')}/"
    fetched = http_fetch(url)
    body = fetched.get("body", "")
    checks = []
    failures = []
    for check in HOME_SECTION_CHECKS:
        if check.get("static"):
            row = {
                "section": check["key"],
                "present": True,
                "expected_if_empty": "12/20 V9 sections deferred per D7-B scope",
                "result": "PASS",
            }
            checks.append(row)
            continue
    ...
    data = {
        ...
        "checks": checks,
        "failures": failures,
        "implemented_sections_visible": sum(1 for c in checks if c.get("present") and c["section"] not in {"deferred_sections_documented", "front_page_orchestration"}),
        "result": "PASS" if not failures else "FAIL",
    }
    write_json("home-section-render-smoke.json", data)
    if failures:
        raise RuntimeError(f"home section smoke failed: {failures}")
    return data
...
def rollback_readiness(checkpoint, theme_baseline_before):
    ...
    data = {
        ...
        "expected_post_rollback_validation": "Restore theme snapshot; re-run D.5 seven routes HTTP 200; verify D7-A shell intact.",
        "result": "PASS",
    }
...
def main():
    ...
    try:
        ...
        home_section_smoke()
        summary["steps"].append("home-section-smoke")
        asset_smoke()
        ...
        write_json("final-verdict.json", {
            ...
            "home_sections": "VISIBLE",
            "header_footer_assets": "VISIBLE",
            "runtime_mutations": "THEME_FILES_ONLY",
            "db_writes": 0,
            "content_acf_writes": 0,
            "recommended_next_phase": "CREATE_V9_06D7C_SERVICES_HUB_TEMPLATE_SOURCE_TASK",
            "v9_06d7c": "READY FOR OPERATOR REVIEW",
        })
        summary["result"] = "PASS"

		...
		return array(
			...
			'fields'                => array(
				...
				self::section_message(
					...
					__( 'Выключатель: ВЫКЛ — блок скрыт. ВКЛ + локальные поля заполнены — контент этой услуги. ВКЛ + локальные поля пустые — повторяемый блок Comfort Requirements (Настройки сайта → Повторяемые блоки). Аварийный резерв темы — только технический запас.', 'shpigovsky-core' )
				),
				self::toggle(
				...
				self::section_message(
					...
					__( 'Уникальный контент раздела: заголовки, тексты, изображения и карточки. Выключатель: ВЫКЛ — блок скрыт. ВКЛ + локальные карточки заполнены — карточки этой страницы. ВКЛ + локальные карточки пустые — повторяемые карточки (Настройки сайта → Подход к лечению — карточки). Локальные значения не перезаписываются глобальными.', 'shpigovsky-core' )
				),
				self::toggle(
				...
				array(
					...
					'type'              => 'repeater',
					'instructions'      => __( 'Если карточки заполнены здесь, они заменяют повторяемые карточки для этой услуги. Пустые ряды: на сайте подставляются карточки из «Настройки сайта → Подход к лечению — карточки». На макете «Заглушка» блок на сайте не выводится. Аварийный резерв темы — только технический запас.', 'shpigovsky-core' ),
					'required'          => 0,
					'conditional_logic' => self::when_section(),
					'layout'            => 'row',
					'button_label'      => __( 'Добавить карточку', 'shpigovsky-core' ),

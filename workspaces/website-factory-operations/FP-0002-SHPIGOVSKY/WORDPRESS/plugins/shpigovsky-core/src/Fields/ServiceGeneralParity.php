		...
		return array(
			...
			'fields'                => array(
				...
				self::section_message(
					...
					__( 'Уникальный контент услуги: заголовки, тексты, изображение команды и карточки. Выключатель: ВЫКЛ — блок скрыт. ВКЛ + локальные карточки заполнены — карточки этой услуги. ВКЛ + локальные карточки пустые — повторяемые карточки (Настройки сайта → Подход к лечению — карточки). Локальные значения не перезаписываются глобальными. На макете «Заглушка» блок на сайте не выводится.', 'shpigovsky-core' )
				),
				self::toggle(
				...
				array(
					...
					'type'              => 'repeater',
					'instructions'      => __( 'Если карточки заполнены здесь, они заменяют повторяемые карточки для этой услуги. Пустые ряды: на сайте подставляются карточки из «Настройки сайта → Подход к лечению — карточки». Не оставляйте Lorem/DEMO — такие тексты на фронте заменяются техническим запасным описанием. На макете «Заглушка» блок на сайте не выводится.', 'shpigovsky-core' ),
					'required'          => 0,
					'conditional_logic' => self::when_service(),
					'layout'            => 'row',

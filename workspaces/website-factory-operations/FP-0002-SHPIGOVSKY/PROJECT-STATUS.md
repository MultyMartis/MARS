...
**Current WordPress phase:** `FP-0002 PROD-P17 PRE-CUTOVER / CONT1` — **PASS**. `FP-0002 P17 PRE-CUTOVER NOW INCLUDES A VERIFIED LEGACY REDIRECT LAYER AND A COMPLETE NAMESERVER-MIGRATION PLAN`. Seven legacy 301s live on `http://shpigovsky.beget.tech/` (path-relative `.htaccess`, no future-host hardcode). `shpigovsky.ru` DNS inventoried (NS `ns1/ns2.hosting.reg.ru`, mail REG.RU hosting); Beget NS **not** switched. Core `0.3.8-p17`. Open tails: Beget zone panel prep → NS/SSL/siteurl → SMTP → indexing → sitemaps → crawl — `REPORTS/OPEN-ITEMS-FP-0002-AFTER-P17.md`. Report: `REPORTS/REPORT-FP-0002-PROD-P17-PRE-CUTOVER.md`. Evidence: `REPORTS/evidence/prod-p17-precutover/`. WPilot `write_enabled=false`.

...
**Last updated:** 2026-09-14 (Service approach cards render + reusable fallback 01 — **PASS_WITH_RESIDUALS**)

**Current project state:** **PRODUCTION / MAINTENANCE — STABLE**. Domain `https://shpigovsky.ru/`. Core **`0.3.32-blog-hub-announcement-01`** (this wave did not bump core version). Indexing **OPEN — HUMAN-APPROVED** (`blog_public=1`). P18G guard **ACTIVE**; watchdog **ACTIVE**. Physical `/robots.txt` remains Olya-approved SEO policy; robots hashes in reports are evidence, not eternal policy; fresh Olya production robots always wins. Forms **ACTIVE**; SMTP **VERIFIED / ACTIVE**; native first-party anti-spam **ACTIVE**. Privacy **ACTIVE**; Metrika **CONSENT-GATED**. Specialists hub canonical: `https://shpigovsky.ru/specialisty/` (Page `#1030`, dedicated Hub template, automatic CPT listing, existing cards, `has_archive=false`, breadcrumbs intentionally absent). CPT singles: `/specialisty/{slug}/`. Old `/specyalisty/` is **DEPRECATED / REDIRECT-ONLY**. SEO title / Meta Description are editor-owned source truth (one SEO owner). **JSON-LD Schema.org graph LIVE** via `shpigovsky-core` module `structured-data.schema-org` (single `<head>` owner; no manual JSON-LD UI). **Open Graph LIVE** via `shpigovsky-core` module `open-graph.meta` (single `<head>` owner, separate from SEO + JSON-LD). Contacts maps: full Yandex Constructor code remains Admin input; per-row scroll toggle default/legacy **false**; no global unsafe script allowance. Blog Hub card preview owner: ACF **`article_hub_announcement`** (`Анонс статьи для хаба`) → native `get_the_excerpt()` fallback; `article_lead` remains article hero owner. Open items: `REPORTS/OPEN-ITEMS-FP-0002-PRODUCTION-MAINTENANCE.md`. Handoff: `REPORTS/FP-0002-NEXT-WEBGPT-HANDOFF.md`.

**Current production-code phase:** `FP-0002 SERVICE APPROACH CARDS RENDER + REUSABLE FALLBACK 01` — **PASS_WITH_RESIDUALS**. Approach cards precedence: OFF → hidden; ON + local cards → local; ON + local empty → reusable `fp02-block-approach-cards`. Local editorial values are never auto-overwritten by global defaults. Genotyping `#1889` keeps local 4 cards (`source=local`). Stages Comfort Requirements fallback remains healthy. Pending multi-image child-card production delta remains preserved/uncommitted. Report: `REPORTS/REPORT-FP-0002-SERVICE-APPROACH-CARDS-RENDER-REUSABLE-FALLBACK-01.md`. Evidence: `REPORTS/evidence/prod-maint-service-approach-cards-reusable-fallback-01/`.

**Prior production-code phase:** `FP-0002 SERVICE STAGES REUSABLE-BLOCK FALLBACK REPAIR 01` — **PASS**. Stages block precedence: OFF → hidden; ON + local meaningful → local; ON + local empty → reusable Comfort Requirements (`fp02-block-comfort-requirements`). Genotyping `#1889` `/uslugi/genotipirovenie/` keeps empty local section fields and renders reusable content. Pending multi-image child-card production delta remains preserved/uncommitted. Report: `REPORTS/REPORT-FP-0002-SERVICE-STAGES-REUSABLE-BLOCK-FALLBACK-REPAIR-01.md`. Evidence: `REPORTS/evidence/prod-maint-service-stages-reusable-fallback-01/`.

**Prior production-code phase:** `FP-0002 SERVICE CHILD-CARDS ADAPTIVE LAYOUT 01` — **PASS_WITH_RESIDUALS** (+ pending uncommitted multi-image composition 02 on preserved worktree). Service-stack child tile block: content-aware CSS Grid, optional deterministic featured image card, compact title-only cards, default heading `Состояния, которые мы лечим` when ACF override empty. Report: `REPORTS/REPORT-FP-0002-SERVICE-CHILD-CARDS-ADAPTIVE-LAYOUT-01.md`. Evidence: `REPORTS/evidence/prod-maint-service-child-cards-01/`.

**Prior production-code phase:** `FP-0002 BLOG HUB DEDICATED ANNOUNCEMENT FIELD 01` — **PASS**. Dedicated ACF textarea for `/blog/` article-card announcement; empty field keeps historical excerpt fallback; Lead / SEO / OG / Schema remain independent. Core `0.3.32-blog-hub-announcement-01`. Report: `REPORTS/REPORT-FP-0002-BLOG-HUB-DEDICATED-ANNOUNCEMENT-FIELD-01.md`. Evidence: `REPORTS/evidence/prod-blog-hub-announcement-01/`.

**Prior maintenance phase:** `FP-0002 GLOBAL OPEN GRAPH META 01` — **PASS_WITH_ATTENTION**. Server-rendered `og:*` tags on all sampled public surfaces; SEO title/meta description reused as OG title/description (description omitted when empty); canonical absolute URLs; conservative `og:type` (`website` / `article`); page-aware images; no manual OG UI; Schema.org unchanged. Core was `0.3.31-open-graph-01`. Report: `REPORTS/REPORT-FP-0002-GLOBAL-OPEN-GRAPH-META-01.md`. Evidence: `REPORTS/evidence/prod-open-graph-01/`. ATTENTION: many inner pages omit `og:description` (no Admin meta description set); external Facebook OG debugger not run in agent session.

**Prior maintenance phase:** `FP-0002 PROD YANDEX SCHEMA.ORG STRUCTURED DATA 01` — **PASS_WITH_ATTENTION**. One server-rendered JSON-LD `@graph` per public page from Admin-owned data; `MedicalClinic` org + separate branch locations; page-aware types (`ContactPage`, `CollectionPage`, `Service`, `Person`, `Article`, `AboutPage` on `/o-centre/` hub); conditional `BreadcrumbList`; no Product/Offer/QAPage/fake ratings/universal Physician. Core was `0.3.30-yandex-schema-org-01`. Report: `REPORTS/REPORT-FP-0002-PROD-YANDEX-SCHEMA-ORG-01.md`. Evidence: `REPORTS/evidence/prod-yandex-schema-org-01/`.

**Prior maintenance phase:** `FP-0002 PROD-MAINT SPECIALISTS NAV + SEO + MAPS 01` — **PASS_WITH_ATTENTION**. Hub `/specialisty/` — breadcrumbs removed entirely (not CSS hide). Admin SEO (`fp02_seo_title` / `fp02_seo_description`) now applies to static front page and posts page; homepage/contacts verified in source. Contacts `/kontakty/` — Yandex Constructor embeds live; per-map `map_scroll` toggle (default OFF). Core was `0.3.29-specialists-nav-seo-maps-01`. Report: `REPORTS/REPORT-FP-0002-PROD-MAINT-SPECIALISTS-NAV-SEO-MAPS-01.md`. Evidence: `REPORTS/evidence/prod-maint-specialists-nav-seo-maps-01/`.

**Prior maintenance phase:** `FP-0002 PROD-MAINT SPECIALISTS CANONICAL URL MIGRATION 01` — **PASS**. Operator-approved spelling `specialisty` is now canonical. Page `#1030` slug `specialisty`; CPT rewrite base `specialisty` (`has_archive=false`); hub `https://shpigovsky.ru/specialisty/`; singles `/specialisty/{slug}/`. Deprecated `/specyalisty/` → **301** (htaccess + `SpecialistLegacyRedirect`). Sitemap/canonicals/internal links use new URLs only. Core was `0.3.28-specialists-canonical-url`. Report: `REPORTS/REPORT-FP-0002-PROD-MAINT-SPECIALISTS-CANONICAL-URL-MIGRATION-01.md`. Evidence: `REPORTS/evidence/prod-maint-specialists-canonical-url-migration-01/`.

**Prior maintenance phase:** `FP-0002 PROD-MAINT SPECIALISTS HUB LAYOUT POLISH 01` — **PASS**. Hub-only: remove `internal-page-nav` shell (breadcrumbs kept); trim `.plain-page-content__body` CSS; omit nested rehab `.container` on Hub. Report: `REPORTS/REPORT-FP-0002-PROD-MAINT-SPECIALISTS-HUB-LAYOUT-POLISH-01.md`.

**Prior maintenance phase:** `FP-0002 PROD-MAINT SPECIALISTS HUB ADMIN UX 01` — **PASS**. ACFE checkbox fatal compatibility + reusable enable/disable UX; core `0.3.27-specialists-hub-admin-ux`. Evidence: `REPORTS/evidence/prod-maint-specialists-hub-admin-ux-01/`.

**Prior maintenance phase:** `FP-0002 PROD-MAINT SPECIALISTS HUB 01` — **PASS**. Page `#1030` uses `page-templates/specialists-hub.php`; published CPT specialists render via `shpigovsky_get_specialists_cards()`. Historical hub URL spelling at that wave was `/specyalisty/` (now migrated). Report: `REPORTS/REPORT-FP-0002-PROD-MAINT-SPECIALISTS-HUB-01.md`.
...
**Prior WordPress phase:** `FP-0002 PROD-P18J INDEXING GUARD QA NOISE CLEANUP + ALERT VERIFICATION` — **PASS**. Forensics classified four 2026-08-19 Activity Log rows (`p18g_qa_guard_test`) as **synthetic P18G guard QA**, not real incidents. Production indexing remained **OPEN — HUMAN-APPROVED** throughout. P18J deployed `IndexingQaContext` (explicit `FP02_INDEXING_QA_MODE_AUTHORIZED` gate), QA evidence sink, Activity Log presentation normalization for historical QA rows, alert suppression boundary (QA only when effective state stays OPEN). Core `0.3.22-p18j`. Guard **ACTIVE**; watchdog **ACTIVE**. Report: `REPORTS/REPORT-FP-0002-PROD-P18J-INDEXING-QA-NOISE-CLEANUP.md`. Evidence: `REPORTS/evidence/prod-p18j-indexing-qa-noise/`.

**Prior WordPress phase:** `FP-0002 PROD-P18I FINAL LAUNCH CLOSEOUT` — **PASS**. Fresh production intake after Olya's editorial work; indexing **OPEN — HUMAN-APPROVED**; P18G guard active; sitemap `https://shpigovsky.ru/wp-sitemap.xml` valid (58 URLs); final crawl **CLEAN** (107 URLs, 0 CRITICAL). Bounded fix: legacy `beget.tech` internal links normalized in theme (`0.3.21-p18i`). GSC/Yandex submission **AUTH BLOCKER** (operator UI). Privacy regression **PASS**. Project phase: **PRODUCTION / MAINTENANCE**. Report: `REPORTS/REPORT-FP-0002-PROD-P18I-FINAL-LAUNCH-CLOSEOUT.md`. Baseline: `REPORTS/BASELINE-FP-0002-PRODUCTION-FINAL.md`. Open items: `REPORTS/OPEN-ITEMS-FP-0002-PRODUCTION-MAINTENANCE.md`. Evidence: `REPORTS/evidence/prod-p18i-final-launch-closeout/`.

**Prior WordPress phase:** `FP-0002 PROD-P18H PRIVACY / RETENTION DECISIONS + LAUNCH-TAIL READINESS` — **PASS**. Fresh production intake (SSH + public probes) verified Cookie Policy factually matches runtime; consent evidence **browser-only**; consent lifetime **365d**; UTM `sessionStorage` documented; lead retention **730d recommended** (production config remains **0** — no historical purge). Launch-tail **READY WITH NON-BLOCKING LEGAL NOTE**. Indexing **OPEN — HUMAN-APPROVED**; P18G guard unchanged. Report: `REPORTS/REPORT-FP-0002-PROD-P18H-PRIVACY-RETENTION-DECISIONS.md`. Evidence: `REPORTS/evidence/prod-p18h-privacy-decisions/`. Open items (historical): `REPORTS/OPEN-ITEMS-FP-0002-AFTER-P18H.md`.

**Prior WordPress phase:** `FP-0002 PROD-P18G INDEXING SAFETY GUARD + CRITICAL ADMIN ALERTS` — **PASS**. Forensics proved 2026-08-19 13:13:25 closure was P18D-FU01 technical closeout (`IndexingControl::set_site_indexability(false)` via wp_eval, Activity Log user_id=0 → «Система»). Olya/admin reopened indexing at 19:18:26. P18G deployed guard (`request_state`, `blog_public` filter), multi-surface `IndexingState`, administrator alerts (4 WP admins, not form recipients), hourly watchdog (alert-only). Production remains **INDEXING OPEN**. Core `0.3.17-p18g`. Baseline: `REPORTS/BASELINE-FP-0002-PRODUCTION-P18G.md`. Report: `REPORTS/REPORT-FP-0002-PROD-P18G-INDEXING-SAFETY.md`. Evidence: `REPORTS/evidence/prod-p18g-indexing-safety/`. Open items: `REPORTS/OPEN-ITEMS-FP-0002-AFTER-P18G.md`.

**Prior WordPress phase:** `FP-0002 PROD-P18D-FU01 SMTP CLOSEOUT + OLYA INTAKE` — **PASS**. Fresh production intake proved runtime had not actually landed P18D yet: SMTP was complete-but-failing (`smtp_encryption=none`, `verified=0`, `delivery_active=0`), the pre-cutover suppression MU still existed, and indexing was open. FU01 corrected SMTP to `ssl:465`, verified and activated outbound delivery, physically removed `fp02-pre-cutover-mail-suppression.php`, ran one bounded QA mail/form proof (`MAIL_ACCEPTED`) and deleted the exact QA row, then re-closed indexing via `IndexingControl`. Olya/Admin editorial changes in the Beget DB were preserved as current production truth. Public `https://shpigovsky.ru/` now returns WordPress. Core target: `0.3.16-p18d-fu01`. Baseline: `REPORTS/BASELINE-FP-0002-PRODUCTION-P18D-FU01.md`. Report: `REPORTS/REPORT-FP-0002-PROD-P18D-FU01-SMTP-CLOSEOUT.md`. Evidence: `REPORTS/evidence/prod-p18d-fu01-smtp-closeout/`. Open items: `REPORTS/OPEN-ITEMS-FP-0002-AFTER-P18D.md`. Git replay to canonical remote remains a clean-worktree task; dirty main foreign WIP untouched. WPilot `write_enabled=false`.

**Prior WordPress phase:** `FP-0002 PROD-P18C-FU02 MULTIPLE RECIPIENTS` — **PASS**. Настройки сайта → Почта и формы now has Add/Remove recipient rows (email + label; first = primary). Operator SMTP remained stored (`smtp.beget.com:465`, sender `noreply@shpigovsky.ru`, password **CONFIGURED**). Recipients preserved structurally. SMTP was still **CONFIGURED / NOT VERIFIED** in that wave. Mail suppression **ON**. Core `0.3.14-p18c-fu02`. Report: `REPORTS/REPORT-FP-0002-PROD-P18C-FU02-MULTI-RECIPIENTS.md`.

**Prior WordPress phase:** `FP-0002 PROD-P18C-FU01 ADMIN MENU EXPOSURE` — **PASS**. P18C internals existed; **Настройки сайта → Почта и формы** was an orphan submenu under logical slug `fp02-site-settings` while the visible parent is ACF-redirect `fp02-site-settings-general`. FU01 registers after ACF (`admin_menu` 100) via `OptionsPage::visible_menu_slug()`. Olya/Admin can discover, open, and use the page. **Заявки** reachable. SMTP was **NOT CONFIGURED** at that wave. Mail suppression **ON**. Indexing **CLOSED**. Core `0.3.13-p18c-fu01`. `4/4 SOURCE ↔ PRODUCTION MATCH`. Report: `REPORTS/REPORT-FP-0002-PROD-P18C-FU01-ADMIN-MENU.md`. Evidence: `REPORTS/evidence/prod-p18c-fu01-admin-menu/`. Open items at that time: `REPORTS/OPEN-ITEMS-FP-0002-AFTER-P18C-FU01.md`. Git: isolated worktree; dirty main foreign WIP untouched. WPilot `write_enabled=false`.

**Prior WordPress phase:** `FP-0002 PROD-P18C SMTP / FORMS FOUNDATION` — **PASS** with a **menu-discoverability gap** closed in FU01. One Admin owner implemented; sender `noreply@shpigovsky.ru`; write-only SMTP secret; table `fp02_form_leads`; persist before mail; menu **Заявки**; Metrika goal after backend-confirmed submit. Mail suppression **ON**. SMTP **NOT CONFIGURED / NOT VERIFIED**. Indexing **CLOSED**. Core `0.3.12-p18c`. `14/14 SOURCE ↔ PRODUCTION MATCH` at that wave. Report: `REPORTS/REPORT-FP-0002-PROD-P18C-SMTP-FORMS-FOUNDATION.md`. Evidence: `REPORTS/evidence/prod-p18c-smtp-forms-foundation/`. Open items at that time: `REPORTS/OPEN-ITEMS-FP-0002-AFTER-P18C.md`.

**Prior WordPress phase:** `FP-0002 PROD-P18B DASHBOARD REALITY + INDEXING CONTROL` — **PARTIAL PASS**. MetaCODE Dashboard synced to current operator-visible truth (no stale “future host” / “NS pending”). Safe Admin `SET SITE INDEXABILITY` control LIVE (`manage_options` + nonce + POST + confirm). Indexing **CLOSED** after reversible QA. SMTP mailbox `noreply@shpigovsky.ru` exists; WP SMTP **PENDING** at that time. Public `https://shpigovsky.ru/` latest probe = **Craftum CMS**. Core `0.3.11-p18b`. `6/6 SOURCE ↔ PRODUCTION MATCH`. Open items: `REPORTS/OPEN-ITEMS-FP-0002-AFTER-P18B.md`. Report: `REPORTS/REPORT-FP-0002-PROD-P18B-DASHBOARD-INDEXING.md`. Evidence: `REPORTS/evidence/prod-p18b-dashboard-indexing/`. Git: isolated worktree (see evidence `GIT-CHECKPOINT.json`); dirty main foreign WIP untouched. WPilot `write_enabled=false`.

**Prior WordPress phase:** `FP-0002 PROD-P18A LIVE DOMAIN REALITY + LEGAL STATE FIX` — **PARTIAL PASS**. `OPERATOR LIVE-DOMAIN CUTOVER INTAKEN AS CURRENT PRODUCTION REALITY`. WordPress `home`/`siteurl` = `https://shpigovsky.ru` (operator DB change, not reverted). Public NS = Beget. Public apex still observed serving a **legacy** origin (A `45.130.41.70` @8.8.8.8; local cache `92.255.111.71`); WordPress remains reachable on `http://shpigovsky.beget.tech` inner routes. SSL on the WordPress origin = **IN PROGRESS**; public apex LE cert is valid for the **legacy** host. Legal DEMO banner owner fixed (`legal_demo_marker` `0` → no banner). Indexing **CLOSED**. SMTP **PENDING**. Core `0.3.10-p18a`. Open items: `REPORTS/OPEN-ITEMS-FP-0002-AFTER-P18A.md`. Report: `REPORTS/REPORT-FP-0002-PROD-P18A-LIVE-DOMAIN-LEGAL-STATE.md`. Evidence: `REPORTS/evidence/prod-p18a-live-domain-legal-state/`. Git checkpoint pushed: `d96dfce1f4d8e8d18ba026809923e1e1dbb067c6` + `95ade9bd4baa00f22a80c589e43c55d3ed586e8c` on `origin/mars/canonical-post-recovery` (clean worktree; dirty main foreign WIP untouched). WPilot `write_enabled=false`.

**Prior WordPress phase:** `FP-0002 PROD-P17-FU02 FINAL PRE-CUTOVER TAIL CLOSURE` — **PASS**. `INTERNAL PRE-CUTOVER READINESS = GO`. `MANUAL NS SWITCH = OPERATOR ACTION REQUIRED` **at that time** (now historical — operator performed NS + URL cutover before P18A). `mars-runtime/` resolved and removed. Public webroot hygiene PASS. Users CLEAN. 7/7 legacy 301s re-verified. Cutover DB/file plans executable without discovery. Freeze + NS handoff runbooks ready. Core `0.3.9-p17fu02`. Open items at that time: `REPORTS/OPEN-ITEMS-FP-0002-AFTER-P17-FU02.md`. Report: `REPORTS/REPORT-FP-0002-PROD-P17-FU02-FINAL-PRE-CUTOVER-TAIL.md`. Evidence: `REPORTS/evidence/prod-p17-fu02-final-tail/`. WPilot `write_enabled=false`.

**Prior WordPress phase:** `FP-0002 PROD-P17 PRE-CUTOVER / CONT1` — **PASS**. `FP-0002 P17 PRE-CUTOVER NOW INCLUDES A VERIFIED LEGACY REDIRECT LAYER AND A COMPLETE NAMESERVER-MIGRATION PLAN`. Seven legacy 301s live on `http://shpigovsky.beget.tech/` (path-relative `.htaccess`, no future-host hardcode). `shpigovsky.ru` DNS inventoried (NS `ns1/ns2.hosting.reg.ru`, mail REG.RU hosting); Beget NS **not** switched. Core `0.3.8-p17`. Open tails superseded by FU02. Report: `REPORTS/REPORT-FP-0002-PROD-P17-PRE-CUTOVER.md`. Evidence: `REPORTS/evidence/prod-p17-precutover/`. Git checkpoint pushed: `1b7fb5c47b2c7acd88e4313e64a15f7e59069fa6` on `origin/mars/canonical-post-recovery` (clean worktree; dirty main foreign WIP untouched). WPilot `write_enabled=false`.

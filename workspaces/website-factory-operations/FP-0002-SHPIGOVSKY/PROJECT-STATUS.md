...
**Last updated:** 2026-08-24 (Yandex Schema.org structured data 01 — **PASS_WITH_ATTENTION** → **PRODUCTION / MAINTENANCE — STABLE**)

**Current project state:** **PRODUCTION / MAINTENANCE — STABLE**. Domain `https://shpigovsky.ru/`. Core **`0.3.30-yandex-schema-org-01`**. Indexing **OPEN — HUMAN-APPROVED** (`blog_public=1`). P18G guard **ACTIVE**; watchdog **ACTIVE**. Physical `/robots.txt` remains Olya-approved SEO policy, separate from global OPEN/CLOSED. Forms **ACTIVE**; SMTP **VERIFIED / ACTIVE**; native first-party anti-spam **ACTIVE**. Privacy **ACTIVE**; Metrika **CONSENT-GATED**. **JSON-LD Schema.org graph LIVE** via `shpigovsky-core` module `structured-data.schema-org` (single `<head>` owner). Open Graph still **not emitted** on sampled pages (pre-existing). Open items: `REPORTS/OPEN-ITEMS-FP-0002-PRODUCTION-MAINTENANCE.md`.

**Current maintenance phase:** `FP-0002 PROD YANDEX SCHEMA.ORG STRUCTURED DATA 01` — **PASS_WITH_ATTENTION**. One server-rendered JSON-LD `@graph` per public page from Admin-owned data; `MedicalClinic` org + separate branch locations; page-aware types (`ContactPage`, `CollectionPage`, `Service`, `Person`, `Article`, `AboutPage` on `/o-centre/` hub); conditional `BreadcrumbList`; no Product/Offer/QAPage/fake ratings/universal Physician. Core `0.3.30-yandex-schema-org-01`. Report: `REPORTS/REPORT-FP-0002-PROD-YANDEX-SCHEMA-ORG-01.md`. Evidence: `REPORTS/evidence/prod-yandex-schema-org-01/`. ATTENTION: authenticated Yandex Webmaster validator UI not run in agent session; OG still absent.

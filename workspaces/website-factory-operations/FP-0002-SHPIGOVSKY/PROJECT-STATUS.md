...
**Last updated:** 2026-09-09 (Blog Hub dedicated announcement field 01 — **PASS** → **PRODUCTION / MAINTENANCE — STABLE**)

**Current project state:** **PRODUCTION / MAINTENANCE — STABLE**. Domain `https://shpigovsky.ru/`. Core **`0.3.32-blog-hub-announcement-01`**. Indexing **OPEN — HUMAN-APPROVED** (`blog_public=1`). P18G guard **ACTIVE**; watchdog **ACTIVE**. Physical `/robots.txt` remains Olya-approved SEO policy, separate from global OPEN/CLOSED. Forms **ACTIVE**; SMTP **VERIFIED / ACTIVE**; native first-party anti-spam **ACTIVE**. Privacy **ACTIVE**; Metrika **CONSENT-GATED**. **JSON-LD Schema.org graph LIVE** via `shpigovsky-core` module `structured-data.schema-org` (single `<head>` owner). **Open Graph LIVE** via `shpigovsky-core` module `open-graph.meta` (single `<head>` owner, separate from SEO + JSON-LD). Blog Hub card preview owner: ACF **`article_hub_announcement`** (`Анонс статьи для хаба`) with native `get_the_excerpt()` fallback. Open items: `REPORTS/OPEN-ITEMS-FP-0002-PRODUCTION-MAINTENANCE.md`.

**Current maintenance phase:** `FP-0002 BLOG HUB DEDICATED ANNOUNCEMENT FIELD 01` — **PASS**. Dedicated ACF textarea for `/blog/` article-card announcement; empty field keeps historical excerpt fallback; Lead / SEO / OG / Schema remain independent. Core `0.3.32-blog-hub-announcement-01`. Report: `REPORTS/REPORT-FP-0002-BLOG-HUB-DEDICATED-ANNOUNCEMENT-FIELD-01.md`. Evidence: `REPORTS/evidence/prod-blog-hub-announcement-01/`.

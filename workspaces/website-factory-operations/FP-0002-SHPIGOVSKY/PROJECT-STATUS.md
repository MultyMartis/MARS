...
**Last updated:** 2026-08-24 (Specialists Hub layout polish 01 — **PASS** → **PRODUCTION / MAINTENANCE — STABLE**)

**Current project state:** **PRODUCTION / MAINTENANCE — STABLE**. Domain `https://shpigovsky.ru/`. Core `0.3.27-specialists-hub-admin-ux`. Indexing **OPEN — HUMAN-APPROVED** (`blog_public=1`). P18G guard **ACTIVE**; watchdog **ACTIVE**. Physical `/robots.txt` remains Olya-approved SEO policy, separate from global OPEN/CLOSED. Forms **ACTIVE**; SMTP **VERIFIED / ACTIVE**; native first-party anti-spam **ACTIVE**. Privacy **ACTIVE**; Metrika **CONSENT-GATED**. Open items: `REPORTS/OPEN-ITEMS-FP-0002-PRODUCTION-MAINTENANCE.md`.

**Current maintenance phase:** `FP-0002 PROD-MAINT SPECIALISTS HUB LAYOUT POLISH 01` — **PASS**. Hub-only: remove `internal-page-nav` shell (breadcrumbs kept); trim `.plain-page-content__body` CSS (drop max-width/font-size/line-height; add `margin-bottom: var(--pad-gap)`); omit nested rehab `.container` on Hub via `omit_inner_container`. Home/other surfaces unchanged. Core version unchanged. Report: `REPORTS/REPORT-FP-0002-PROD-MAINT-SPECIALISTS-HUB-LAYOUT-POLISH-01.md`. Evidence: `REPORTS/evidence/prod-maint-specialists-hub-layout-polish-01/`.

...
**Last updated:** 2026-08-18 (PROD-P17-FU02 **PASS** — final pre-cutover tails closed; READY FOR MANUAL NS SWITCH)

**Current WordPress phase:** `FP-0002 PROD-P17-FU02 FINAL PRE-CUTOVER TAIL CLOSURE` — **PASS**. `INTERNAL PRE-CUTOVER READINESS = GO`. `MANUAL NS SWITCH = OPERATOR ACTION REQUIRED`. `mars-runtime/` resolved and removed. Public webroot hygiene PASS. Users CLEAN. 7/7 legacy 301s re-verified. Cutover DB/file plans executable without discovery. Freeze + NS handoff runbooks ready. Core `0.3.9-p17fu02`. Next: freeze → fresh backup → operator NS in REG.RU → P18. Open items: `REPORTS/OPEN-ITEMS-FP-0002-AFTER-P17-FU02.md`. Report: `REPORTS/REPORT-FP-0002-PROD-P17-FU02-FINAL-PRE-CUTOVER-TAIL.md`. Evidence: `REPORTS/evidence/prod-p17-fu02-final-tail/`. WPilot `write_enabled=false`.

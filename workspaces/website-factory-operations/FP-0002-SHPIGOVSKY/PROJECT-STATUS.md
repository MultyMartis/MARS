...
**Last updated:** 2026-08-19 (PROD-P18E-E/F **PASS** — form-goal consent gating + cookie settings reopen + cookie policy integration live)

**Current WordPress phase:** `FP-0002 PROD-P18E-E/F FORM GOAL + POLICY INTEGRATION` — **PASS**. Fresh read-only intake reconfirmed Olya/Admin editorial DB truth immediately before mutation. Production now keeps one canonical browser privacy owner `PrivacyConsent`, one consent-gated Metrika loader, and one consent-gated post-success form-goal path: after backend acceptance the frontend attempts `reachGoal` exactly once only when analytics consent is allowed, while analytics failure remains non-blocking for form success. A permanent footer entry `Настройки cookie` reopens the same consent settings UI without overwriting editor-owned footer/menu DB state; withdrawal blocks future analytics and future form goals; re-grant restores analytics without duplicate initialization. Cookie Policy owner remains live page `#24`; factual runtime wording was updated to disclose `fp02_cookie_consent`, Yandex Metrika, consent-gated analytics, settings reopen/withdrawal, and current `sessionStorage['fp02_utm']` usage with final legal wording still pending review. Live browser QA verified necessary-only submit = success with no Metrika/goal attempt, analytics-allowed submit = success with consent-gated Metrika activity, and blocked-Metrika submit = success without broken UX. Dashboard/admin truth is updated; source-to-production parity is exact for touched runtime files; indexing remains closed. Core target: `0.3.19-p18e-ef`. Report: `REPORTS/REPORT-FP-0002-PROD-P18E-EF-FORM-GOAL-POLICY-INTEGRATION.md`. Evidence: `REPORTS/evidence/prod-p18e-ef-form-goal-policy-integration/`. Dirty main foreign WIP untouched; work performed in a clean worktree.

**Prior WordPress phase:** `FP-0002 PROD-P18E-C/D COOKIE UI + METRIKA GATING` — **PASS**. Fresh read-only intake reconfirmed current Olya/Admin DB truth and proved that live Metrika still leaked through `custom_head_code` before mutation. Production now has one canonical consent owner `PrivacyConsent` inside `shpigovsky-core`, live public notice/settings UI, fail-closed browser record handling for `fp02_cookie_consent`, and one idempotent frontend loader that initializes Yandex Metrika only after explicit analytics consent. Unconditional theme/bootstrap paths and `noscript` bypass are removed; render-time stripping now neutralizes legacy Metrika snippets in custom code fields. Live browser QA verified undecided / necessary-only / tampered / old-version / JS-disabled states remain analytics-free, while accept and custom-on load Metrika. A secondary Yandex tracking residual from contacts-page map embeds was found and neutralized with a privacy-safe external-open fallback. Dashboard truth is updated; source-to-production parity is exact for touched files; indexing remains closed; form-goal consent integration remains pending by design. Policy page owner remains live page `#24`, status `CURRENT / NEEDS LEGAL CONTENT REVIEW`. Core target: `0.3.18-p18e-cd`. Baseline: `REPORTS/BASELINE-FP-0002-PRODUCTION-P18E-CD.md`. Report: `REPORTS/REPORT-FP-0002-PROD-P18E-CD-COOKIE-UI-METRIKA-GATING.md`. Evidence: `REPORTS/evidence/prod-p18e-cd-cookie-ui-metrika-gating/`. Open items: `REPORTS/OPEN-ITEMS-FP-0002-AFTER-P18E-CD.md`. Dirty main foreign WIP untouched; work performed in a clean worktree.

**Prior WordPress phase:** `FP-0002 PROD-P18E-A/B COOKIE CONSENT FOUNDATION` — **PASS**. Fresh read-only intake reconfirmed Olya/Admin DB truth, current tracker/storage reality, and the earlier legal/provider baseline before any mutation. Production now has one canonical consent owner `PrivacyConsent` inside `shpigovsky-core`, explicit machine states (`UNDECIDED`, `NECESSARY_ONLY`, `ANALYTICS_ALLOWED`), consent version `1`, validated browser-record contract (`fp02_cookie_consent`), future-safe cookie attributes, technical integration registry (`Yandex Metrika -> analytics`), truthful Dashboard status, and a discoverable Admin path `Настройки сайта → Cookie и конфиденциальность`. Save/reload persistence is proven; source-to-production parity is exact for touched plugin files; indexing remains closed; public Metrika loading and current form-goal runtime remain intentionally unchanged. Policy page owner is now set to live page `#24`, with status `CURRENT / NEEDS LEGAL CONTENT REVIEW`. Core target: `0.3.17-p18e-ab`. Baseline: `REPORTS/BASELINE-FP-0002-PRODUCTION-P18E-AB.md`. Report: `REPORTS/REPORT-FP-0002-PROD-P18E-AB-COOKIE-CONSENT-FOUNDATION.md`. Evidence: `REPORTS/evidence/prod-p18e-ab-consent-foundation/`. Open items: `REPORTS/OPEN-ITEMS-FP-0002-AFTER-P18E-AB.md`. Dirty main foreign WIP untouched; work performed in a clean worktree.

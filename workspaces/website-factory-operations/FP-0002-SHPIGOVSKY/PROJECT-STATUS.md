...
**Last updated:** 2026-08-19 (PROD-P18C-FU01 **PASS** — Admin menu exposure for Почта и формы)

**Current WordPress phase:** `FP-0002 PROD-P18C-FU01 ADMIN MENU EXPOSURE` — **PASS**. P18C internals existed; **Настройки сайта → Почта и формы** was an orphan submenu under logical slug `fp02-site-settings` while the visible parent is ACF-redirect `fp02-site-settings-general`. FU01 registers after ACF (`admin_menu` 100) via `OptionsPage::visible_menu_slug()`. Olya/Admin can discover, open, and use the page. **Заявки** reachable. SMTP **NOT CONFIGURED**. Mail suppression **ON**. Indexing **CLOSED**. Core `0.3.13-p18c-fu01`. `4/4 SOURCE ↔ PRODUCTION MATCH`. Operator next: enter SMTP + recipients, Save, do not open indexing. Report: `REPORTS/REPORT-FP-0002-PROD-P18C-FU01-ADMIN-MENU.md`. Evidence: `REPORTS/evidence/prod-p18c-fu01-admin-menu/`. Open items: `REPORTS/OPEN-ITEMS-FP-0002-AFTER-P18C-FU01.md`. Git: isolated worktree; dirty main foreign WIP untouched. WPilot `write_enabled=false`.

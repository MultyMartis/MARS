...
**Last updated:** 2026-08-24 (Specialists canonical URL migration 01 — **PASS** → **PRODUCTION / MAINTENANCE — STABLE**)

**Current project state:** **PRODUCTION / MAINTENANCE — STABLE**. Domain `https://shpigovsky.ru/`. Core `0.3.28-specialists-canonical-url`. Indexing **OPEN — HUMAN-APPROVED** (`blog_public=1`). P18G guard **ACTIVE**; watchdog **ACTIVE**. Physical `/robots.txt` remains Olya-approved SEO policy, separate from global OPEN/CLOSED. Forms **ACTIVE**; SMTP **VERIFIED / ACTIVE**; native first-party anti-spam **ACTIVE**. Privacy **ACTIVE**; Metrika **CONSENT-GATED**. Open items: `REPORTS/OPEN-ITEMS-FP-0002-PRODUCTION-MAINTENANCE.md`.

**Current maintenance phase:** `FP-0002 PROD-MAINT SPECIALISTS CANONICAL URL MIGRATION 01` — **PASS**. Operator-approved spelling `specialisty` is now canonical. Page `#1030` slug `specialisty`; CPT rewrite base `specialisty` (`has_archive=false`); hub `https://shpigovsky.ru/specialisty/`; singles `/specialisty/{slug}/`. Deprecated `/specyalisty/` → **301** (htaccess + `SpecialistLegacyRedirect`). Sitemap/canonicals/internal links use new URLs only. Core `0.3.28-specialists-canonical-url`. Report: `REPORTS/REPORT-FP-0002-PROD-MAINT-SPECIALISTS-CANONICAL-URL-MIGRATION-01.md`. Evidence: `REPORTS/evidence/prod-maint-specialists-canonical-url-migration-01/`.

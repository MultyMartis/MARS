...
**Last updated:** 2026-08-19 (PROD-P18C **PASS** — SMTP / forms Admin foundation)

**Current WordPress phase:** `FP-0002 PROD-P18C SMTP / FORMS FOUNDATION` — **PASS**. One Admin owner: Настройки сайта → Почта и формы. Sender default `noreply@shpigovsky.ru`. SMTP secret write-only (never rendered/logged/Git). Internal table `fp02_form_leads`; persist **before** mail. Menu **Заявки**. Metrika goal Admin-configurable; fires only after backend-confirmed submit. Mail suppression **ON**. SMTP **NOT CONFIGURED / NOT VERIFIED**. Indexing **CLOSED**. Core `0.3.12-p18c`. `14/14 SOURCE ↔ PRODUCTION MATCH`. Operator next: enter SMTP + recipients in Admin, Save, do not open indexing. Report: `REPORTS/REPORT-FP-0002-PROD-P18C-SMTP-FORMS-FOUNDATION.md`. Evidence: `REPORTS/evidence/prod-p18c-smtp-forms-foundation/`. Open items: `REPORTS/OPEN-ITEMS-FP-0002-AFTER-P18C.md`. Git: isolated worktree (see evidence `GIT-CHECKPOINT.json`); dirty main foreign WIP untouched. WPilot `write_enabled=false`. Public apex may still be legacy Craftum — remaining launch work, not a P18C SMTP-settings defect.

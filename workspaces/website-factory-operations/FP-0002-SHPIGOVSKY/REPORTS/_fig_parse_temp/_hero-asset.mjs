...
function textOf(n){if(!n)return"";if(n.characters)return n.characters.trim();for(const o of n.symbolData?.symbolOverrides||[]){if(o.characters)return o.characters.trim();if(o.textData?.characters)return o.textData.characters.trim();}return "";}
function buttonText(id){const n=byGuid.get(id);if(!n)return"";for(const o of n.symbolData?.symbolOverrides||[])if(o.characters)return o.characters;if(o.textData?.characters)return o.textData.characters;return textOf(n);}
console.log('hero btn', buttonText('1:3540'));
console.log('hero images', collectImages('1:3528'));
console.log('hub hero images from services v2 - search image 13030403');

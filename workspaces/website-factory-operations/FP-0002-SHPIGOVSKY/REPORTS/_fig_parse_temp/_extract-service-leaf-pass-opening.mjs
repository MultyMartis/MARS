...
function collectImages(id, acc = []) {
  ...
  for (const f of n.fillPaints || []) {
    if (f.type === "IMAGE" || f.image)
      acc.push({ id, name: n.name, hash: f.image?.hash });
  }
...
function sectionKids(frameId) {
  const kids = (children.get(frameId) || []).filter(
...
const desktop = findFrameByName("Услуга конечная");
const mobile = findFrameByName("Услуга конечная - моб");
const rejected = ["Услуга подраздел", "Услуга подраздел - моб"].map(findFrameByName);

const out = {
  figPath,
  figSha256Note: "BAE5D91C74B5A22AFC610F7C7845B9BADC6B87EC8DA85C5705ECF4EEC4DE3041",
  desktop,
  mobile,
...
const outPath = join(__dirname, "_service-leaf-fig-extract.json");
writeFileSync(outPath, JSON.stringify(out, null, 2), "utf8");
console.log("desktop", desktop?.id, desktop?.w, desktop?.h, "sections", out.desktopSections.length);
console.log("mobile", mobile?.id, mobile?.w, mobile?.h, "sections", out.mobileSections.length);

# -*- coding: utf-8 -*-
"""P18A git checkpoint via isolated worktree. Dirty main untouched. Two commits + push."""
...
EV = SRC / "REPORTS/evidence/prod-p18a-live-domain-legal-state"
...
FP_FILES = [
    "WORDPRESS/theme/shpigovsky/inc/legal-helpers.php",
    "WORDPRESS/theme/shpigovsky/functions.php",
    "WORDPRESS/theme/shpigovsky/template-parts/legal/document-page.php",
    ...
    "WORDPRESS/plugins/shpigovsky-core/src/Admin/EditorRestrictions.php",
    "WORDPRESS/acf-json/group_fp02_page_legal.json",
    "PROJECT-STATUS.md",
    "REPORTS/REPORT-FP-0002-PROD-P18A-LIVE-DOMAIN-LEGAL-STATE.md",
    "REPORTS/OPEN-ITEMS-FP-0002-AFTER-P18A.md",
    "REPORTS/LEGAL-DEMO-PLACEHOLDER-INVENTORY.md",
...
FW_FILES = [
    "standards/FORGE-WORDPRESS-ACF-FIELD-MODELING-STANDARD-v1.md",
    "standards/FORGE-WORDPRESS-EDITOR-UX-STANDARD-v1.md",
    "standards/FORGE-WORDPRESS-ANTI-PATTERN-REGISTRY-v1.md",
    "standards/FORGE-WORDPRESS-CMS-ANTI-PATTERNS-v1.md",
    "runbooks/FORGE-WORDPRESS-SOURCE-RUNTIME-AUTHORITY-STANDARD-v1.md",
...
def main() -> int:
    ...
    for src in EV.iterdir():
        if not src.is_file():
            continue
        if src.name.startswith("GIT-CHECKPOINT"):
            continue
        rel = f"REPORTS/evidence/prod-p18a-live-domain-legal-state/{src.name}"
        copy_file(src, dest_fp / Path(*rel.split("/")))
        copied.append(rel)
    ...
    for extra in ("GIT-SECRET-SCAN.json", "GIT-STAGED-PATHS-FP.txt"):
        copy_file(EV / extra, dest_fp / "REPORTS/evidence/prod-p18a-live-domain-legal-state" / extra)
        run(["git", "add", "--", f"{REL_FP}/REPORTS/evidence/prod-p18a-live-domain-legal-state/{extra}"], cwd=str(REPO))

    run(["git", "commit", "-m", "FP-0002: intake live domain and fix legal demo state"], cwd=str(REPO))
    ...
    if hits2:
        ...
        return 3
    if staged2:
        run(["git", "commit", "-m", "WP Forge: boolean false-is-a-value and manual cutover intake"], cwd=str(REPO))
    ...
    evidence = {
        ...
        "remote": remote,
        "messages": [
            "FP-0002: intake live domain and fix legal demo state",
            "WP Forge: boolean false-is-a-value and manual cutover intake",
        ],
        "secret_scan": "PASS",
    ...
    print("REMOTE", remote)
    return 0 if sha2 == remote else 2

/**
 * V9-06E57-FIX02 lifebuoy start/reveal/easing/rotation validation + screenshots
 */
...
(async () => {
  ...
  // Long page detailed scroll states @ 1440 (includes early response samples)
  {
  ...
  // Early-response delta proof: compare top vs p02 scale/x/rotate
  {
    const top = matrix.find((r) => r.route === 'home' && r.state === 'top' && r.viewport === '1440x900');
    const p02 = matrix.find((r) => r.route === 'home' && r.state === 'p02' && r.viewport === '1440x900');
    const early = {
      topScale: top && top.vars && top.vars.scale,
      p02Scale: p02 && p02.vars && p02.vars.scale,
      topX: top && top.vars && top.vars.x,
      p02X: p02 && p02.vars && p02.vars.x,
      topRotate: top && top.vars && top.vars.rotate,
      p02Rotate: p02 && p02.vars && p02.vars.rotate,
      scaleDelta:
        top && p02 && top.vars && p02.vars
          ? Number(p02.vars.scale) - Number(top.vars.scale)
          : null,
      xDeltaPct:
        top && p02 && top.vars && p02.vars
          ? parseFloat(p02.vars.x) - parseFloat(top.vars.x)
          : null,
      rotateDelta:
        top && p02 && top.vars && p02.vars
          ? parseFloat(p02.vars.rotate) - parseFloat(top.vars.rotate)
          : null,
    };
    fs.writeFileSync(path.join(outDir, 'early-response.json'), JSON.stringify(early, null, 2));
  }
  ...
  const csvLines = [
    'route,viewport,state,present,count,mode,progress,attrProgress,x,y,scale,rotate,visibleWidthPct,scrollable,opacity,pointerEvents,hasOverflowX,docWidth,winWidth',
  ];
  ...
  fs.writeFileSync(path.join(outDir, 'motion-metrics.csv'), csvLines.join('\n'));

  const homeStates = matrix.filter(
    (r) =>
      r.route === 'home' &&
      r.viewport === '1440x900' &&
      ['top', 'p02', 'p10', 'p25', 'p50', 'p75', 'bottom'].includes(r.state)
  );
  fs.writeFileSync(
  ...
  // Reverse parity check
  const fwd50 = matrix.find((r) => r.route === 'home' && r.state === 'p50' && r.viewport === '1440x900');
  const rev50 = matrix.find(
    (r) => r.route === 'home' && r.state === 'reverse-p50' && r.viewport === '1440x900'
  );
  const reverseParity = {
    forward: fwd50 && fwd50.vars,
    reverse: rev50 && rev50.vars,
    match:
      fwd50 &&
      rev50 &&
      fwd50.vars &&
      rev50.vars &&
      fwd50.vars.x === rev50.vars.x &&
      fwd50.vars.y === rev50.vars.y &&
      fwd50.vars.scale === rev50.vars.scale &&
      fwd50.vars.rotate === rev50.vars.rotate,
  };
  fs.writeFileSync(path.join(outDir, 'reverse-parity.json'), JSON.stringify(reverseParity, null, 2));

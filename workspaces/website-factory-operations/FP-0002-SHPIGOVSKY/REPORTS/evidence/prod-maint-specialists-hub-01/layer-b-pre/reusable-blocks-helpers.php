...
/**
 * Normalize a specialist post into render-ready card data.
 *
 * @param WP_Post $page Specialist post.
...
function shpigovsky_normalize_specialist_page_card( $page ) {
	...
	$role = '';
	if ( function_exists( 'get_field' ) ) {
		$acf_role = get_field( 'specialist_role', $page->ID );
		if ( is_string( $acf_role ) && '' !== trim( $acf_role ) ) {
			$role = trim( $acf_role );
		}
	}
	if ( '' === $role ) {
...
function shpigovsky_get_specialists_cards() {
	...
	// Pre-migration / rollback safety: if CPT empty, fall back to hub child pages once.
	if ( empty( $posts ) ) {
		$parent_id = shpigovsky_get_specialists_parent_page_id();
		if ( $parent_id > 0 ) {
			$posts = get_posts(

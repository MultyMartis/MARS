# -*- coding: utf-8 -*-
"""P18C git checkpoint via isolated worktree. Dirty main untouched. Two commits + push."""
...
EV = SRC / "REPORTS/evidence/prod-p18c-smtp-forms-foundation"
...
FP_FILES = [
    ...
    "WORDPRESS/plugins/shpigovsky-core/src/Mail/SmtpTransport.php",
    "WORDPRESS/plugins/shpigovsky-core/src/Leads/LeadRegistry.php",
    "WORDPRESS/plugins/shpigovsky-core/src/Privacy/LeadPersonalData.php",
    "WORDPRESS/plugins/shpigovsky-core/src/Admin/MailFormsSettings.php",
    "WORDPRESS/plugins/shpigovsky-core/src/Admin/LeadsAdmin.php",
    ...
    "WORDPRESS/plugins/shpigovsky-core/src/Settings/SiteSettings.php",
    ...
    "WORDPRESS/mu-plugins/fp02-pre-cutover-mail-suppression.php",
    "WORDPRESS/theme/shpigovsky/assets/js/v9-shell.js",
    "PROJECT-STATUS.md",
    "REPORTS/REPORT-FP-0002-PROD-P18C-SMTP-FORMS-FOUNDATION.md",
    "REPORTS/OPEN-ITEMS-FP-0002-AFTER-P18C.md",
...
FW_FILES = [
    ...
    "standards/FORGE-WORDPRESS-GLOBAL-SETTINGS-OWNERSHIP-STANDARD-v1.md",
    "standards/FORGE-WORDPRESS-ANTI-PATTERN-REGISTRY-v1.md",
    "standards/FORGE-WORDPRESS-SITE-SETTINGS-STANDARD-v1.md",
    "standards/FORGE-WORDPRESS-PRODUCTION-WEBSITE-BLUEPRINT-v1.md",
    "standards/FORGE-WORDPRESS-CODING-AND-SECURITY-STANDARD-v1.md",
    "registries/FORGE-WORDPRESS-MODULE-CATALOG-v1.md",
    "templates/FORGE-WORDPRESS-NEW-SITE-STARTER-CHECKLIST-v1.md",
    "runbooks/FORGE-WORDPRESS-PRE-CUTOVER-AND-LAUNCH-SOP-v1.md",
    "knowledge/FORGE-WORDPRESS-ADR-PACK-v1.md",
...
def run(cmd, cwd=None, check=True):
    ...
    p = subprocess.run(cmd, cwd=cwd or str(DIRTY), capture_output=True, text=True, encoding="utf-8", errors="replace")
...
def secret_scan(repo: Path, staged: list[str]) -> list[dict]:
    ...
    for rel in staged:
        ...
        for m in REAL_SECRET.finditer(text):
            val = m.group(2)
            if any(x in val.lower() for x in ("operator", "example", "changeme", "xxxx", "***", "redacted", "placeholder", "new-password", "smtp_password")):
                continue
            if "пароль" in val.lower() or "укажите" in val.lower():
                continue
...
def main() -> int:
    ...
    for src in EV.iterdir():
        ...
        if src.name in SKIP_EV_NAMES:
            continue
        rel = f"REPORTS/evidence/prod-p18c-smtp-forms-foundation/{src.name}"
        copy_file(src, dest_fp / Path(*rel.split("/")))
        copied.append(rel)

    paths = [f"{REL_FP}/{rel}" for rel in copied]
    run(["git", "add", "--"] + paths, cwd=str(REPO))
    staged = [x.replace("\\", "/") for x in run(["git", "diff", "--cached", "--name-only"], cwd=str(REPO)).stdout.splitlines() if x]
    ...
    for extra in ("GIT-SECRET-SCAN.json", "GIT-STAGED-PATHS-FP.txt"):
        copy_file(EV / extra, dest_fp / "REPORTS/evidence/prod-p18c-smtp-forms-foundation" / extra)
        run(["git", "add", "--", f"{REL_FP}/REPORTS/evidence/prod-p18c-smtp-forms-foundation/{extra}"], cwd=str(REPO))
    ...
    run(
        [
            ...
            "commit",
            "-m",
            "FP-0002: add SMTP and forms operations foundation",
        ],
        cwd=str(REPO),
    )
    ...
    staged2 = [x.replace("\\", "/") for x in run(["git", "diff", "--cached", "--name-only"], cwd=str(REPO)).stdout.splitlines() if x]
    ...
    if staged2:
        run(
            [
                ...
                "commit",
                "-m",
                "WP Forge: standardize form lead persistence and SMTP admin configuration",
            ],
            cwd=str(REPO),
        )
    ...
    evidence = {
        ...
        "foreign_wip_untouched": True,
    }
    (EV / "GIT-CHECKPOINT.json").write_text(json.dumps(evidence, indent=2) + "\n", encoding="utf-8")
    copy_file(EV / "GIT-CHECKPOINT.json", dest_fp / "REPORTS/evidence/prod-p18c-smtp-forms-foundation/GIT-CHECKPOINT.json")
    run(["git", "add", "--", f"{REL_FP}/REPORTS/evidence/prod-p18c-smtp-forms-foundation/GIT-CHECKPOINT.json"], cwd=str(REPO))
    staged3 = [x.replace("\\", "/") for x in run(["git", "diff", "--cached", "--name-only"], cwd=str(REPO)).stdout.splitlines() if x]
    if staged3:
        run(["git", "commit", "-m", "FP-0002: record P18C git checkpoint"], cwd=str(REPO))
        run(["git", "push", "origin", "HEAD:mars/canonical-post-recovery"], cwd=str(REPO))
        sha3 = run(["git", "rev-parse", "HEAD"], cwd=str(REPO)).stdout.strip()
        remote = run(["git", "rev-parse", "origin/mars/canonical-post-recovery"], cwd=str(REPO)).stdout.strip()
        evidence["commits"].append(sha3)
        evidence["remote"] = remote
        evidence["messages"].append("FP-0002: record P18C git checkpoint")
        (EV / "GIT-CHECKPOINT.json").write_text(json.dumps(evidence, indent=2) + "\n", encoding="utf-8")
        print("COMMIT3", sha3)

    print("REMOTE", remote)
    return 0 if remote == run(["git", "rev-parse", "HEAD"], cwd=str(REPO)).stdout.strip() else 2

# -*- coding: utf-8 -*-
"""P18B git checkpoint via isolated worktree. Dirty main untouched. Two commits + push."""
...
EV = SRC / "REPORTS/evidence/prod-p18b-dashboard-indexing"
...
FP_FILES = [
    ...
    "PROJECT-STATUS.md",
    "REPORTS/REPORT-FP-0002-PROD-P18B-DASHBOARD-INDEXING.md",
    "REPORTS/OPEN-ITEMS-FP-0002-AFTER-P18B.md",
...
FW_FILES = [
    ...
    "standards/FORGE-WORDPRESS-SEARCH-INDEXING-CONTROL-STANDARD-v1.md",
    "standards/FORGE-WORDPRESS-ADMIN-UX-STANDARD-v1.md",
    "standards/FORGE-WORDPRESS-FORMS-AND-SMTP-STANDARD-v1.md",
    "standards/FORGE-WORDPRESS-SEO-AND-SITEMAP-STANDARD-v1.md",
    ...
    "knowledge/README.md",
    "knowledge/KNOWLEDGE-RETRIEVAL-VALIDATION.md",
    ...
    "registries/FORGE-WORDPRESS-CONTRACTS-AND-STANDARDS-REGISTER-v1.md",
]
...
def secret_scan(repo: Path, staged: list[str]) -> list[dict]:
    ...
    for rel in staged:
        ...
        for m in REAL_SECRET.finditer(text):
            val = m.group(2)
            if any(x in val.lower() for x in ("operator", "example", "changeme", "xxxx", "***", "redacted", "placeholder")):
                continue
...
def main() -> int:
    ...
    for src in EV.iterdir():
        ...
        if src.name in SKIP_EV_NAMES:
            continue
        rel = f"REPORTS/evidence/prod-p18b-dashboard-indexing/{src.name}"
        copy_file(src, dest_fp / Path(*rel.split("/")))
        copied.append(rel)
    ...
    for extra in ("GIT-SECRET-SCAN.json", "GIT-STAGED-PATHS-FP.txt"):
        copy_file(EV / extra, dest_fp / "REPORTS/evidence/prod-p18b-dashboard-indexing" / extra)
        run(["git", "add", "--", f"{REL_FP}/REPORTS/evidence/prod-p18b-dashboard-indexing/{extra}"], cwd=str(REPO))
    ...
    run(
        [
            ...
            "commit",
            "-m",
            "FP-0002: sync production dashboard and add indexing control",
        ],
        cwd=str(REPO),
    )
    ...
    if staged2:
        run(
            [
                ...
                "commit",
                "-m",
                "WP Forge: standardize live status panels and explicit indexing approval",
            ],
            cwd=str(REPO),
        )
    ...
    copy_file(EV / "GIT-CHECKPOINT.json", dest_fp / "REPORTS/evidence/prod-p18b-dashboard-indexing/GIT-CHECKPOINT.json")
    run(["git", "add", "--", f"{REL_FP}/REPORTS/evidence/prod-p18b-dashboard-indexing/GIT-CHECKPOINT.json"], cwd=str(REPO))
    staged3 = [x.replace("\\", "/") for x in run(["git", "diff", "--cached", "--name-only"], cwd=str(REPO)).stdout.splitlines() if x]
    if staged3:
        run(["git", "commit", "-m", "FP-0002: record P18B git checkpoint"], cwd=str(REPO))
        run(["git", "push", "origin", "HEAD:mars/canonical-post-recovery"], cwd=str(REPO))
        sha3 = run(["git", "rev-parse", "HEAD"], cwd=str(REPO)).stdout.strip()
        remote = run(["git", "rev-parse", "origin/mars/canonical-post-recovery"], cwd=str(REPO)).stdout.strip()
        evidence["commits"].append(sha3)
        evidence["remote"] = remote
        evidence["messages"].append("FP-0002: record P18B git checkpoint")
        (EV / "GIT-CHECKPOINT.json").write_text(json.dumps(evidence, indent=2) + "\n", encoding="utf-8")
        print("COMMIT3", sha3)

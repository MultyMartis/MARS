...
def build_master() -> tuple[list, list]:
    nodes = [
        ...
        text_node(
            "n-layer-archive",
            "## Archive Layer\n\n`archive/`, `web-gpt-sources/`, historical imports.\n\nSee `archive.canvas`.",
            860,
            500,
            280,
            100,
...
def build_programs() -> tuple[list, list]:
    ...
    programs = [
        ...
        ("n-prog-triumph", "Triumph\n\nClient program pack + Factory execution case\n`projects/triumph-manipulator-landing/`", 10),
        (
            "n-prog-gitguard",
            "GitGuard\n\n**REGISTERED** · Repository Survivability Layer\nEntry: `mars-survivability/registries/gitguard-system-entry-v1.md`\n**Not** `projects/gitguard/` · **no** `project_id`",
            11,
        ),
...
def build_website_factory() -> tuple[list, list]:
    nodes = [
        ...
        text_node(
            "n-wf-triumph",
            "## Triumph (execution case #1)\n\nClient delivery + reference simulation\nCanonical ws: `workspaces/triumph-manipulator-landing-v6/`",
            0,
            640,
            280,
            110,
...
def build_infrastructure() -> tuple[list, list]:
    nodes = [
        ...
        text_node(
            "n-inf-incoming-active",
            "## Active Incoming\n\n`incoming/` — untrusted staging in Active Brain\nHybrid model · operator triage",
            40,
            240,
            280,
            100,
        ...
        text_node(
            "n-inf-incoming-bulk",
            "## Historical Bulk\n\nRetired raw packs in Storage Layer\n**After** triage sign-off — not default in `incoming/`",
            480,
            240,
            280,
            100,
        ...
        text_node("n-inf-storage-ws", "## workspaces (bulk)\n\nSite archives, temp extracts", 480, 660),
        text_node("n-inf-storage-arch", "## archive (bulk)\n\nCold paths (operator-defined)", 480, 800),
        text_node(
            "n-inf-external",
        ...
        group_node(
            "n-inf-obs-flow-group",
            "OBSERVED INFORMATION FLOW — NOT runtime · NOT architecture",
            40,
            1220,
            760,
            120,
    ...
    edges = [
        edge("e-incoming-brain", "n-inf-incoming-active", "n-inf-brain", "staging", "top", "bottom"),
        edge("e-bulk-storage", "n-inf-incoming-bulk", "n-inf-storage-root", "retirement", "top", "bottom"),
...
def build_archive() -> tuple[list, list]:
    ...
    placements = [
        ...
        ("n-ent-lifecycle-log", "Lifecycle Log — **KEEP**\n`logs/lifecycle-log.md` · Key Event History", "n-cat-operational", 20, 150),

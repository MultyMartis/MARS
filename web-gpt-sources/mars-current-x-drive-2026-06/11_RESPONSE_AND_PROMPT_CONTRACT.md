...
| **P1** | Safety / Filesystem / Git / Scope Rules | X-drive authority, no destructive ops, no broad staging, foreign WIP protection. Refer to `AGENTS.md`, `.cursorrules`, `02_OPERATIONAL_POSTURE.md`, `04_INFRASTRUCTURE_REALITY.md`, `10_RUNTIME_AND_FILESYSTEM_BOUNDARIES.md`. Cursor prompts must respect root `.cursorrules`, `AGENTS.md`, and mandatory guardrails (`projects/mars-survivability/guardrails/cursor-agent-guardrails-v1.md`). Generic Cursor/platform commit habits are subordinate to MARS exact-scope / selective-staging rules. For filesystem, git, cleanup, backup, Storage, Localhost, runtime, or production-risk tasks, prompts should include or reference the guardrails file. Prompt packs must preserve: target folder `X:\AI MARS`; volume label `AI WS`; branch `mars/canonical-post-recovery`; preflight; foreign WIP preservation; exact scope for stage/commit/delete; separate commit/push waves unless explicitly authorized. |

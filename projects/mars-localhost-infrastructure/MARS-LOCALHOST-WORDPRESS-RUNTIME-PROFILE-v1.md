...
Define the **WordPress runtime profile** for MARS Localhost Infrastructure (MLI): how WordPress sites are classified, laid out on `X:\MARS-Localhost`, registered in the brain (`X:\AI MARS`), configured, validated, and consumed by Forge WordPress and related programs.
...
|------|------|-------------------|
| **`X:\AI MARS` (brain)** | Source of truth for manifests, standards, registries, validation reports | Pointers only — paths, URLs, database **names**, secrets **location** |
| **`X:\MARS-Localhost` (runtime)** | Executing files — core, uploads, caches, Laragon stack | WordPress tree, DB data, TLS material, dumps, logs |
| **`X:\AI MARS\local\mli\{slug}\`** | Runtime secrets outside Git | `runtime.env` — credentials; **never** in docs or manifests |

**Rule WP-RT-01:** Governance and manifest SoT live in brain; WordPress core and media live on D:.  
**Rule WP-RT-02:** `X:\MARS-Localhost` is **not** a Git repository for MARS.  
...
```text
X:\MARS-Localhost\laragon\www\fws-0001  →  sites\wordpress\synthetic\fws-0001
```
...
cd /d X:\MARS-Localhost\sites\wordpress\synthetic\fws-0001
wp --info
wp core version
...
Logs stay on D: under site or `X:\MARS-Localhost\logs\applications\`. Do not commit debug logs to brain.
...
Consumer installs theme/plugin from `X:\AI MARS\workspaces\` after MLI handoff.
...
```text
X:\MARS-Localhost\databases\dumps\
X:\MARS-Localhost\backups\wordpress\
```
...
Fixture path: `X:\MARS-Localhost\tools\playwright-smoke\`.
...
```bat
phpcs --standard=X:\MARS-Localhost\tools\phpcs\rulesets\mars-wordpress.xml {path}
```
...
| **Owns** | Theme/plugin source, methodology, FWS/FP passports in `X:\AI MARS\workspaces\` |
| **Does not own** | `X:\MARS-Localhost` root, Laragon, shared toolchain |
...
| **Boundary** | WPilot does **not** provision or govern `X:\MARS-Localhost` |

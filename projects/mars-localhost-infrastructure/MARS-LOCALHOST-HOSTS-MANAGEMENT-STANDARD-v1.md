...
Domains are driven by `X:\MARS-Localhost\runtime\registries\mli-hosts-domains.txt` (runtime) with built-in defaults in `add-mli-host.ps1`.
...
| Script | Purpose |
|--------|---------|
| `X:\MARS-Localhost\tools\hosts\add-mli-host.cmd` | Add or refresh managed block |
| `X:\MARS-Localhost\tools\hosts\remove-mli-host.cmd` | Remove managed block only |
| `X:\MARS-Localhost\tools\hosts\add-mli-host.ps1` | PowerShell implementation |
| `X:\MARS-Localhost\tools\hosts\remove-mli-host.ps1` | PowerShell implementation |
...
2. **Backup** hosts to `X:\MARS-Localhost\backups\runtime\hosts\` before mutation.
...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File "X:\MARS-Localhost\tools\hosts\add-mli-host.ps1"
```

#!/usr/bin/env python3
"""Create INACTIVE Operational.v3.rollback n8n workflow + mars_core registry row.

Does NOT activate any workflow. Does NOT touch Operational.dev / Admin.dev.
"""
...
WF_PATH = ROOT / "workflows" / "operational-v3-rollback" / "Operational.v3.rollback.n8n.json"
...
CAND_WF_ID = "NH4uV145Amrgnmkm"
WF_NAME = "i-SEO Sales Manager - Operational.v3.rollback"
EXPORT_HASH = "531ad7eb8f7aeb15040f82ad45023d4b91ccf9989c2894b14c4b3569b70eecd4"
...
def main() -> int:
    ...
    file_hash = hashlib.sha256(WF_PATH.read_bytes()).hexdigest()
    if file_hash != EXPORT_HASH:
        print("HASH_MISMATCH", file_hash, EXPORT_HASH)
        return 1

    # Find or create inactive rollback workflow
    ...
    # Update local export with assigned id (still inactive)
    export["id"] = wf_id
    export["active"] = False
    export["name"] = WF_NAME
...
SELECT a.id, 'operational_intake', '{wf_id}', 'Operational.v3.rollback',
...
"""
    # Prefer upsert by unique if exists — use delete+insert for same release_version
    reg_sql = f"""
...
"""
    remote = f"/opt/mars/tmp/v3_rollback_register_{ts}.sql"
    local = EVID / f"register_rollback_{ts}.sql"
    local.write_text(reg_sql, encoding="utf-8")
    # upload via sftp to /tmp then sudo mv
    c = paramiko.SSHClient()
    c.set_missing_host_key_policy(paramiko.AutoAddPolicy())
    pkey = paramiko.Ed25519Key.from_private_key_file(str(PRIV))
    c.connect(HOST, 22, "marsops", pkey=pkey, timeout=60, allow_agent=False, look_for_keys=False)
    sftp = c.open_sftp()
    tmp = f"/tmp/v3_rollback_register_{ts}.sql"

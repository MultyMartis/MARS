...
// - ERROR / 429 MUST NOT mark the business date as sent
...
const evaluationWrites = [
  { key: 'pending_reminder_last_evaluation_at', value: nowIso, description: 'Phase 3H.8.2 last reminder evaluation' },
  { key: 'pending_reminder_last_decision', value: decision, description: 'Phase 3H.8.2 reminder decision' },
  { key: 'pending_reminder_last_check_at', value: nowIso, description: 'Phase 3H.8.2 last reminder check' },
];
if (pendingSnap !== '') {
  evaluationWrites.push({
    key: 'pending_reminder_last_pending_count',
    value: pendingSnap,
    description: 'Phase 3H.8.2 pending count at last evaluation',
  });
}

if (decision === 'ERROR') {
  evaluationWrites.push(
    { key: 'pending_reminder_last_error_safe', value: String(j.last_error_safe || 'ошибка чтения таблицы'), description: 'Phase 3H.8.2 safe error' },
    { key: 'pending_reminder_last_error_class', value: String(j.last_error_class || j.reminder_decision_code || 'ERROR'), description: 'Phase 3H.8.2 error class' },
    { key: 'pending_reminder_last_error_stage', value: String(j.last_error_stage || ''), description: 'Phase 3H.8.2 error stage' },
    { key: 'pending_reminder_last_error_at', value: nowIso, description: 'Phase 3H.8.2 error time' },
    { key: 'pending_reminder_last_retry_attempts', value: String(j.sheets_retry_attempts || j.sheets_fail_count || 0), description: 'Phase 3H.8.2 retry attempts' },
  );
} else {
  evaluationWrites.push(
    { key: 'pending_reminder_last_error_safe', value: '', description: 'Phase 3H.8.2 clear error' },
    { key: 'pending_reminder_last_error_class', value: '', description: 'Phase 3H.8.2 clear error class' },
    { key: 'pending_reminder_last_error_stage', value: '', description: 'Phase 3H.8.2 clear error stage' },
  );
}

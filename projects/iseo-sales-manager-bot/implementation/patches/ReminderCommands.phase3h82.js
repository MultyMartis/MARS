...
function statusText(moderatorShort) {
  ...
  if (rawDecision === 'ERROR' || rawDecision === 'ERROR_SHEETS_429_ACCESS' || /^ERROR/.test(rawDecision)) lastDecision = 'Ошибка';
  ...
  const lines = [
    ...
    'Заявок на последней оценке: ' + lastPending,
  ...
  if (rawDecision === 'ERROR' || rawDecision === 'ERROR_SHEETS_429_ACCESS' || /^ERROR/.test(String(map.pending_reminder_last_decision || ''))) {

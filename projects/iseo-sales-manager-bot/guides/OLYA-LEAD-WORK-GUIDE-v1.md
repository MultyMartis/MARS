...
**Версия:** 1.5 · 2026-08-04 (Phase 3D.6 — личный статус и уведомления о правах)
...
- **`/my_status`** — показывает только ваши текущие права. Для активного модератора это подтверждение доступа к карточкам и кнопкам; административные настройки не появляются.
...
- `/my_status` покажет, что рабочие права отозваны; для восстановления обратитесь к администратору.

## 4.1 Уведомление о выдаче прав
- После выдачи вы получите сообщение «Вам выданы права модератора Sales Manager.» с предложением открыть `/start` или `/help`.
- Если такого сообщения нет, не считайте права невыданными: администратор проверит доставку и ваш статус в реестре.

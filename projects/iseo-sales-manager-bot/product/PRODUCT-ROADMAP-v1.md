...
5. **First Reply Engine v2 + manager card polish (3E.2)** — context-aware deterministic drafts (`sm-reply-v2.0`), known-information guard, card `sm-msg-v2.4`; AI OFF; operator copy acceptance required before final COMPLETE.
6. **Pending-lead reminders** — **next product phase after First Reply v2 acceptance**: `/pending_leads`, `/pending_count`, configurable daily reminder (default 10:00), timezone, active recipients only, pending lifecycle only, deduplicated reminder windows. **Not implemented in 3E.2.**
...
Phase 3D.8.x completed buttons/attribution/labels. Phase 3E.1 closed. Phase 3E.2 ships First Reply Engine v2. Reminders are intentionally deferred until after operator copy acceptance of 3E.2.

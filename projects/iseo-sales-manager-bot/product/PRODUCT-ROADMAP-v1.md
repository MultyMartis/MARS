...
5. **First Reply Engine v2 + Human Reply Style v1 (3E.2 / 3E.2.1)** — context-aware drafts (`sm-reply-v2.1` / `sm-human-v1.0`), silent known-info guard, quality linter, card `sm-msg-v2.4`, delivery fail-closed; AI OFF; **operator copy acceptance pending** before final COMPLETE.
6. **Pending-lead reminders** — **next product phase after First Reply human-copy acceptance**: `/pending_leads`, `/pending_count`, configurable daily reminder (default 10:00), timezone, active recipients only, pending lifecycle only, deduplicated reminder windows. **Not implemented in 3E.2.1.**
...
Phase 3D.8.x completed buttons/attribution/labels. Phase 3E.1 closed. Phase 3E.2.1 hardens delivery and humanizes replies; final COMPLETE waits on operator visual acceptance.

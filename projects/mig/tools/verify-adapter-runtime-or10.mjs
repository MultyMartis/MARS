#!/usr/bin/env node
/**
 * OR-10 verification: groundtruth_run request → adapter → runMigSession → manifest v0.2.
 * Usage: node projects/mig/tools/verify-adapter-runtime-or10.mjs [fixture-path]
 */
...
const { normalizeFromFile } = require(path.join(migRoot, "lib", "task-file-adapter", "normalize-request.js"));
const { validateCanonicalRequest } = require(path.join(
  migRoot,
  "lib",
  "task-file-adapter",
  "validate-canonical.js"
));

const fixturePath =
  process.argv[2] ||
  path.join(migRoot, "test", "fixtures", "task-file-adapter", "research-request-adapter-or10-v0.1.json");
...
async function main() {
  ...
  const sessionId = "mig-20260604-dead01";

  const request = { ...raw };
  delete request.description;
  delete request.expectations;

  const transportRef = `incoming/mig/requests/request-${requestId}.json`;
  const normalized = normalizeFromFile(request, transportRef);
  validateCanonicalRequest(normalized);
  assert(normalized.request_type === "groundtruth_run", "normalized request_type must be groundtruth_run");

  const tmpInbox = fs.mkdtempSync(path.join(migRoot, "test", ".verify-adapter-or10-"));
  ...
  assert(outcome.request_type === "groundtruth_run", "outcome request_type is groundtruth_run");
  ...
  assert(
    manifest.request_type === (expectations.request_type || "groundtruth_run"),
    `manifest request_type ${manifest.request_type}`
  );
  assert(
    manifest.runtime_profile === (expectations.request_type || "groundtruth_run"),
    `manifest runtime_profile ${manifest.runtime_profile}`
  );
  ...
  console.log(
    JSON.stringify(
      {
        status: "ok",
        verification: "adapter-runtime-or10",
        fixture: fixturePath,
        ...
        manifest_summary: {
          ...
          request_type: manifest.request_type,
        ...
        note: "Temp inbox retained under test/.verify-adapter-or10-* for inspection",
      },
      null,
      2
    )

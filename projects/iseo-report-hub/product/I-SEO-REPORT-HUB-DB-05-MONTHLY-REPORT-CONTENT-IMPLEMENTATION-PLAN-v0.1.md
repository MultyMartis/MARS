# I-SEO Report Hub — DB-05 Monthly Report Content Implementation Plan v0.1
...
**Related:** [I-SEO-REPORT-HUB-DB-05-MONTHLY-REPORT-CONTENT-CHARTER-v0.1.md](I-SEO-REPORT-HUB-DB-05-MONTHLY-REPORT-CONTENT-CHARTER-v0.1.md), [I-SEO-REPORT-HUB-DB-05-MONTHLY-REPORT-CONTENT-SCHEMA-PLAN-v0.1.md](I-SEO-REPORT-HUB-DB-05-MONTHLY-REPORT-CONTENT-SCHEMA-PLAN-v0.1.md), [I-SEO-REPORT-HUB-DB-05-MONTHLY-REPORT-CONTENT-VALIDATION-PLAN-v0.1.md](I-SEO-REPORT-HUB-DB-05-MONTHLY-REPORT-CONTENT-VALIDATION-PLAN-v0.1.md)
...
| Sequence | **`_000004`** (authoritative next sequence after `_000003`) |
| If system date differs | Keep `_000004`; adjust date prefix only if project convention requires the apply-day date — do **not** renumber sequence |
...
| Path / class | Allowed |
|--------------|---------|
| `app-source/database/migrations/2026_07_26_000004_create_monthly_report_contents_table.sql` | **Yes** (create) |
...
| Prior migrations `000001` / `000002` / `000003` | **No** edits |
...
| Migration count | **3** | **4** |
| Table count | **11** | **12** |
| New table | — | `monthly_report_contents` |
...
2. Insert **one** demo monthly report content row:
   - status `draft`
   - title `Demo Monthly Report — July 2026 — LOCAL_FIXTURE_ONLY`
   - all content TEXT fields contain `LOCAL_FIXTURE_ONLY`
   - `source_weekly_checkpoint_ids` = JSON array of current local fixture weekly checkpoint ids for `2026-07`, resolved dynamically by `checkpoint_key` (`2026-07-W1` … `2026-07-W4` if present)
   - `owner_user_id` / `created_by` / `updated_by` = local admin id if safely resolvable
3. Reject duplicate `reporting_period_id`.
4. Reject invalid `reporting_period_id` FK.
5. Reject invalid `status`.
6. Verify JSON validity for `source_weekly_checkpoint_ids` if applicable.
...
`feat(iseo-report-hub): add db05 monthly report content migration`
...
- Migration `_000004` already applied with different checksum and no repair charter
- Apply charter attempts monthly CRUD UI, report blocks, export, or portal in same wave without explicit expansion

Output token pattern:

`STOP — I-SEO MONTHLY REPORT CONTENT DB-05 MIGRATION APPLY SAFETY CONDITION FAILED`

---

## 8. Explicit non-goals of next wave

- Monthly report CRUD UI
- Report block editor
- PDF / export / client portal
- Topvisor / API metrics tables
- Production deployment
- Automatic weekly→monthly text aggregation tool (beyond optional demo insert)

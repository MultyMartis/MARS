...
<section class="panel">
    <p class="note">
        ...
        · <a href="<?= e(url_path('/reporting-periods/' . $periodId . '/weekly-checkpoints')) ?>">Список заметок</a>
    </p>
</section>
...
<section class="panel">
    <h2><?= $isEdit ? 'Изменить еженедельную заметку' : 'Создать еженедельную заметку' ?></h2>
    <form class="rp-form" method="post" action="<?= e($action) ?>" novalidate>
        ...
        <label>
            ...
            <span class="field-hint">Родительский период неизменяем.</span>
        </label>

        <div class="form-row">
            <label>
                <span>Номер недели (1–6)</span>
                <input type="number" name="week_index" min="1" max="6" value="<?= e((string) $old['week_index']) ?>"<?= $locks['week_index'] ? ' readonly' : ' required' ?>>
                <?php if ($locks['week_index']): ?><span class="field-hint">Номер недели заблокирован после черновика.</span><?php endif; ?>
                <?php if (isset($errors['week_index'])): ?><span class="field-error"><?= e($errors['week_index']) ?></span><?php endif; ?>
            </label>
            <label>
                <span>Ключ заметки (YYYY-MM-WN)</span>
                ...
                <?php if ($locks['checkpoint_key']): ?><span class="field-hint">Ключ заметки заблокирован после черновика.</span><?php endif; ?>
        ...
        <?php if ($locks['dates']): ?><p class="field-hint">Даты заблокированы вне статусов черновик/в работе.</p><?php endif; ?>
        ...
        <label>
            ...
            <span class="field-hint">Жёсткое удаление недоступно — используйте пропуск или архив.</span>
        </label>
        ...
        <p>
            <button class="btn" type="submit"><?= $isEdit ? 'Сохранить изменения' : 'Создать заметку' ?></button>

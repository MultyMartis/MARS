...
$shareReason = ui_message((string) ($shareEligibility['reason'] ?? 'Готовность не оценена.'));
if ($shareReason === '') {
    $shareReason = 'Готовность не оценена.';
}
...
?>
<section class="card panel export-card export-detail mb-24">
    <div class="panel-head">
        <h2>Файл отчета</h2>
        <p>
            <?php if ($exportId > 0 && $exportStatus === 'ready'): ?>
                <a class="btn btn-primary" href="<?= e(url_path('/report-exports/' . $exportId . '/download')) ?>"><?= e($downloadLabel) ?></a>
            <?php endif; ?>
            <?php if ($exportId > 0): ?>
                <a class="btn btn-primary" href="<?= e(url_path('/report-exports/' . $exportId . '/shares')) ?>">Ссылки для клиента</a>
            <?php endif; ?>
            <?php if ($snapshotId > 0): ?>
...
<section class="card panel export-card handoff-panel mb-24" data-handoff-panel>
    <div class="panel-head">
        ...
        <p>
            <a class="btn btn-primary" href="<?= e(url_path('/report-exports/' . $exportId . '/shares')) ?>">Открыть ссылки и тексты</a>
        </p>
    </div>
    ...
    <?php if ($shareEligible): ?>
        ...
        <?php if ($activeShareCount > 0 || !empty($hShare['has_active'])): ?>
            <p class="field-hint handoff-active-share">Активная ссылка уже есть. Полный URL показывается только при создании.</p>
        <?php endif; ?>
    <?php else: ?>
        <p>
...
<details class="card panel tech-details technical-details">

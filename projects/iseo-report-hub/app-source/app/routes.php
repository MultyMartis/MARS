...
use Iseo\Controllers\MonthlyReportAssemblyController;
use Iseo\Controllers\MonthlyReportContentController;
use Iseo\Controllers\MonthlyReportContentWorkflowController;
use Iseo\Controllers\MonthlyReportWorkEntryController;
...
$workEntryRepo = new MonthlyReportWorkEntryRepository($db);
$seoWorkCategoryRepo = new SeoWorkCategoryRepository($db);
$seoWorkItemRepo = new SeoWorkItemRepository($db);
$workEntryService = new MonthlyReportWorkEntryService(
    $workEntryRepo,
    $monthlyRepo,
    $seoWorkCategoryRepo,
    $seoWorkItemRepo,
...
$monthlyReportWorkEntries = new MonthlyReportWorkEntryController(
    $app,
    $view,
    $config,
    $auth,
...
$assemblyService = new MonthlyReportSummaryAssemblyService(
    $workEntryRepo,
    $monthlyRepo,
    $blockRepo,
    $db
);
$applyService = new MonthlyReportSummaryApplyService(
    $assemblyService,
    $monthlyRepo,
    $blockRepo,
    $db
);
$monthlyReportAssembly = new MonthlyReportAssemblyController(
    $app,
    $view,
    $config,
    $auth,
...
$contentWorkflowService = new SpecialistReportContentWorkflowService(
    $monthlyRepo,
    $blockRepo,
    $assemblyService,
    $db
);
$monthlyReportContentWorkflow = new MonthlyReportContentWorkflowController(
    $app,
    $view,
    $config,
    $auth,
...
$reportPreview = new ReportPreviewController(
    $app,
    $view,
    $config,
    $auth,
...
} elseif (preg_match('#^/monthly-reports/(\d+)/work-entries/create$#', $requestPath, $m) === 1) {
    $reportId = (int) $m[1];
    $router->get($requestPath, static function () use ($monthlyReportWorkEntries, $reportId): void {
        $monthlyReportWorkEntries->create($reportId);
    });
} elseif (preg_match('#^/monthly-reports/(\d+)/work-entries$#', $requestPath, $m) === 1) {
    $reportId = (int) $m[1];
    $router->post($requestPath, static function () use ($monthlyReportWorkEntries, $reportId): void {
        $monthlyReportWorkEntries->store($reportId);
    });
} elseif (preg_match('#^/monthly-report-work-entries/(\d+)/edit$#', $requestPath, $m) === 1) {
    $entryId = (int) $m[1];
    $router->get($requestPath, static function () use ($monthlyReportWorkEntries, $entryId): void {
        $monthlyReportWorkEntries->edit($entryId);
    });
} elseif (preg_match('#^/monthly-report-work-entries/(\d+)$#', $requestPath, $m) === 1) {
    $entryId = (int) $m[1];
    $router->post($requestPath, static function () use ($monthlyReportWorkEntries, $entryId): void {
        $monthlyReportWorkEntries->update($entryId);
    });
...
} elseif (preg_match('#^/monthly-reports/(\d+)/content-workflow/sections/([a-z_]+)$#', $requestPath, $m) === 1) {
    $reportId = (int) $m[1];
    $sectionKey = (string) $m[2];
    $router->post($requestPath, static function () use ($monthlyReportContentWorkflow, $reportId, $sectionKey): void {
        $monthlyReportContentWorkflow->saveSection($reportId, $sectionKey);
    });
} elseif (preg_match('#^/monthly-reports/(\d+)/content-workflow$#', $requestPath, $m) === 1) {
    $reportId = (int) $m[1];
    $router->get($requestPath, static function () use ($monthlyReportContentWorkflow, $reportId): void {
        $monthlyReportContentWorkflow->show($reportId);
    });
} elseif (preg_match('#^/monthly-reports/(\d+)/assembly-preview$#', $requestPath, $m) === 1) {
    $reportId = (int) $m[1];
    $router->get($requestPath, static function () use ($monthlyReportAssembly, $reportId): void {
        $monthlyReportAssembly->preview($reportId);
    });
} elseif (preg_match('#^/monthly-reports/(\d+)/assembly-apply$#', $requestPath, $m) === 1) {
    $reportId = (int) $m[1];
    $router->post($requestPath, static function () use ($monthlyReportAssembly, $reportId): void {
        $monthlyReportAssembly->apply($reportId);
    });

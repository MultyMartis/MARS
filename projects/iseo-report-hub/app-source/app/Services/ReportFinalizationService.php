...
final class ReportFinalizationService
{
    ...
    private function buildGates(?array $monthly, ?array $period): array
    {
        $gates = [
            'monthly_exists' => [
                'pass' => $monthly !== null,
                'detail' => $monthly !== null ? 'Месячный отчет найден.' : 'Месячный отчет не найден.',
            ],
            'period_exists' => [
                'pass' => $period !== null,
                'detail' => $period !== null ? 'Отчетный период найден.' : 'Отчетный период не найден.',
            ],
            'title_present' => [
                'pass' => false,
                'detail' => 'Название не заполнено.',
            ],
            'preview_renderable' => [
                'pass' => false,
                'detail' => 'Предпросмотр не собирается.',
            ],
            'render_mode_valid' => [
                'pass' => false,
                'detail' => 'Режим генерации не подходит для финализации.',
            ],
            'has_non_archived_blocks' => [
                'pass' => false,
                'detail' => 'Нет активных блоков отчета.',
            ],
            'required_blocks_present' => [
                'pass' => false,
                'detail' => 'Не найдены обязательные блоки.',
            ],
            'required_blocks_reviewed' => [
                'pass' => false,
                'detail' => 'Обязательные блоки еще не проверены.',
            ],
            'no_draft_or_in_progress_blocks' => [
                'pass' => false,
                'detail' => 'Остаются черновые блоки.',
            ],
            'source_weekly_refs_resolve' => [
                'pass' => false,
                'detail' => 'Связанные недельные заметки не найдены.',
            ],
        ];
        ...
        $gates['title_present'] = [
            'pass' => $title !== '',
            'detail' => $title !== '' ? 'Название заполнено.' : 'Название пустое.',
        ];
        ...
        $gates['preview_renderable'] = [
            'pass' => $previewOk,
            'detail' => $previewOk ? 'Предпросмотр собирается успешно.' : 'Не удалось собрать предпросмотр.',
        ];
        ...
        $gates['render_mode_valid'] = [
            'pass' => $modeValid,
            'detail' => $modeValid
                ? 'Режим генерации «' . $renderMode . '» допустим.'
                : 'Режим генерации «' . $renderMode . '» не подходит для финализации (нужен blocks_primary или flat_fallback).',
        ];
        ...
        $gates['has_non_archived_blocks'] = [
            'pass' => $active !== [],
            'detail' => $active !== []
                ? 'Активных блоков: ' . count($active) . '.'
                : 'Нужен хотя бы один активный блок отчета.',
        ];
        ...
        $gates['required_blocks_present'] = [
            ...
            'detail' => $missingRequired === []
                ? 'Все обязательные блоки есть.'
                : 'Не найдены обязательные блоки: ' . implode(', ', $missingRequired),
        ];
        ...
        $gates['required_blocks_reviewed'] = [
            ...
            'detail' => ($missingRequired === [] && $notReviewed === [])
                ? 'Обязательные блоки проверены.'
                : 'Обязательные блоки не готовы: ' . implode(', ', $notReviewed),
        ];
        ...
        $gates['source_weekly_refs_resolve'] = [
            'pass' => $missingList === [],
            'detail' => $missingList === []
                ? 'Все связанные недельные заметки найдены.'
                : 'Не найдены недельные заметки: ' . implode(', ', $missingList),
        ];
    ...
    private function describeActions(array $actor, ?array $monthly, bool $ready, array $failed): array
    {
        ...
        if ($monthly === null) {
            foreach ($actions as $k => $_) {
                $actions[$k]['reason'] = 'Месячный отчет не найден.';
            }
            return $actions;
        }
        ...
        if ($this->userHasAnyRole($actor, self::SUBMIT_ROLES) && $status === 'in_progress') {
            ...
            $actions['submit_review']['reason'] = 'Роль не позволяет отправить на проверку.';
        } else {
            $actions['submit_review']['reason'] = 'Нужен статус «В работе» (сейчас: «' . \Iseo\Support\UiLabels::status($status) . '»).';
        }
        ...
        if ($this->userHasAnyRole($actor, self::REVIEW_ROLES) && $status === 'ready_for_review') {
            ...
            $actions['mark_reviewed']['reason'] = 'Роль не позволяет отметить проверенным.';
        } else {
            $actions['mark_reviewed']['reason'] = 'Нужен статус «На проверке» (сейчас: «' . \Iseo\Support\UiLabels::status($status) . '»).';
        }
        ...
        if ($this->userHasAnyRole($actor, self::FINALIZE_ROLES) && $status === 'reviewed' && $ready) {
            ...
            $actions['finalize']['reason'] = 'Роль не позволяет финализировать.';
        } elseif ($status !== 'reviewed') {
            $actions['finalize']['reason'] = 'Нужен статус «Проверено» (сейчас: «' . \Iseo\Support\UiLabels::status($status) . '»).';
        } else {
            $failedLabels = array_map(
                static fn (string $k): string => \Iseo\Support\UiLabels::readinessKey(trim($k)),
                $failed
            );
            $actions['finalize']['reason'] = 'Готовность не пройдена: ' . implode(', ', $failedLabels);
        }
        ...
        if ($this->userHasAnyRole($actor, self::REOPEN_ROLES) && $status === 'finalized') {
            ...
            $actions['reopen']['reason'] = 'Открыть снова может только admin_owner.';
        } else {
            $actions['reopen']['reason'] = 'Нужен статус «Финализирован» (сейчас: «' . \Iseo\Support\UiLabels::status($status) . '»).';
        }

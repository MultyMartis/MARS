...
| **Implementation** | **Phase 1B complete** + **local vhost/hosts mapping complete** + **DB `iseo_report_hub_dev` created** + **DB-01/DB-02 first migration applied** + **auth persistence + local admin bootstrap implemented** + **DB-03 reporting periods migration applied** + **local fixture apply complete** + **Reporting Period CRUD Implementation 01 complete** + **Weekly Checkpoints DB-04 Charter 01 complete** + **DB-04 migration apply complete** + **Weekly Checkpoints CRUD Charter 01 complete** + **Weekly Checkpoints CRUD Implementation 01 complete** + **Monthly Report Content DB-05 Charter 01 complete** + **DB-05 migration apply complete** + **Monthly Report Content CRUD Charter 01 complete** + **Monthly Report Content CRUD Implementation 01 complete** + **Report Blocks DB-06 Charter 01 complete** + **DB-06 migration apply complete** + **Report Blocks CRUD Charter 01 complete** + **Report Blocks CRUD Implementation 01 complete** + **Report Preview / Render Charter 01 complete** + **Report Preview / Render Implementation 01 complete** + **Report Finalization Charter 01 complete** + **Report Finalization Implementation 01 complete** + **Report Snapshot Charter 01 complete** + **Report Snapshot DB-07 Migration Apply 01 complete** — DB-backed login/logout; demo fixture + smoke period; internal reporting-period CRUD; `weekly_checkpoints` table + W1–W4; period-scoped weekly checkpoint CRUD; `monthly_report_contents` table + period-scoped monthly report content CRUD (demo id **1**, status **finalized** after finalization smoke); `report_blocks` table + monthly-scoped report block CRUD (6 local blocks under monthly id **1**); internal preview/print (`blocks_primary`); finalization workflow + readiness + locks; `report_snapshots` table (DB-07; **0** rows); snapshot service/routes/UI **not** yet; **no** drag/drop / PDF/export / client portal |
...
| Field | Value |
|-------|-------|
| **Status** | planned / product architecture + Phase 0 scaffold + Model A `app-source/` + Phase 1A/1B + local DB + **auth persistence implemented** + **DB-03 migration applied** + **local fixture apply complete** + **Reporting Period CRUD Implementation 01 complete** + **Weekly Checkpoints DB-04 Charter 01 complete** + **DB-04 migration apply complete** + **Weekly Checkpoints CRUD Charter 01 complete** + **Weekly Checkpoints CRUD Implementation 01 complete** + **Monthly Report Content DB-05 Charter 01 complete** + **DB-05 migration apply complete** + **Monthly Report Content CRUD Charter 01 complete** + **Monthly Report Content CRUD Implementation 01 complete** + **Report Blocks DB-06 Charter 01 complete** + **DB-06 migration apply complete** + **Report Blocks CRUD Charter 01 complete** + **Report Blocks CRUD Implementation 01 complete** + **Report Preview / Render Charter 01 complete** + **Report Preview / Render Implementation 01 complete** + **Report Finalization Charter 01 complete** + **Report Finalization Implementation 01 complete** + **Report Snapshot Charter 01 complete** + **Report Snapshot DB-07 Migration Apply 01 complete** |
| **Lane** | Lane B — product formation and architecture |
| **Active stage** | **Report Snapshot DB-07 Migration Apply 01 complete** — next recommended: **Report Snapshot Implementation 01** |
...
| **Next recommended stage** | **I-SEO Report Hub — Report Snapshot Implementation 01** |
...
1. **Report Snapshot Implementation 01** — **recommended next** (DB-07 `report_snapshots` applied; 0 rows; service/routes/UI + first snapshot create)
2. Optional: **Report Blocks CRUD Hardening 01** if multi-role HTTP smoke is needed
3. Optional: **Monthly Report Content CRUD Hardening 01** if multi-role HTTP smoke is needed
4. Optional: **Weekly Checkpoints CRUD Hardening 01** if multi-role HTTP smoke is needed
5. Optional: **Reporting Period CRUD Hardening 01** if account-manager edit / multi-role smoke is needed
6. Optional parallel: **v0.5 demo corrections** from backlog (UX only; not product runtime)
7. **SEO specialist feedback** — still **deferred** until operator opens feedback charter
8. Work dictionary extraction/sanitization (из Nikita materials; **exclude** credential sheet)
9. MVP implementation phases 2–11 per implementation charter (Anton / i-SEO)
10. Later: n8n/API/AI integration (events only; human approval gates); later export/snapshot from finalized reports
...
- **Local MySQL DB `iseo_report_hub_dev` exists** with core auth/org tables + **`reporting_periods`** (DB-03) + **`weekly_checkpoints`** (DB-04) + **`monthly_report_contents`** (DB-05) + **`report_blocks`** (DB-06) + **`report_snapshots`** (DB-07 applied; **0** rows; migrations **6**; tables **14**)
...
- **Report Snapshot DB-07 Migration Apply 01 is complete** — `report_snapshots` table exists; **0** rows; migrations **6**; tables **14**; **no** snapshot service/routes/UI yet
- **No snapshot rows / PDF / public share yet** — next = Report Snapshot Implementation 01

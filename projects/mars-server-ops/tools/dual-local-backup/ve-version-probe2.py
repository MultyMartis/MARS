...
echo ---
pid=$(pgrep -n xray || true)
echo PID=$pid
if [ -n "$pid" ]; then readlink -f /proc/$pid/exe; tr '\0' ' ' </proc/$pid/cmdline; echo; fi
"""
_, o, e = c.exec_command(cmd, timeout=60)
text = o.read().decode("utf-8", "replace") + e.read().decode("utf-8", "replace")
Path(
    r"X:\AI MARS\projects\mars-server-ops\evidence\DUAL-LOCAL-BACKUP-FRIENDHOSTING-VEESP-01\ve-versions.txt"

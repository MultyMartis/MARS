...
**Agent identities:** stable planned **`agent_id`** rows in [`../../agents/registry.md`](../../agents/registry.md) §4.1; **v0 agent cards** for all listed factory roles in [`../../agents/cards/`](../../agents/cards/) (documentation-only, **not** runtime). **SoT** for extended per-role prose remains **[`agent-map.md`](agent-map.md)**. Factory **`entity_id`** rows: [`../../governance/dependency-map.md`](../../governance/dependency-map.md) §4 (`mars_website_factory`, `website_factory_*`, `website_factory_workflow_v0`, `website_factory_semantic_relationship_layer_v0`).
...
*Last updated: 2026-05-12 — **Semantic Relationship Layer v0** (overview, object model, cross-artifact semantics, semantic dependency rules, inheritance, consistency, conceptual site semantic graph, freeze semantics, semantic QA; **documentation only — not a graph database, not a vector engine, not a semantic runtime/automation layer, not autonomous reasoning**); **Reference Project Layer v0** (reference project model, artifact tree, lifecycle, HITL governance, QA matrix, delivery packages, multi-page orchestration; **documentation only — not storage, not runtime persistence, not a project database, not an orchestration engine**); previously: 2026-05-11 — **Execution Semantics Layer v0**; Prompt Standards Layer v0; Artifact Architecture Layer v0; Website Factory Workflow v0; agent cards v0; registries + handoff contracts + blueprint QA; documentation only.*

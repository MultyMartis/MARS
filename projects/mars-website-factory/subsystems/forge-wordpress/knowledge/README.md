...
| Global vs document-local IDs | Global sequential IDs (`FW-S-*`, `AP-*`) are unique within family. Local packs use `AP-CMS-*` / `AP-COOKIE-*` / `PRIVACY-*` / `FORM-SPAM-*` / `INDEX-*`. Allocation rules: [contracts register](../registries/FORGE-WORDPRESS-CONTRACTS-AND-STANDARDS-REGISTER-v1.md) · [ANTI-PATTERN-REGISTRY](../standards/FORGE-WORDPRESS-ANTI-PATTERN-REGISTRY-v1.md) |

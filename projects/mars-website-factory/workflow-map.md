...
**SCSS gate:** [css-variable-first-law-v1.md](css-variable-first-law-v1.md) — reusable values from shared token lookup; unique geometry → direct local CSS. [universal-style-scale-law-v1.md](universal-style-scale-law-v1.md) — compact `--pad-*` scale; unified `--radius-main` / `--radius-full`. [one-project-scss-file-law-v1.md](one-project-scss-file-law-v1.md) — all project styles in `src/scss/style.scss`; no project partials by default. [universal-button-system-law-v1.md](universal-button-system-law-v1.md) — single `.btn` system; no parallel block button geometry. [no-button-letter-spacing-law-v1.md](no-button-letter-spacing-law-v1.md). Visual QA corrections via core scale or documented exact geometry only.

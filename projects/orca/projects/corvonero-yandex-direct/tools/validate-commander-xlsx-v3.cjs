...
/**
 * Strict structural validation for Corvonero Commander XLSX v3.
 */
const fs = require('fs');
...
const XLSX = path.join(ROOT, 'exports/CORVONERO-YANDEX-DIRECT-COMMANDER-v3.xlsx');
const DATASET = path.join(ROOT, 'production/direct-commander-production-dataset-v3.json');
const OUT_JSON = path.join(ROOT, 'production/validation/direct-commander-v3-validation.json');
const OUT_MD = path.join(ROOT, 'production/validation/direct-commander-v3-validation.md');

const DATA_START = 16;
const COL = { groupName: 5, phrase: 8, h1: 10, h2: 11, text: 12, url: 48, bid: 54, groupNeg: 68, adType: 2 };

const BAD_AD_CLAIMS = [/под ключ/i, /срочно/i, /быстро и точно/i, /гарант/i, /24\/7/, /официальн.*партн/i, /сертифиц/i, /лучш/i, /любой сложности/i, /без ошибок/i, /результат гарант/i];
const BAD_KW_PATTERNS = [
  { re: /^как (сделать|настроить|изменить)/, label: 'DIY/informational' },
...
function main() {
  ...
  const dsCollisions = (dataset.collision_validation?.errors || []).length;
  if (dsCollisions) errors.push(`Dataset collision errors: ${dsCollisions}`);

...
const XLSX = path.join(ROOT, 'exports/CORVONERO-YANDEX-DIRECT-COMMANDER-v6.xlsx');
const DATASET = path.join(ROOT, 'production/direct-commander-production-dataset-v6.json');
const SEMANTIC = path.join(ROOT, 'production/validation/semantic-validation-v6.json');
const REVIEW_WB = path.join(ROOT, 'production/validation/review-workbook-v6-result.json');
const OUT_JSON = path.join(ROOT, 'production/validation/direct-commander-validation-v6.json');
const OUT_MD = path.join(ROOT, 'production/validation/direct-commander-validation-v6.md');
...
function main() {
  ...
  for (const r of dataRows) {
    ...
    if (ph) {
      ...
      for (const edu of EDUCATION_PHRASES) {
        if (np === normPhrase(edu)) errors.push(`row ${r}: education/career phrase leaked to export: ${ph}`);
      }
  ...
  if (fs.existsSync(SEMANTIC)) {
    ...
    if (!sem.passed) errors.push('semantic-validation-v6.json: FAILED');
    if ((sem.checks?.career_education_leakage || 0) > 0) {
      errors.push(`Education/career leakage: ${sem.checks.career_education_leakage}`);
    }
    checks.push(`semantic_validation: ${sem.passed ? 'pass' : 'fail'}`);
  } else {
    errors.push('semantic-validation-v6.json missing');
  }

  if (fs.existsSync(REVIEW_WB)) {
    const rw = JSON.parse(fs.readFileSync(REVIEW_WB, 'utf8'));
    if (!rw.evidence_populated) errors.push('Review workbook v6 evidence not populated');
    checks.push(`review_workbook_evidence: ${rw.evidence_populated ? 'pass' : 'fail'}`);
  } else {
    warnings.push('review-workbook-v6-result.json missing — run generate-review-workbook-v6.cjs');
  }

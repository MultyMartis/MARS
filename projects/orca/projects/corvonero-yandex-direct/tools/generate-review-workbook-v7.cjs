...
/**
 * Human review workbook v7 — 30-sheet operator deliverable.
 */
const fs = require('fs');
...
const OUTPUT = path.join(ROOT, 'exports/CORVONERO-CAMPAIGN-REVIEW-v7.xlsx');
...
async function main() {
  ...
  const sheets = {
    ...
    'Collision summary': [
      ...
      ['pair_layer_note', 'v7 scope recovery — cross-negative narrowed for CORV-G05-06 data transfer'],
    ],
    'Collision findings': [
  ...
  const reviewDir = path.join(ROOT, 'exports/review-v7-csv');
  ...
  const result = {
    ...
    note: xlsxWritten ? 'Review XLSX v7 generated' : 'CSV fallback only',
  };

  fs.writeFileSync(path.join(ROOT, 'production/validation/review-workbook-v7-result.json'), JSON.stringify(result, null, 2));
  fs.writeFileSync(
    path.join(ROOT, 'production/validation/review-workbook-validation-v7.json'),
    JSON.stringify({ validated_at: new Date().toISOString(), passed: result.evidence_populated, integrity }, null, 2)
  );
  fs.writeFileSync(
    path.join(ROOT, 'production/validation/review-workbook-validation-v7.md'),

...
/**
 * Corvonero Direct Commander XLSX export v7 — unified single campaign.
 */
const fs = require('fs');
const path = require('path');

const TRIUMPH_CLI = path.resolve(__dirname, '../../../ppc/triumph-manipulator/tools/exporter-cli');
...
const DATASET_PATH = path.join(ROOT, 'production/direct-commander-production-dataset-v7.1.json');
const OUTPUT = path.join(ROOT, 'exports/CORVONERO-YANDEX-DIRECT-COMMANDER-v7.1.xlsx');
...
function buildFillRows(dataset) {
  ...
  for (const group of dataset.groups) {
    const groupName = normalizeTransportText(group.group_export_name || group.group_name);
...
async function main() {
  if (!fs.existsSync(DATASET_PATH)) {
    console.error('Run run-full-production-v7.mjs first');
    process.exit(1);
  }
  ...
  fs.writeFileSync(
    path.join(ROOT, 'production/validation/export-run-result-v7.1.json'),

...
/**
 * Corvonero Direct Commander XLSX export v3 — unified single campaign.
 */
const fs = require('fs');
...
const DATASET_PATH = path.join(ROOT, 'production/direct-commander-production-dataset-v3.json');
const OUTPUT = path.join(ROOT, 'exports/CORVONERO-YANDEX-DIRECT-COMMANDER-v3.xlsx');
...
async function main() {
  if (!fs.existsSync(DATASET_PATH)) {
    console.error('Run run-full-production-v3.mjs first');
    process.exit(1);
  }
  ...
  fs.writeFileSync(
    path.join(ROOT, 'production/validation/export-run-result-v3.json'),

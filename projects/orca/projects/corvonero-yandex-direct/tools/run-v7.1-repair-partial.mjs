...
async function validateActualReviewXlsx(xlsxPath, dataset, controlled) {
  const wb = new ExcelJS.Workbook();
  await wb.xlsx.readFile(xlsxPath);
  const errors = [];
  const ws = wb.getWorksheet('Keywords');
  if (!ws) errors.push('Keywords sheet missing');
  ...
  ws?.eachRow((row, i) => {
    if (i === 1) return;
    rowCount++;
    const hyp = row.getCell(8).value;
    const phrase = String(row.getCell(3).value || '');
    const hypStr = hyp == null ? '' : typeof hyp === 'object' ? JSON.stringify(hyp) : String(hyp);
    if (hypStr === '272' || hyp === 272) hypothesis272++;
    const v = isValidNarrativeField(hypStr, { field: 'hypothesis' });
    if (!v.ok && hypStr !== NON_CONTROLLED_HYPOTHESIS_SENTINEL_RU) {
      if (!controlledById.size || !phrase) {
        errors.push(`row ${i} hypothesis invalid: ${v.reason} = ${hypStr.slice(0, 40)}`);
      }
    }
    if (!hypStr || hypStr.trim() === '') {
      errors.push(`row ${i} empty hypothesis`);
    }
  });

  if (rowCount !== dataset.keywords.length) {
    errors.push(`keyword rows ${rowCount} vs dataset ${dataset.keywords.length}`);
  }
  if (hypothesis272 > 0) errors.push(`hypothesis=272 count ${hypothesis272}`);

  return { passed: errors.length === 0, errors, rowCount, hypothesis272 };
}

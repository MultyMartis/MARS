...
const XLSX = path.join(ROOT, 'exports/CORVONERO-YANDEX-DIRECT-COMMANDER-v4.xlsx');
const DATASET = path.join(ROOT, 'production/direct-commander-production-dataset-v4.json');
const SEMANTIC = path.join(ROOT, 'production/validation/semantic-review-v4.json');
const REVIEW_WB = path.join(ROOT, 'production/validation/review-workbook-v4-result.json');
const OUT_JSON = path.join(ROOT, 'production/validation/direct-commander-v4-validation.json');
const OUT_MD = path.join(ROOT, 'production/validation/direct-commander-v4-validation.md');
...
const BAD_KW_PATTERNS = [
  ...
  { re: /тестирование доработок/, label: 'training' },
];
...
function main() {
  ...
  const dsCollisions = (dataset.collision_validation?.after?.blocking || 0);
  if (dsCollisions) errors.push(`Dataset collision blocking after correction: ${dsCollisions}`);

  if (fs.existsSync(SEMANTIC)) {
    const sem = JSON.parse(fs.readFileSync(SEMANTIC, 'utf8'));
    if (!sem.passed) errors.push('semantic-review-v4.json: FAILED');
    if (sem.checks?.informational_in_active?.length) {
      errors.push(`Informational phrases in active set: ${sem.checks.informational_in_active.length}`);
    }
    if (sem.checks?.operator_anchor_leaks?.length) {
      errors.push(`Operator anchor leaks: ${sem.checks.operator_anchor_leaks.join('; ')}`);
    }
    checks.push(`semantic_review: ${sem.passed ? 'pass' : 'fail'}`);
  } else {
    errors.push('semantic-review-v4.json missing');
  }

  if (fs.existsSync(REVIEW_WB)) {
    const rw = JSON.parse(fs.readFileSync(REVIEW_WB, 'utf8'));
    if (!rw.evidence_populated) errors.push('Review workbook collision evidence not populated');
    checks.push(`review_workbook_evidence: ${rw.evidence_populated ? 'pass' : 'fail'}`);
  } else {
    warnings.push('review-workbook-v4-result.json missing — run generate-review-workbook-v4.cjs');
  }

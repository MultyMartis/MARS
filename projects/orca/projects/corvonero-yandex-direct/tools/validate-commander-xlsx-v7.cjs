...
const XLSX = path.join(ROOT, 'exports/CORVONERO-YANDEX-DIRECT-COMMANDER-v7.xlsx');
const DATASET = path.join(ROOT, 'production/direct-commander-production-dataset-v7.json');
const SEMANTIC = path.join(ROOT, 'production/validation/semantic-validation-v7.json');
const REVIEW_WB = path.join(ROOT, 'production/validation/review-workbook-v7-result.json');
const OUT_JSON = path.join(ROOT, 'production/validation/direct-commander-validation-v7.json');
const OUT_MD = path.join(ROOT, 'production/validation/direct-commander-validation-v7.md');
...
/** v7 hard informational excludes — must not appear in Commander export */
...
function main() {
  ...
  for (const r of dataRows) {
    ...
    if (ph) {
      ...
      for (const banned of V7_HARD_EXCLUDE_PHRASES) {
        if (np === normPhrase(banned)) errors.push(`row ${r}: informational phrase leaked to export: ${ph}`);
      }
  ...
  if (fs.existsSync(SEMANTIC)) {
    ...
    if (!sem.passed) errors.push('semantic-validation-v7.json: FAILED');
    if ((sem.checks?.active_informational_leakage || 0) > 0) {
      errors.push(`Informational leakage: ${sem.checks.active_informational_leakage}`);
    }
    checks.push(`semantic_validation: ${sem.passed ? 'pass' : 'fail'}`);
  } else {
    errors.push('semantic-validation-v7.json missing');
  }

  if (fs.existsSync(REVIEW_WB)) {
    const rw = JSON.parse(fs.readFileSync(REVIEW_WB, 'utf8'));
    if (!rw.evidence_populated) errors.push('Review workbook v7 evidence not populated');
    checks.push(`review_workbook_evidence: ${rw.evidence_populated ? 'pass' : 'fail'}`);
  } else {
    warnings.push('review-workbook-v7-result.json missing — run generate-review-workbook-v7.cjs');
  }

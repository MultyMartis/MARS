...
'use strict';
/**
 * Triumph Contract Audit Workbook v7 — 14 sheets.
 */
const fs = require('fs');
...
const LAWS = path.join(ROOT, '../../../knowledge/triumph-derived-orca-laws-v1.md');
const OUTPUT = path.join(ROOT, 'exports/CORVONERO-V7-TRIUMPH-CONTRACT-AUDIT.xlsx');
  ...
  const sheets = {
    ...
    'Operator scope coverage': [
      ...
      ...(scope.services || []).map((s) => {
        ...
        const match = !s.recovery_required || !String(s.current_group_status).includes('HOLD') || String(prod).includes('HOLD') ? 'OK' : 'STALE';
    ...
    'Required corrections': [
      ...
      ...(audit.high_violations || []).map((v, i) => ['HIGH', v.entity_id, v.required_action]),
      ['MANUAL', 'operator-service-scope-v1.json', 'Sync 6 recovered service statuses from HOLD to ACTIVE NARROW'],
      ['MANUAL', 'Operator approval', 'Review actual v7 XLSX independently of pipeline PASS'],
    ],
    'Gate decision': [
      ['gate', audit.gate_decision],
      ['next_step', audit.gate_decision.startsWith('PASS') ? 'ACTUAL V7 XLSX REVIEW AND COMMANDER DRY-RUN' : 'CONTRACT-GUIDED V7 CORRECTION PLAN'],
      ['v8', 'NOT AUTHORIZED IN THIS TASK'],
    ],
  };

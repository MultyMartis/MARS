# ORCA PPC Semantic Core — World Practice Research (Normalized Companion v1)
...
Вывод: для ORCA оптимален **hybrid hierarchical system** с четким разделением authority layers. Выбор поддерживается промышленными кейсами Snorkel/weak supervision, intent taxonomies и selective classification. [SRC-SNORKEL-PAPER-v0; SRC-ORCAS-I-PAPER-v0; SRC-ACTIVE-LEARNING-SURVEY-v0; SRC-COST-SENSITIVE-REJECTION-v0]

### Референсная архитектура ORCA {#sec-референсная-архитектура-orca}
...
### Стандарт semantic adjudication для одной фразы {#sec-стандарт-semantic-adjudication-для-одной-фразы}
...
### Практическое дерево решений {#sec-практическое-дерево-решений}
...
1. **Есть ли очевидный hard exclusion?**  
   Карьера, вакансии, стажировка, резюме, обучение, курсы, с нуля, бесплатно скачать, DIY/how-to, нормативка, личный кабинет, документация, forum-only intent, malformed spam. Если да — `reject`, если нет — дальше. Поддержка такой предфильтрации согласуется с weak supervision и intent taxonomies. [SRC-ORCAS-I-PAPER-v0; SRC-SNORKEL-PAPER-v0]

2. **Какова буквальная задача пользователя?**  
   Нанять, купить, разобраться самому, изучить, найти работу, скачать, войти, узнать правила, найти модуль/продукт, получить поддержку. Если задача не формулируется без натяжки — `abstain`. [SRC-PRODUCT-INSIGHTS-BING-v0; SRC-SEARCH4CODE-PAPER-v0]
...
Рекомендованное operational rule: **если сомнение нельзя снять из буквальной формулировки запроса, ORCA должна abstain, а не “домысливать коммерцию”**. Это соответствует логике classification with rejection и cost-sensitive admission. [SRC-COST-SENSITIVE-REJECTION-v0]

## План исследования, gold dataset, метрики и безопасный пилот {#sec-план-исследования-gold-dataset-метрики-и-безопасный-пилот}

### Дизайн benchmark и gold dataset {#sec-дизайн-benchmark-и-gold-dataset}

Для ORCA нужен не случайный sample, а **stratified benchmark**, где отдельно представлены классы, критичные именно для дорогих false positives. ORCAS-I и исследования по weak supervision/active learning полезны здесь прежде всего тем, что показывают ценность явной taxonomy, abstain-класса и targeted annotation на трудноразрешимых кейсах. [SRC-ORCAS-I-PAPER-v0; SRC-SNORKEL-PAPER-v0; SRC-ACTIVE-LEARNING-SURVEY-v0]
...
Практический размер первого качественного gold set: **1,200–2,000 фраз**, из которых blind test — не менее 300–400, pilot set — 300–500, hard-negative set — отдельный и фиксированный. Такой объем уже позволяет сравнить baseline rules, weak supervision baseline, hybrid model и human-review policy без иллюзии качества на слишком маленькой выборке. Если available historical search terms, CRM labels и отраслевой split отсутствуют, это нужно пометить как **не указано** и заложить в риски. [SRC-ORCA-ATTACHED-BRIEF-v0] [SRC-ACTIVE-LEARNING-SURVEY-v0; SRC-SNORKEL-PAPER-v0]

### Метрики и production thresholds {#sec-метрики-и-production-thresholds}

Для ORCA главная бизнес-ошибка — не общий recall loss, а **ложноположительный допуск некоммерческих фраз в ядро**. Поэтому production-blocker должен быть связан с precision и cost-weighted risk, а не только с macro-F1. Современные системы необходимости abstention и calibrated confidence подтверждаются работами по rejection/cost-sensitive classification и calibration. [SRC-COST-SENSITIVE-REJECTION-v0]
...
Если система «признает слишком много фраз коммерческими», production должна блокировать именно **Commercial Precision на auto-accept потоке**. Это лучший one-number guardrail для ORCA, потому что напрямую защищает от дорогих false positives при платформенном семантическом расширении. [SRC-GOOGLE-ADS-SEARCH-TECH-GUIDE-v0; SRC-GOOGLE-ADS-KEYWORD-MATCHING-v0; SRC-YANDEX-DIRECT-AUTOTARGETING-v0; SRC-COST-SENSITIVE-REJECTION-v0]

### Safe pilot design {#sec-safe-pilot-design}

Безопасный пилот перед перезапуском Corvonero должен идти через размеченную выборку, а не через полный корпус. Это полностью соответствует attached brief. [SRC-ORCA-ATTACHED-BRIEF-v0]
...
### Failure-mode catalogue {#sec-failure-mode-catalogue}
...
### Приоритетный backlog ORCA {#sec-приоритетный-backlog-orca}
...
### Suggested search queries {#sec-suggested-search-queries}
...
### Стартовый набор авторитетных источников {#sec-стартовый-набор-авторитетных-источников}
...
1. **Google Ads Help — About keyword matching options**: официальное описание broad / phrase / exact, дополнительных сигналов broad match и роли Smart Bidding. [SRC-GOOGLE-ADS-KEYWORD-MATCHING-v0]  
2. **Google Ads technical guide — Unlock the Power of Search**: официальная схема того, как Google интерпретирует query, проверяет eligibility и выбирает keyword/ad group. [SRC-GOOGLE-ADS-SEARCH-TECH-GUIDE-v0]  
3. **Google Ads Help — About the search terms report**: официальный источник по post-launch search-term learning и ограничениям видимости. [SRC-GOOGLE-ADS-SEARCH-TERMS-REPORT-v0]  
4. **Google Ads Help — About negative keywords**: официальный источник по negative match types и ограничениям close variants. [SRC-GOOGLE-ADS-NEGATIVE-KEYWORDS-v0]  
5. **Yandex Direct — Keywords**: официальное описание word-by-word и semantic matching, stop words и search query report. [SRC-YANDEX-DIRECT-KEYWORDS-v0]  
6. **Yandex Direct — Symbols and operators**: официальный справочник по `" "`, `!`, `+`, `[]` и их смысловым последствиям. [SRC-YANDEX-DIRECT-OPERATORS-v0]  
7. **Yandex Direct — Negative keywords**: официальный источник по минус-фразам, уровням применения и рискам отсечения аудитории. [SRC-YANDEX-DIRECT-NEGATIVE-KEYWORDS-v0]  
8. **Yandex Direct — Autotargeting**: официальный источник по non-keyword matching и роли landing/ad text. [SRC-YANDEX-DIRECT-AUTOTARGETING-v0]  
9. **ORCAS-I: Queries Annotated with Intent using Weak Supervision**: полезная intent taxonomy с abstain-слоем и weak supervision на веб-запросах. [SRC-ORCAS-I-PAPER-v0]  
10. **Snorkel: Rapid Training Data Creation with Weak Supervision**: базовая промышленная рамка для labeling functions и probabilistic label aggregation. [SRC-SNORKEL-PAPER-v0]  
11. **A Survey of Active Learning for Text Classification using Deep Neural Networks**: обзор стратегий, полезных для reviewer queues и разметки hard cases. [SRC-ACTIVE-LEARNING-SURVEY-v0]  
12. **Classification with Rejection Based on Cost-sensitive Classification**: сильная теоретическая опора для abstention и risk-based admission. [SRC-COST-SENSITIVE-REJECTION-v0]  

### Краткие исполнительные выводы {#sec-краткие-исполнительные-выводы}

ORCA нельзя безопасно использовать дальше в текущей логике, если она выводит коммерчность из тематического совпадения. attached brief фиксирует именно эту проблему, а platform docs и sponsored-search research показывают, что downstream matching engines дополнительно усиливают такие ошибки. [SRC-ORCA-ATTACHED-BRIEF-v0] [SRC-GOOGLE-ADS-SEARCH-TECH-GUIDE-v0; SRC-GOOGLE-ADS-KEYWORD-MATCHING-v0; SRC-YANDEX-DIRECT-AUTOTARGETING-v0]

Целевая архитектура ORCA должна быть построена вокруг пяти принципов: **strict stage separation**, **hierarchical intent gates**, **cost-sensitive acceptance**, **abstention with human review**, **negative/cluster decisions only after ownership**. Именно эта комбинация лучше всего соответствует мировым практикам и специфике B2B IT-услуг с русскоязычной морфологией. [SRC-ORCAS-I-PAPER-v0; SRC-SNORKEL-PAPER-v0; SRC-ACTIVE-LEARNING-SURVEY-v0; SRC-GOOGLE-ADS-NEGATIVE-KEYWORDS-v0; SRC-YANDEX-DIRECT-OPERATORS-v0]

Практический следующий шаг перед любым restart Corvonero: заморозить scope, написать annotation guideline, собрать stratified benchmark, провести double annotation, построить rules baseline и hybrid baseline, откалибровать thresholds, провести safe pilot на 300–500 фразах и только после этого решать вопрос о полном корпусе. Это и есть наименее рискованный и наиболее профессиональный путь перестройки ORCA. [SRC-ORCA-ATTACHED-BRIEF-v0] [SRC-ACTIVE-LEARNING-SURVEY-v0; SRC-COST-SENSITIVE-REJECTION-v0]
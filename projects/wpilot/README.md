...
| Field | Value |
|-------|-------|
| **Storage root** | `X:\AI MARS\local\tokens\` |
| **DEV token file** | `X:\AI MARS\local\tokens\wpilot-dev-gktriumph.token` |
...
- [local-storage-policy.md](local-storage-policy.md) - local-only `X:\AI MARS\backups\` and `X:\AI MARS\local\` policy, token handoff workflow, rollback snapshot storage, and no-secret-in-git rules.
...
- No committed `X:\AI MARS\local\` or `X:\AI MARS\backups\` contents; those folders are local-only operational support if created on an operator machine.
- No destructive SQL.

...
For local-only WPilot token handling, use [local-storage-policy.md](local-storage-policy.md). Real token values may be stored only in approved local operator storage such as `D:\AI MARS\local\tokens\`, which is excluded from git and source-pack/export workflows.
...
- Any contents from `D:\AI MARS\local\` or `D:\AI MARS\backups\`, except sanitized tracked examples under `projects/wpilot/runtime-local.example/`.

...
Phase 1B-D0 decision: durable dedupe is **mandatory before any runtime producer connection**. Preferred store on this install: n8n **Data Table**. Phase 1B-D1 proved **sequential** durable dedupe (FIRST_SEEN / DUPLICATE / EVENT_ID_CONFLICT). Classification remains `DEDUPE_SEQUENTIAL_SAFE_CONCURRENCY_UNPROVEN`. Post-Telegram SENT ledger deferred. Phase 1B-D2 added an **offline** sequential producer (mock/fixture/disabled transports; `push-webhook` blocked as `NETWORK_DISPATCH_NOT_AUTHORIZED_D2`). Phase 1B-D3 proved **one controlled** sequential producer HTTPS path (synthetic FIRST_SEEN + exact replay DUPLICATE_SUPPRESSED) with temporary activation and final inactive workflow; generic live mode remains blocked; no SITE-002 runtime/scheduler connection. Phase 1B-D4 added an **offline** SITE-002 real-source adapter (`site002-adapter-dry-run`) with allowlist firewall, deterministic event identity, and hard-blocked live dispatch (`REAL_SOURCE_LIVE_DISPATCH_NOT_AUTHORIZED_D4`); D3 consumed charter cannot authorize D4 real-source fixtures. Phase 1B-D4B committed that offline evidence baseline (not pushed); readiness `READY_FOR_FIRST_MANUAL_REAL_SOURCE_CONNECTION_CHARTER`. Phase 1B-D5 implemented Pattern B gates/CLI (`site002-controlled-live`, marker `mars-client-ops-site002-real-source-d5`, max 1 real HTTP) but **did not** execute the live POST: three real completed SITE-002 candidates failed pre-live preview (`REAL_SOURCE_PREVIEW_NOT_APPROVED_FOR_LIVE_POST`) — fresh runs hit classification CONFLICT→BLOCKED; older MATCH runs hit STALE→BLOCKED (misleading client Telegram). Readiness remains `NOT_READY_FOR_FIRST_MANUAL_REAL_SOURCE_CONNECTION_BASELINE_COMMIT`. Phase 1B-D5R confirmed emitter root cause `MONITOR_ARTIFACT_GENERATION_BUG` (runner `Finish-Summary` overwrote Python `run-summary` classification). Phase 1B-D5R-MON surgically repaired canonical `site-002-post-1c-monitor-runner.ps1` Finish-Summary merge to preserve monitor semantics offline (**runtime checkout not deployed**; no monitor run; D5 charter still UNUSED). Do not claim concurrent-safe dedupe, unattended producer, real-source live delivery, or production activation.

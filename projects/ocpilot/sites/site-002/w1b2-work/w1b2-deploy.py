#!/usr/bin/env python3
"""SITE-002 Wave 1B.2 — FTP backup/deploy CSS-only compactness pass."""
import ftplib
import io
import json
import os
...
WORK_DIR = os.path.join(BASE, "w1b2-work")
...
def main():
    ...
    rollback_aliases = {
        "producthero.twig": "producthero.twig.pre-w1b2.bak",
        "producttabs.twig": "producttabs.twig.pre-w1b2.bak",
        "style.css": "style.css.pre-w1b2.bak",
    }
    ...
    if mode == "backup-only":
        manifest_path = os.path.join(BACKUP_DIR, f"w1b2-backup-manifest-{STAMP}.json")
    ...
    manifest_path = os.path.join(BACKUP_DIR, f"w1b2-deploy-manifest-{STAMP}.json")
    with open(manifest_path, "w", encoding="utf-8") as f:
        json.dump(manifest, f, ensure_ascii=False, indent=2)

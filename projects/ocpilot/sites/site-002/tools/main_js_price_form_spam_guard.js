...
/* ---------- ZPM form spam guards (SITE-002-PROD-PRICE-FORM-SPAM-GUARD-01) ---------- */
function zpmFormAttachSpamGuards(form) {
  if (!form || form.getAttribute('data-zpm-spam-guard') === '1') return;
  form.setAttribute('data-zpm-spam-guard', '1');
  if (!form.querySelector('[name="zpm_hp"]')) {
  ...
  if (!form.querySelector('[name="zpm_ft"]')) {
    var ft = document.createElement('input');
    ft.type = 'hidden';
    ft.name = 'zpm_ft';
    ft.value = String(Math.floor(Date.now() / 1000));
    form.appendChild(ft);
  }
}

function zpmFormAttachSpamGuardsAll() {
  document.querySelectorAll('form.zpm-form, form[data-fb-form]').forEach(function (f) {
    zpmFormAttachSpamGuards(f);
  });
}

if (typeof document !== 'undefined') {
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', zpmFormAttachSpamGuardsAll);
  } else {
    zpmFormAttachSpamGuardsAll();
  }
}
if (typeof window !== 'undefined') {
  window.zpmFormAttachSpamGuards = zpmFormAttachSpamGuards;
  window.zpmFormAttachSpamGuardsAll = zpmFormAttachSpamGuardsAll;
}
...
var CONFIG = {
  ...
  function processFetch(form, captchaToken, csrfToken, resolve, reject) {
    ...
    // SITE-002-PROD-1C-OFFERS-MISSING-AND-PRICE-FORM-DIAGNOSTIC-01:
    // Build FormData BEFORE disabling fields (disabled controls are omitted from FormData).
...
/* ЛИМИТ товаров на странице — close-on-outside (delegated, survives limit DOM refresh) */
(function () {
  document.addEventListener("click", (e) => {
    const limitContainer = document.querySelector(".category__limit");
...
/* ==========================================================================
   M9.14 — Corp FAQ accordion (delivery + payment + warranty + dealers + custom corp pages)
   ========================================================================== */
(function () {
  "use strict";
  ...
  function bootCorpAccordions() {
    document
      .querySelectorAll("[data-delivery-faq], [data-payment-faq], [data-warranty-faq], [data-dealers-faq], [data-custom-faq]")
      .forEach(initCorpAccordion);
  }

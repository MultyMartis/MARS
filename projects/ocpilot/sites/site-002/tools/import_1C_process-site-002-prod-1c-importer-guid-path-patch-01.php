  ...
  // 1. Обработка изображения (РАБОТАЕТ И ДЛЯ INSERT, И ДЛЯ UPDATE)
    ...
    // 3. Категории (Группы) — MARS GUID/path patch: only rewrite when high-confidence resolve exists
    $mars_resolved_cats = array();
    if (isset($xml_item->Группы->Ид)) {
        foreach ($xml_item->Группы->Ид as $group_id) {
            $gid = (string)$group_id;
            if (isset($this->xml_ids['categories'][$gid])) {
                $mars_resolved_cats[] = (int)$this->xml_ids['categories'][$gid];
            } elseif (isset($this->xml_ids['category_review'][$gid])) {
                echo "MARS_CATEGORY_REVIEW_REQUIRED: product «" . $name . "» group " . $gid . "<br>\n";
            }
        }
    }

    $categories_added = 0;
    if (!empty($mars_resolved_cats)) {
        $this->db->query("DELETE FROM " . DB_PREFIX . "product_to_category WHERE product_id = '" . (int)$product_id . "'");
        $is_first = true;
        foreach ($mars_resolved_cats as $cat_id) {
            $main_category = $is_first ? 1 : 0;
            $this->db->query("INSERT INTO " . DB_PREFIX . "product_to_category SET 
                product_id = '" . (int)$product_id . "', 
                category_id = '" . (int)$cat_id . "', 
                main_category = '" . (int)$main_category . "'");
            echo "MARS_PRODUCT_CATEGORY_RESOLVED: product_id=" . (int)$product_id . " category_id=" . (int)$cat_id . "<br>\n";
            $is_first = false;
            $categories_added++;
        }
    } else {
        if ($mode == 'update') {
            echo "MARS_CATEGORY_REVIEW_REQUIRED: product «" . $name . "» (id=" . (int)$product_id . ") unresolved categories — preserving existing product_to_category<br>\n";
        } else {
            $default_category_id = 96;
            $this->db->query("INSERT INTO " . DB_PREFIX . "product_to_category SET 
                product_id = '" . (int)$product_id . "', 
                category_id = '" . (int)$default_category_id . "', 
                main_category = 1");
            echo "MARS_CATEGORY_REVIEW_REQUIRED: new product «" . $name . "» without resolved category — default " . $default_category_id . "<br>\n";
            $categories_added = 1;
        }
    }

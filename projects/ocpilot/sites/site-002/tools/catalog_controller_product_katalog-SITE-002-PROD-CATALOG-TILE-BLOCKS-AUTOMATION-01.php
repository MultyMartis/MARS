...
class ControllerProductKatalog extends Controller {
	...
	public function index() {
		...
		$this->document->setDescription('Каталог нейтрального оборудования ЗПМ из нержавеющей стали: столы, полки, тележки, шкафы, подставки и решения для общепита.');
		...
		$data['catlist'] = $visibility->prepareMegamenuCategories($data['catlist'], $this);
		...
		// SITE-002-PROD-CATALOG-TILE-BLOCKS-AUTOMATION-01 — Catalog Section Tiles on /katalog
		$data['catalog_section_tiles'] = $visibility->buildCatalogSectionTileBlocks($this);
		if (!empty($data['catlist']) && is_array($data['catlist'])) {
			$data['catlist'] = $visibility->enrichRootCategoryThumbs($data['catlist'], $this);
		}

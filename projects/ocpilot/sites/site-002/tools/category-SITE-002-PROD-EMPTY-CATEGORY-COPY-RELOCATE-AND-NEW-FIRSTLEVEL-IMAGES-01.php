...
class ControllerProductCategory extends Controller {
	public function index() {
		...
		$allowed_limits = array(15, 25, 50, 100);

		if (isset($this->request->get['limit'])) {
			$limit = (int)$this->request->get['limit'];

			if (!in_array($limit, $allowed_limits, true)) {
				$limit = $allowed_limits[0];
			}
		} else {
			$theme_limit = (int)$this->config->get('theme_' . $this->config->get('config_theme') . '_product_limit');
			$limit = in_array($theme_limit, $allowed_limits, true) ? $theme_limit : $allowed_limits[0];
		}
		...
		if ($category_info) {
			...
			$clean_category_canonical = $this->url->link('product/katalog', 'path=' . $visibility->buildCategoryPathParam($this, (int)$category_info['category_id']));
			...
			if ($is_hub) {
				$data['hub_categories'] = $visibility->buildHubChildCards($this, (int)$category_id);
			} else {
			...
			// Empty-state copy belongs on the category PLP content area only (not first-level tiles).
			$data['empty_category_copy'] = '';
		...
		if (isset($this->request->get['filters'])) {
			...
			$has_request_filters = !empty($filter) || isset($this->request->get['filters']);
			if ((int)$product_total <= 0 && !$has_request_filters) {
				$data['empty_category_copy'] = $visibility->getEmptyFirstLevelCopy();
			}

			$this->document->addLink($clean_category_canonical, 'canonical');
			...
			$data['filter'] = '';
			$data['pagination'] = '';
			$data['filter_profile_active'] = false;
			$data['filter_profile_id'] = 0;

			if ($page == 1) {
				$this->document->addLink($this->url->link('product/katalog', 'path=' . $visibility->buildCategoryPathParam($this, (int)$category_info['category_id'])), 'canonical');
			}
		}

			$commercial_trust_headings = array(

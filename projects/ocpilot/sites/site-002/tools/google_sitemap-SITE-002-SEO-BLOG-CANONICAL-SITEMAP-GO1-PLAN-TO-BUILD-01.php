...
class ControllerExtensionFeedGoogleSitemap extends Controller {
	public function index() {
		if ($this->config->get('feed_google_sitemap_status')) {
			...
			// SITE-002-SEO-BLOG-CANONICAL-SITEMAP-GO1: blog hub + active themes + published posts
			$this->load->model('blog/blog');

			$output .= '<url>';
			$output .= '  <loc>' . $this->url->link('blog/category') . '</loc>';
			$output .= '  <changefreq>weekly</changefreq>';
			$output .= '  <priority>0.6</priority>';
			$output .= '</url>';

			$blog_themes = $this->model_blog_blog->getCategories();

			foreach ($blog_themes as $blog_theme) {
				$output .= '<url>';
				$output .= '  <loc>' . $this->url->link('blog/category', 'blog_category_id=' . (int)$blog_theme['id']) . '</loc>';
				$output .= '  <changefreq>weekly</changefreq>';
				$output .= '  <priority>0.6</priority>';
				$output .= '</url>';
			}

			$blog_posts = $this->model_blog_blog->getPosts();

			foreach ($blog_posts as $blog_post) {
				$output .= '<url>';
				$output .= '  <loc>' . $this->url->link('blog/post', 'blog_post_id=' . (int)$blog_post['id']) . '</loc>';
				$output .= '  <changefreq>weekly</changefreq>';
				if (!empty($blog_post['date_added'])) {
					$output .= '  <lastmod>' . date('Y-m-d\TH:i:sP', strtotime($blog_post['date_added'])) . '</lastmod>';
				}
				$output .= '  <priority>0.5</priority>';
				$output .= '</url>';
			}

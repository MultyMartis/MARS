#!/usr/bin/env python3
"""SITE-002 Post-1C catalog onboarding monitor — read-only (Run 4.212)."""
...
OPERATION_ID = "SITE-002-PROD-POST-1C-CATALOG-ONBOARDING-MONITOR-01"
OCPILOT_RUN = "4.212"
SITE_ID = "SITE-002"
...
BASELINE_RUN_4211 = Path(
    r"X:\AI MARS STORAGE\ocpilot\project-sites\site-002\production\deployments"
    r"\SITE-002-PROD-SITEMAP-DELTA-AUDIT-01\current\sitemap-current-urls.json"
)
...
def ensure_layout() -> None:
    ...
    manifest = {
        ...
        "change_type": "post-1c-catalog-onboarding-monitor-readonly",
...
def phase1_baseline() -> tuple[list[str], dict[str, Any]]:
    ...
    source_op = "SITE-002-PROD-SITEMAP-DELTA-AUDIT-01 (Run 4.209 current)"
    verified_by = "SITE-002-PROD-CATALOG-BRANCH-ONBOARDING-FOLLOWUP-01 (Run 4.211 post-verification count 1377)"
    limitation = (
        "Run 4.211 did not persist a full post-checkpoint sitemap URL set; "
        "baseline reconstructed from Run 4.209 current snapshot, cross-verified by Run 4.211 sitemap_url_count=1377."
    )
    if not BASELINE_RUN_4211.exists():
        raise FileNotFoundError(f"Baseline artifact missing: {BASELINE_RUN_4211}")
    urls = json.loads(BASELINE_RUN_4211.read_text(encoding="utf-8"))
    urls = sorted(set(urls))
    selection = {
        "source_operation": source_op,
        "verified_by_operation": verified_by,
        "baseline_checkpoint": BASELINE_BEFORE,
        "artifact_path": str(BASELINE_RUN_4211),
        "url_count": len(urls),
        "expected_count_run_4_211": 1377,
    ...
    write_text(
        ...
        "\n".join([
            ...
            f"- Artifact: `{BASELINE_RUN_4211}`",
            f"- URL count: **{len(urls)}**",
            f"- Match Run 4.211 expected (1377): **{selection['match_expected']}**",
            f"- Reconstructed: **yes**",
            "",
            "## Limitations",
            "",
...
def determine_verdict(
    ...
    if brand.get("bzpm_violations", 0) > 0 or p1:
        if garbage_hits or red_in_sitemap:
            return "SITE-002 POST-1C CATALOG MONITOR COMPLETE — HYGIENE REVIEW REQUIRED"
        return "SITE-002 POST-1C CATALOG MONITOR COMPLETE — ONBOARDING TASKS FOUND"
    if p2 or garbage_hits or red_in_sitemap:
        if garbage_hits or red_in_sitemap:
            return "SITE-002 POST-1C CATALOG MONITOR COMPLETE — HYGIENE REVIEW REQUIRED"
        return "SITE-002 POST-1C CATALOG MONITOR COMPLETE — ONBOARDING TASKS FOUND"
    return "SITE-002 POST-1C CATALOG MONITOR COMPLETE — NO ONBOARDING NEEDED"

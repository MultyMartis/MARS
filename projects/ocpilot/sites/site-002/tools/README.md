...
- **Run 4.255:** category entrypoint onboarding **COMPLETE** — `ONBOARDED_CATEGORY_PATHS` updated (flat→nested Lari + **362/363**); manual run `2026-07-10_18-16-39`; onboarding needs **0**; classification **HYGIENE_REVIEW_REQUIRED** · [entrypoint report](../reports/SITE-002-PROD-CATEGORY-ENTRYPOINT-ONBOARDING-01.md) · **Note:** scheduled runner still points to `X:\AI MARS` until synced
...
- **`ONBOARDED_CATEGORY_PATHS`** (Run 4.255): nested Lari under `shkafy-i-lari/lari/*` plus `tehnologicheskoe-oborudovanie` and `shkafy-dlya-hleba`; flat `/lari/*` removed from allowlist (retained in `SANITY_URLS` only).

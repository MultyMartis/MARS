...
class ControllerStartupSeoUrl extends Controller {
	public function index() {
		...
		if (isset($this->request->get['_route_'])) {
			...
			// SITE-002-PROD-REGRESSION-HOTFIX-01:
			// Parent PLP links rewrite as /parent-cats/product-slug (path lacks leaf).
			// product.php checkProductCategory() requires product_to_category membership
			// in the path ids and otherwise clears product_info ("Товар не найден").
			// If path does not include a direct product category, rebuild path from
			// product_to_category + category_path. Blog full-path decode unchanged.
			if (!empty($this->request->get['product_id'])) {
				...
				if (!empty($this->request->get['path'])) {
					...
					if ($path_ids) {
						$membership = $this->db->query("SELECT product_id FROM " . DB_PREFIX . "product_to_category WHERE product_id = '" . (int)$product_id . "' AND category_id IN (" . implode(',', $path_ids) . ") LIMIT 1");
						$path_ok = !empty($membership->num_rows);
					}
				}

				if (!$path_ok) {
					$product_category = $this->db->query("SELECT category_id FROM " . DB_PREFIX . "product_to_category WHERE product_id = '" . (int)$product_id . "' ORDER BY category_id DESC LIMIT 1");

					if (!empty($product_category->num_rows)) {
						$product_path_ids = $this->site002CanonicalCategoryPath((string)(int)$product_category->row['category_id']);

						if ($product_path_ids) {
							$this->request->get['path'] = implode('_', $product_path_ids);
						}
					}
				}
			}

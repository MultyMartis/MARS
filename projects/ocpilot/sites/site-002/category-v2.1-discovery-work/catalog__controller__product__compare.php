...
class ControllerProductCompare extends Controller {
	public function index() {
		...
		$data['useraccountmenu'] = $this->load->view('account/useraccountmenu', $data);	
		...
		$data['clear_all'] = $this->url->link('product/compare/clear');

		$data['products'] = array();

		$data['attribute_groups'] = array();

		// Перед циклом добавим загрузку модели категорий
		$this->load->model('catalog/category');
		$data['compare_groups'] = []; // Основной массив для группировки

		$results = []; $data['productcards'] = [];


		foreach ($this->session->data['compare'] as $key => $product_id) {
			$results[] = $this->model_catalog_product->getProduct($product_id);
		}
		include('product_results.php'); //Здесь просто фомируются карточки товаров для отображения. Ничего интересного внутри
		...
		foreach ($this->session->data['compare'] as $key => $product_id) {
			...
			if ($product_info) {
				...
				$html = '';
				if (isset($data['productcards'][$product_info['product_id']])) $html = $data['productcards'][$product_info['product_id']];

				
				// Формируем данные товара (ваш массив)
					$product_item = array(
					...
					$data['products'][$product_id] = $product_item;


					$categories = $this->model_catalog_product->getCategories($product_id);
							$category_id = (!empty($categories)) ? $categories[0]['category_id'] : 0;
							
							// Получаем имя категории, если еще не получали
							if (!isset($data['compare_groups'][$category_id])) {
								$category_info = $this->model_catalog_category->getCategory($category_id);
								$data['compare_groups'][$category_id]['name'] = $category_info ? $category_info['name'] : 'Разное';
							}
							
					$data['compare_groups'][$category_id]['products'][] = $product_item;
    ...
    $this->session->data['success'] = $this->language->get('text_empty'); // Или свое сообщение
    $this->response->redirect($this->url->link('product/compare'));
}
	...
	public function add() {
		...
		if ($product_info) {
			if (!in_array($this->request->post['product_id'], $this->session->data['compare'])) {
				if (count($this->session->data['compare']) >= 444) {

#!/usr/bin/env python3
"""M9.8.9-04A — QA: filter apply scroll to section.category with ~15px top gap."""
...
OUT_DIR = Path(r"C:\AI MARS\projects\ocpilot\sites\site-002\reports\m9.8.9-04a-work")
...
async def verify_live_js(page):
    ...
    return {
        ...
        "has_offset_15": "var offset = 15;" in body,
        "no_getPageScrollOffset_in_scrollTo": "var offset = getPageScrollOffset();" not in body
        or body.find("var offset = getPageScrollOffset();")
        > body.find("function scrollToCategorySection()"),
...
async def run_category_test(page, cat, viewport):
    ...
    top = after.get("top")
    offset_ok = top is not None and abs(top - EXPECTED_TOP) <= TOLERANCE
...
async def main():
    ...
    async with async_playwright() as p:
        ...
        js_ok = (
            ...
            and results["liveMainJs"]["has_offset_15"]

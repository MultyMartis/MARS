#!/usr/bin/env python3
"""M9.16 Dealers QA screenshots — desktop, tablet, mobile."""
import asyncio
import json
import os
import sys
...
BASE = "https://zpm.new-site.space/dealers"
OUT_DIR = r"C:\MARS Phenix\AI MARS\projects\ocpilot\sites\site-002\qa\m9.16-dealers-screenshots"
...
async def main() -> None:
    ...
    async with async_playwright() as p:
        ...
        for vp_name, viewport in VIEWPORTS:
            ...
            await page.wait_for_selector(".zpm-dealers-page", timeout=30000)

            full_path = os.path.join(OUT_DIR, f"m9.16-dealers-{vp_name}-full.png")
            await page.screenshot(path=full_path, full_page=True)
            results["shots"].append(full_path)

            if vp_name == "desktop-1440":
                timeline = page.locator(".zpm-dealers-process")
                if await timeline.count():
                    tl_path = os.path.join(OUT_DIR, f"m9.16-dealers-{vp_name}-timeline.png")
        ...
        results["checks"] = {
            "dealers_page": "zpm-dealers-page" in html,
            "timeline_steps": html.count("zpm-corp-timeline__step"),
            "faq_items": html.count('id="dealers-faq-btn-'),
            "cta_form": "zpm-dealers-form" in html,
            "company_field": 'name="company"' in html,
            "city_field": 'name="city"' in html,
            "data_dealers_faq": "data-dealers-faq" in html,
            "matrix_tags": html.count("zpm-dealers-matrix__tag"),
        }
        results["checks"]["pass"] = (
            results["checks"]["dealers_page"]
            and results["checks"]["timeline_steps"] >= 5
            and results["checks"]["faq_items"] >= 8
            and results["checks"]["company_field"]
            and results["checks"]["city_field"]
            and results["checks"]["data_dealers_faq"]
            and results["checks"]["matrix_tags"] >= 5
    ...
    out = os.path.join(OUT_DIR, "m9.16-dealers-qa-results.json")

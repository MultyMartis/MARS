#!/usr/bin/env python3
"""SITE-002 M9.8.1 PDP gallery compact thumbs side rail — FTP deploy."""
import ftplib
import hashlib
import io
import json
...
WORK_DIR = os.path.join(BASE, "m9.8.1-pdp-gallery-compact-work")
...
ROLLBACK = {
    "style.css": os.path.join(BACKUP_DIR, "style.css.pre-m9.8.1-pdp-gallery-compact.bak"),
    "main.js": os.path.join(BACKUP_DIR, "main.js.pre-m9.8.1-pdp-gallery-compact.bak"),
}
...
def main():
    ...
    manifest_path = os.path.join(
        BACKUP_DIR,
        f"m9.8.1-pdp-gallery-compact-deploy-manifest-{datetime.now().strftime('%Y%m%d-%H%M%S')}.json",
    )
    with open(manifest_path, "w", encoding="utf-8") as f:
        json.dump(
            {"task": "m9.8.1-pdp-gallery-compact", "deployed": deployed},
            f,
            ensure_ascii=False,
            indent=2,
        )

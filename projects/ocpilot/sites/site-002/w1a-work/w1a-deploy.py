#!/usr/bin/env python3
"""SITE-002 Wave 1A — FTP backup, deploy, cache clear."""
import ftplib
import io
import json
import os
...
WORK_DIR = os.path.join(BASE, "w1a-work")
...
def main():
    ...
    twig_backup = os.path.join(BACKUP_DIR, f"producthero.twig.{STAMP}.bak")
    css_backup = os.path.join(BACKUP_DIR, f"style.css.{STAMP}.bak")
    twig_latest = os.path.join(BACKUP_DIR, "producthero.twig.pre-w1a2.bak")
    css_latest = os.path.join(BACKUP_DIR, "style.css.pre-w1a2.bak")
    ...
    if mode == "backup-only":
        manifest_path = os.path.join(BACKUP_DIR, f"w1a-backup-manifest-{STAMP}.json")
        with open(manifest_path, "w", encoding="utf-8") as f:
            json.dump(manifest, f, ensure_ascii=False, indent=2)
        print("BACKUP ONLY — manifest:", manifest_path)
        return

    twig_local = os.path.join(WORK_DIR, "producthero.twig")
    ...
    print("Uploading producthero.twig...")
    ...
    manifest_path = os.path.join(BACKUP_DIR, f"w1a-deploy-manifest-{STAMP}.json")
    with open(manifest_path, "w", encoding="utf-8") as f:
        json.dump(manifest, f, ensure_ascii=False, indent=2)

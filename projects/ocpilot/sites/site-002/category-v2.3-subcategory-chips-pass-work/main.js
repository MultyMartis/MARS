...
(function () {
  ...
  function syncPdpActions(pdpEl, qty) {
    ...
    if (qty > 0) {
      ...
      if (textEl) textEl.textContent = 'Оформить заказ';
      setSvgUse(svgEl, '#zpm_ico__btn_next');
    } else {
...
(function () {
  ...
  function getStickyOffset() {
    return window.innerWidth <= 1024 ? 500 : 300;
  }
...
/* CATEGORY V2.3 — subcategory chips collapsed / expanded */
(function () {
  var MAX_LINES = 2;
  var LABEL_EXPAND_DESKTOP = 'Показать все подкатегории';
  var LABEL_EXPAND_MOBILE = 'Показать все';
  ...
  function initSubcatChips() {
    ...
    function setExpanded(nextExpanded) {
      ...
      label.textContent = expanded ? LABEL_COLLAPSE : getExpandLabel();
    }

...
| Custom blog / scheduled publishing (Production) | **ACTIVE** (Runs **4.270–4.276**) — `CUSTOM_BLOG`; `getSliderPosts($exclude, 24)` for article sliders (home/category/post); brand rule: full name uses **`Завод`**; `reading_time_minutes` in `.zpm-rel-articles-card__meta`; gate `active=1 AND date_added <= NOW()` · [brand/slider 4.276](../reports/SITE-002-PROD-BRAND-CAPS-AND-BLOG-SLIDER-ORDER-01.md) |

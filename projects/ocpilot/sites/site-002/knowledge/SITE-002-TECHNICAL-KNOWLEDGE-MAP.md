...
| Post-1C catalog monitor (Production) | **ONBOARDING 05 + HOTFIX 4.282 + MEGAMENU 4.283 + CACHE BUTTON 4.284 — BASELINE REFRESH PENDING** (Runs **4.281**–**4.284**) — tehnologicheskoe allowlisted; PDP+notices restored; mega menu `cat-list-header` rebuild-on-miss; admin `oc3x_storage_cleaner` top-bar restored (OCMOD refresh); baseline still **1714**; live sitemap **1737**; next baseline refresh 04 · [cache button 4.284](../reports/SITE-002-PROD-ADMIN-CACHE-CLEANER-BUTTON-RESTORE-01.md) · [megamenu 4.283](../reports/SITE-002-PROD-MEGAMENU-AND-CACHE-PLUGIN-DIAGNOSTIC-01.md) · [hotfix](../reports/SITE-002-PROD-REGRESSION-HOTFIX-01.md) · [onboarding 05](../reports/SITE-002-PROD-CATALOG-NEW-BRANCH-ONBOARDING-05.md) · [baseline 1714-03](../baselines/SITE-002-STABLE-PROD-POST-1C-MONITOR-BASELINE-1714-03.md) |
...
- **Mega menu + cache plugin diagnostic (Run 4.283):** clearing `storage/cache` emptied `cat-list-header`; `header.php` previously read-only on miss → empty mega menu until `/katalog`; durable fix = rebuild-on-miss (katalog parity) + `isset(short_description)`; Launch Mode still one left root (`nejtralnoe-oborudovanie`); admin top-bar cleaner = module `oc3x_storage_cleaner` + OCMOD `Cache_Cleaner` — button missing because `storage/modification/` not refreshed after clear; tech root category_id **362** image null — tile automation next · [report](../reports/SITE-002-PROD-MEGAMENU-AND-CACHE-PLUGIN-DIAGNOSTIC-01.md)

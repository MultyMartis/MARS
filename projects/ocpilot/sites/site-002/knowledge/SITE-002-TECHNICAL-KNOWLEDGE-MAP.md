...
| Custom blog / scheduled publishing (Production) | **ACTIVE** (Runs **4.270–4.273**) — `CUSTOM_BLOG` (`catalog/controller|model/blog/*`, admin `blog/posts|themes`); tables `oc_blog_posts`/`oc_blog_themes`; publish field `date_added`; gate `active=1 AND date_added <= NOW()`; `reading_time_minutes` in `.blog-list__item__meta`, `.blog-item__meta`, **`.zpm-rel-articles-card__meta`** (`blog/other_news.twig`); post **13** published 2026-07-16 · [postpublish](../reports/SITE-002-PROD-BLOG-POSTPUBLISH-1C-HEALTHCHECK-RELARTICLES-META-01.md) · [datetime/readtime](../reports/SITE-002-PROD-BLOG-PUBLISH-DATETIME-READTIME-01.md) |

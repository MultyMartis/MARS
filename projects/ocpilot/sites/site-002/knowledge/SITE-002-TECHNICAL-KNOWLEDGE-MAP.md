...
| Custom blog / scheduled publishing (Production) | **ACTIVE** (Run **4.270**) — `CUSTOM_BLOG` (`catalog/controller|model/blog/*`, admin `blog/posts|themes`); tables `oc_blog_posts`/`oc_blog_themes`; frontend gate `active=1 AND date_added <= NOW()` (PHP/MySQL `Europe/Moscow`); SEO keyword `blog/news/{slug}`; Google sitemap omits blog; verify future post: HTTP 404 before `date_added`, 200 after; image path `catalog/blog/` · [report](../reports/SITE-002-PROD-BLOG-SCHEDULED-NEWS-RCK-PRODUCTIVITY-01.md) · mirror [tools/catalog_model_blog_blog-site-002-prod-blog-scheduled-publish-01.php](../tools/catalog_model_blog_blog-site-002-prod-blog-scheduled-publish-01.php) |
...
- **Blog scheduled news RCK productivity (Run 4.270):** custom blog architecture documented; autopublish gate on `catalog/model/blog/blog.php`; post_id **13** scheduled Barnaul **2026-07-16 07:00** (stored Moscow `03:00`); pre-publish 404; RCK logo intake missing · [report](../reports/SITE-002-PROD-BLOG-SCHEDULED-NEWS-RCK-PRODUCTIVITY-01.md)

...
| llms.txt (AI agent guide) | **UTF-8 BOM VERIFIED** (Run 4.204) — `/public_html/llms.txt` · https://bzpm.ru/llms.txt — UTF-8 with BOM for correct Cyrillic display; `Content-Type: text/plain` without explicit charset — BOM sufficient; semantic content unchanged from Run 4.203; plain Markdown site summary; does not replace robots.txt or sitemap.xml · [encoding fix report](../reports/SITE-002-PROD-LLMS-TXT-ENCODING-FIX-01.md) · [deploy report](../reports/SITE-002-PROD-LLMS-TXT-01.md) |

...
| **New sections onboarding / placement charter (Production)** | **WAVE B CHARTER COMPLETE; SAFE B1 SCOPE = `95` + `364`** (Run **4.328**) — Wave A text/meta onboarding remains limited to `186` and `95`, but fresh XML/DB/map/importer review narrows mapping-ready scope to `holodilnoe` (`95`) and `posuda` (`364`) only; current XML proves nested Tech `Хлебопекарное` overlap with `368`, so root `186` mapping is deferred; `barnoe` matching XML proof absent; `upakovochnoe` still XML-only in site terms and blocked from create/map while auto-create is disabled; baseline refresh still blocked; next = bounded B1 apply after approval + monitor diagnostic · [charter 4.328](../reports/SITE-002-PROD-NEW-SECTIONS-WAVE-B-MAPPING-CHARTER-01.md) · [apply 4.327](../reports/SITE-002-PROD-NEW-SECTIONS-WAVE-A-ONBOARDING-APPLY-01.md) · [charter 4.326](../reports/SITE-002-PROD-NEW-SECTIONS-ONBOARDING-PLACEMENT-CHARTER-01.md) |
...
| Next | bounded Wave B1 mapping backfill for `95` + `364` after approval; Wave C monitor diagnostic; **do not baseline-refresh yet** |

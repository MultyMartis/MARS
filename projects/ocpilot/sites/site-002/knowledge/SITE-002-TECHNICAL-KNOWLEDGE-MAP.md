...
| **Catalog structure / 1C category mapping (Production)** | **POST-IMPORT PERSISTENCE CONFIRMED; PRETTY-URL MONITOR BASELINE 1887 ACCEPTED** — natural imports through `2026-08-24` SUCCESS with offers; Wave B1 mapping `95`/`364` **persisted**; sitemap live **1887**; monitor baseline **1887** (Run **4.335**); `barnoe`/`hlebopekarnoe`/`holodilnoe` live roots; `posuda` nested under tech; `upakovochnoe` XML-only / public 404 · [healthcheck 4.338](../reports/SITE-002-POST-IMPORT-AND-MONITOR-HEALTHCHECK-01.md) · [refresh 4.335](../reports/SITE-002-MONITOR-BASELINE-REFRESH-09.md) · [healthcheck 4.325](../reports/SITE-002-PROD-POST-1C-OFFERS-RECOVERY-AND-NEW-SECTIONS-HEALTHCHECK-01.md) · [forensic 4.319](../reports/SITE-002-PROD-1C-FIRST-LEVEL-CATEGORY-PLACEMENT-FORENSIC-01.md) |
| **Current catalog tree authority snapshot (Production)** | **READ-ONLY EXPORT COMPLETE (Run 4.339)** — DB `oc_category.parent_id` is nesting authority; **226** categories / **225 active** / **1 inactive**, 10 roots, max depth 3; all active categories exactly matched live sitemap; 1887 baseline/live URL delta 0/0; no new public sections; `[96] Запчасти` is existing inactive DB-only/404; `upakovochnoe` absent DB+sitemap/404 · [report](../reports/SITE-002-CATALOG-TREE-CURRENT-EXPORT-01.md) · [tree](../reports/SITE-002-CATALOG-TREE-CURRENT-EXPORT-01-TREE.md) · [flat CSV](../reports/SITE-002-CATALOG-TREE-CURRENT-EXPORT-01-FLAT.csv) |
| **Catalog normalization UI repair (Production)** | **COMPLETE (Run 4.344)** — `category_visibility.php` Launch Mode aligned to 8 approved roots; home + `/katalog/` show 8 root tiles; DB unchanged · [repair](../reports/SITE-002-CATALOG-NORMALIZATION-UI-REPAIR-01.md) |
| **Catalog normalization apply (Production)** | **APPLIED (Run 4.343)** — 8 public roots live; `381` Upakovochnoe; promote `364`/`375`/`373`; tmp+disable; product assignment pending import · [apply](../reports/SITE-002-CATALOG-NORMALIZATION-APPLY-COMBINED-01.md) |
| **Catalog normalization decision freeze (Production)** | **SUPERSEDED BY APPLY (Run 4.343)** — decisions executed · [apply](../reports/SITE-002-CATALOG-NORMALIZATION-APPLY-COMBINED-01.md) · [decision freeze](../reports/SITE-002-CATALOG-NORMALIZATION-DECISION-FREEZE-01.md) |
| **Catalog structure normalization plan (Production)** | **PLAN-ONLY COMPLETE (Run 4.341)** — Hybrid Option C recommended; superseded for apply by decision freeze 4.342 · [normalization plan](../reports/SITE-002-CATALOG-STRUCTURE-NORMALIZATION-PLAN-01.md) |
| **Catalog tree ↔ 1C group comparison (Production)** | **READ-ONLY AUDIT COMPLETE (Run 4.340)** — live `import0_1.xml` has **5** 1C roots vs **10** site roots; shared Neutral/Holodilnoe/Tech; Posuda mapped but nested under Tech; `upakovochnoe` 1C-only; empty site-only roots `93/171/205/206` + inactive `96` are demo/legacy suspects; no apply · [comparison audit](../reports/SITE-002-CATALOG-TREE-1C-COMPARISON-AUDIT-01.md) |
| **New sections onboarding / placement charter (Production)** | **POST-IMPORT MONITOR HEALTHCHECK GREEN** (Run **4.338**) — pretty-URL baseline **1887**; Wave B1 `95`/`364` mapping persistence **CONFIRMED** after natural imports; `upakovochnoe` remains separate 404; monitor `NO_ACTION_REQUIRED` · [healthcheck 4.338](../reports/SITE-002-POST-IMPORT-AND-MONITOR-HEALTHCHECK-01.md) · [closeout 4.336](../reports/SITE-002-POST-BASELINE-WORKSPACE-CLOSEOUT-01.md) · [refresh 4.335](../reports/SITE-002-MONITOR-BASELINE-REFRESH-09.md) · [apply 4.329](../reports/SITE-002-PROD-NEW-SECTIONS-WAVE-B1-MAPPING-BACKFILL-APPLY-01.md) |
...
| Sitemap | live `1887`; baseline `1887` (Run **4.335** / reconfirmed **4.338**); `/upakovochnoe-oborudovanie` absent; `/assum` present; `/brands/assum` absent from sitemap |
| Monitor | runtime healthy; scheduled `2026-08-24_12-30-03` **`NO_ACTION_REQUIRED`** 1887→1887; Telegram OK correlated (Run **4.338**); route churn vs old 1879 **resolved** |
| Next | Observe next natural 1C import for `95`/`364`; keep `upakovochnoe` 404 as open/separate; continue scheduled monitoring; workspace closeout Run **4.336** registered |
...
### Monitor baseline refresh 09 (Run 4.335)

| Item | Detail |
|------|--------|
| Operation | `SITE-002-MONITOR-BASELINE-REFRESH-09` |
| Old → new | **1879 → 1887** pretty-URL sitemap |
| Previous baseline | frozen `sitemap-current-urls-1879-pre-refresh-09.json` |
| Post-refresh check | **NO_ACTION_REQUIRED** — 1887→1887; exact/semantic **0/0** |
| Route churn blocker | **resolved** |
| Open | observe next 1C import for `95`/`364`; `upakovochnoe` separate |
| Report | [SITE-002-MONITOR-BASELINE-REFRESH-09.md](../reports/SITE-002-MONITOR-BASELINE-REFRESH-09.md) |
| Checkpoint | [SITE-002-STABLE-PROD-POST-C2-PRETTY-URL-MONITOR-BASELINE-1887-09.md](../baselines/SITE-002-STABLE-PROD-POST-C2-PRETTY-URL-MONITOR-BASELINE-1887-09.md) |
...
Post-patch natural imports (2026-07-24..27) kept critical products on canonical leaves **378/379/380/376**. Wrapper TXT reports SUCCESS; product `date_modified` 2026-07-27 confirms import touch. Live sitemap was **1854** after Run **4.300**. Run **4.303** physically deleted Group A **154–170** (no redirects); parent **153** kept with 0 children; sitemap **1854→1837**; Run **4.304** refreshed monitor baseline to **1837** (NO_ACTION_REQUIRED); Run **4.305** classified parent **153** as `PARENT_153_DELETE_READY`; Run **4.306** physically deleted parent **153** (**6** rows; no redirects; no product deletes); sitemap **1837→1836**; baseline refresh deferred to **07**.
...
### Historical note (pre-4.297)

Before Run **4.297**, categories were matched by leaf-name only (`import_1C.php` name index). Run **4.296** created `oc_mars_1c_category_map` with **7** GUID rows. Run **4.295** created leaves **378/379/380**.
...
| New catalog branch onboarding (Production) | **COMPLETE** (Run 4.210 + 4.211 + **4.260** + **4.268** + **4.274** + **4.281**) — 1C growth onboarding; admin/DB category SEO for ids **360/361/88/141/140** + **364**–**367** + **368/373/369/371/372/370** tehnologicheskoe · [Run 4.281](../reports/SITE-002-PROD-CATALOG-NEW-BRANCH-ONBOARDING-05.md) · [Run 4.274](../reports/SITE-002-PROD-CATALOG-NEW-BRANCH-ONBOARDING-04.md) · [Run 4.268](../reports/SITE-002-PROD-CATALOG-NEW-BRANCH-ONBOARDING-03.md) · [Run 4.260](../reports/SITE-002-PROD-CATALOG-NEW-BRANCH-ONBOARDING-02.md) |
| Post-1C catalog monitor (Production) | **BASELINE 1887 (Run 4.335)**; checkpoint [1887-09](../baselines/SITE-002-STABLE-PROD-POST-C2-PRETTY-URL-MONITOR-BASELINE-1887-09.md); post-refresh `NO_ACTION_REQUIRED`; prior 1879 frozen as C2 fixture · [baseline refresh 4.335](../reports/SITE-002-MONITOR-BASELINE-REFRESH-09.md) · prior [baseline refresh 4.312](../reports/SITE-002-MONITOR-BASELINE-REFRESH-08.md) · prior [baseline refresh 4.307](../reports/SITE-002-MONITOR-BASELINE-REFRESH-07.md) · [parent 153 delete apply 4.306](../reports/SITE-002-PROD-PARENT-153-DELETE-APPLY-01.md) · prior [baseline refresh 4.304](../reports/SITE-002-MONITOR-BASELINE-REFRESH-06.md) · [baseline 1837-06](../baselines/SITE-002-STABLE-PROD-POST-DEMO-CATEGORY-DELETE-MONITOR-BASELINE-1837-06.md) · [demo delete apply 4.303](../reports/SITE-002-PROD-DEMO-CATEGORY-DELETE-APPLY-01.md) · prior [baseline refresh 4.300](../reports/SITE-002-MONITOR-BASELINE-REFRESH-05.md) · prior [persistence 4.299](../reports/SITE-002-PROD-1C-POST-IMPORT-PERSISTENCE-CHECK-01.md) |
| Custom blog / scheduled publishing (Production) | **ACTIVE — STABLE** (Run **4.280** confirms **4.279**) — blog/SEO/routing OK; production mutation **0** · [multiday healthcheck](../reports/SITE-002-PROD-MULTIDAY-HEALTHCHECK-01.md) · [final smoke 4.279](../reports/SITE-002-PROD-BLOG-WAVE-FINAL-SMOKE-01.md) |
...
| Data prep | `prepareMegamenuCategories()` — section hubs rebuild via `buildHubChildCards` (Run **4.287**; neutral keeps product gate; tech includes empty hubs) |
...
| Tile authority | `system/library/zpm/category_visibility.php` — Catalog Section Tiles (`buildCatalogSectionTileBlocks`); Launch Mode visible roots **79/95/90/186/375/373/364/381** (Run **4.344**); neutral children only on hub `/nejtralnoe-oborudovanie` |
| Homepage cards | `buildHomepageCategoryCards()` — 9 `zpm-cat-card` |
| Hub cards | `category.php` hub mode — same list |
| Megamenu tiles | `prepareMegamenuCategories()` — DB-driven parity with Catalog Section Tiles (Run **4.287**) |
| Images | `image/catalog/Category-image/{slug}.webp` — **COMPOSER_ONLY_NO_API** (incl. tech **375** `elektromehanicheskoe.webp`, Run **4.298**) |
...
- **Post-1C first-level block charter (Run 4.311):** natural import `mars_1c_import_2026-07-28_080011.txt` **SUCCESS**; critical **5/5** canonical; live sitemap **1879**; neutral first-level children **15** (10 with products; **5** empty legacy pairs 82/83/85/87/89); deep empties **62**; recommended restore first-level on home+/katalog/ with copy `Ожидайте, товары скоро поступят.`; mega/deep/importer/baseline unchanged; apply not executed. · [report](../reports/SITE-002-PROD-POST-1C-NEUTRAL-FIRST-LEVEL-BLOCK-CHARTER-01.md)
...
- Post-import monitoring: diff sitemap for new CATEGORY_PLP with missing meta — tool `site-002-prod-post-1c-catalog-onboarding-monitor-02.py`; **local scheduler verified** (Run 4.216) via `site-002-post-1c-monitor-runner.ps1` + Windows Task `MARS_SITE_002_Post_1C_Catalog_Monitor` — **enabled**; natural scheduled run **CONFIRMED** Run **4.259** artifact `2026-07-12_12-30-02` from runtime checkout; Run **4.260** onboarded needs **364**/`posuda-i-inventar` + **365**/`stellazhi-standart-vysota-1600` — post-onboarding manual `2026-07-12_22-19-55` needs **0**; Run **4.255**/`4.260` allowlist; Run **4.257** scheduler detached from dirty `X:\AI MARS` · [onboarding 02](../reports/SITE-002-PROD-CATALOG-NEW-BRANCH-ONBOARDING-02.md) · [natural run verify](../../mars-infrastructure/reports/MARS-INFRA-RUNTIME-SPLIT-SITE-002-NATURAL-RUN-VERIFY-01.md) · [Git brief](../../mars-infrastructure/GIT-RUNTIME-BRIEF-FOR-PROJECT-CHATS.md) · [infra runtime split report](../../mars-infrastructure/reports/MARS-INFRA-RUNTIME-SPLIT-SITE-002-01.md) · [runtime sync](../reports/SITE-002-LOCAL-RUNTIME-MONITOR-SYNC-01.md) · [runtime checkouts](../../mars-infrastructure/runtime-checkouts.md) · [runbook](../runbooks/SITE-002-POST-1C-MONITOR-AUTOMATION-RUNBOOK.md)
...
### Post-1C monitor scheduler (Run 4.215 / 4.216 / **4.257 runtime split** / **4.258 spotcheck** / **4.259 natural verify** / **4.260 onboarding 02** / **4.261 baseline refresh** / **4.262 runtime pin**)
...
- **Runtime checkout (Run 4.257 / pin 4.262):** `X:\AI MARS STORAGE\runtime-checkouts\site-002-monitor\repo` @ `0ab7e9f5` (**clean**) — **do not** run scheduled monitor from dirty `X:\AI MARS`
- **Spotcheck (Run 4.258):** scheduler confirmed on runtime checkout; natural run after split was pending at that time
- **Natural run verify (Run 4.259):** natural artifact `2026-07-12_12-30-02` **confirmed**; LastTaskResult **0**; classification **ONBOARDING_REQUIRED**; needs **2** new branches; [natural run verify](../../mars-infrastructure/reports/MARS-INFRA-RUNTIME-SPLIT-SITE-002-NATURAL-RUN-VERIFY-01.md) · [Git brief](../../mars-infrastructure/GIT-RUNTIME-BRIEF-FOR-PROJECT-CHATS.md) · [spotcheck report](../../mars-infrastructure/reports/MARS-INFRA-RUNTIME-SPLIT-SITE-002-SCHEDULED-SPOTCHECK-01.md)
- **Monitor baseline refresh 05 (Run 4.300):** storage baseline **1737→1854** after confirmed post-import persistence; post-refresh `2026-07-27_15-24-48` **`NO_ACTION_REQUIRED`**; checkpoint `SITE-002-STABLE-PROD-POST-1C-MONITOR-BASELINE-1854-05` · [report](../reports/SITE-002-MONITOR-BASELINE-REFRESH-05.md)
- **Legacy category cleanup charter 01 (Run 4.301):** read-only Option A 301+disable plan — **superseded** by operator for next action · [report](../reports/SITE-002-PROD-LEGACY-CATEGORY-CLEANUP-CHARTER-01.md)
- **Demo catalog cleanup charter 01 (Run 4.302):** read-only inventory; Group A DELETE_READY **154–170**; apply deferred · [report](../reports/SITE-002-PROD-DEMO-CATALOG-CLEANUP-CHARTER-01.md)
- **Parent 153 review charter 01 (Run 4.305):** read-only; parent **153** `PARENT_153_DELETE_READY`; Option A physical delete plan ready; apply executed in **4.306** · [report](../reports/SITE-002-PROD-PARENT-153-REVIEW-CHARTER-01.md)
- **Parent 153 delete apply 01 (Run 4.306):** physical delete **153**; **6** rows; 154–170 remain deleted; sitemap **1837→1836**; baseline still **1837**; next refresh 07 · [report](../reports/SITE-002-PROD-PARENT-153-DELETE-APPLY-01.md)
- **Ambiguous empty categories review charter 01 (Run 4.308):** read-only; **119** = **99** leaves + **20** empty parents; 119 vs 99 **RESOLVED**; DELETE_READY **0**; operator review **97**; apply not executed · [report](../reports/SITE-002-PROD-AMBIGUOUS-EMPTY-CATEGORIES-REVIEW-CHARTER-01.md)
- **Empty categories HITL triage 01 (Run 4.309):** read-only; **97** candidates enriched; proposed WAIT **96** / KEEP **1** / DELETE_CAND **0**; operator decision sheet ready; apply not executed · [report](../reports/SITE-002-PROD-EMPTY-CATEGORIES-HITL-TRIAGE-01.md)
- **Monitor baseline refresh 06 (Run 4.304):** storage baseline **1854→1837** after demo delete apply; post-refresh 2026-07-27_17-07-47 **NO_ACTION_REQUIRED**; checkpoint SITE-002-STABLE-PROD-POST-DEMO-CATEGORY-DELETE-MONITOR-BASELINE-1837-06 · [report](../reports/SITE-002-MONITOR-BASELINE-REFRESH-06.md)
- **Demo category delete apply 01 (Run 4.303):** physical delete **154–170**; **119** rows; parent **153** kept then deleted in **4.306**; sitemap **1854→1837**; baseline refreshed in **4.304** · [report](../reports/SITE-002-PROD-DEMO-CATEGORY-DELETE-APPLY-01.md)
- **Monitor baseline refresh 04 (Run 4.288):** storage baseline **1714→1737**; post-refresh `2026-07-20_22-32-43` **`NO_ACTION_REQUIRED`**; checkpoint `SITE-002-STABLE-PROD-POST-1C-MONITOR-BASELINE-1737-04`; tech equipment wave closed for monitor · [report](../reports/SITE-002-MONITOR-BASELINE-REFRESH-04.md)
- **Monitor baseline refresh 03 (Run 4.275):** storage baseline **1615→1714**; post-refresh `2026-07-16_15-03-50` **`NO_ACTION_REQUIRED`**; checkpoint `SITE-002-STABLE-PROD-POST-1C-MONITOR-BASELINE-1714-03` · [report](../reports/SITE-002-MONITOR-BASELINE-REFRESH-03.md)
- **Monitor baseline refresh 02 (Run 4.269):** storage baseline **1530→1615**; post-refresh `2026-07-15_15-53-13` **`NO_ACTION_REQUIRED`**; checkpoint `SITE-002-STABLE-PROD-POST-1C-MONITOR-BASELINE-1615-02` · [report](../reports/SITE-002-MONITOR-BASELINE-REFRESH-02.md)
- **Blog scheduled news RCK productivity (Run 4.270):** custom blog architecture documented; autopublish gate on `catalog/model/blog/blog.php`; post_id **13** scheduled Barnaul **2026-07-16 07:00** (stored Moscow `03:00`); pre-publish 404 · [report](../reports/SITE-002-PROD-BLOG-SCHEDULED-NEWS-RCK-PRODUCTIVITY-01.md)
- **Blog wave final smoke (Run 4.279):** readonly verification after 4.270–4.278 — ALL CHECKS GREEN; post **13** SEO+route 200; sliders/readtime/brand OK; sitemap **1714**; monitor **NO_ACTION_REQUIRED**; production mutation **0** · [report](../reports/SITE-002-PROD-BLOG-WAVE-FINAL-SMOKE-01.md)
- **Multiday healthcheck (Run 4.280):** readonly after several days — 1C imports **2026-07-16..20** SUCCESS; sitemap **1737** (+23 vs baseline **1714**); monitor **`ONBOARDING_REQUIRED`**; **6** new teplovoe/hlebopekarnoe/myaso branches; blog/SEO/brand/forms **OK**; next **`SITE-002-PROD-CATALOG-NEW-BRANCH-ONBOARDING-05`** · [report](../reports/SITE-002-PROD-MULTIDAY-HEALTHCHECK-01.md) · Storage `audits/SITE-002-PROD-MULTIDAY-HEALTHCHECK-01/`
- **Onboarding 05 (Run 4.281):** ids **368**/hlebopekarnoe + **373**/myasopererabatyvayuschee + **369**/teplovoe + **371**/grili-kontaktnye + **372**/risovarki + **370**/vodonagrevateli; exact meta + flat allowlist + runtime sync; manual `2026-07-20_18-05-09` target needs **0**; overall needs **230** (URL churn); baseline still **1714**; next **`SITE-002-MONITOR-BASELINE-REFRESH-04`** · [report](../reports/SITE-002-PROD-CATALOG-NEW-BRANCH-ONBOARDING-05.md)
- **Production regression hotfix (Run 4.282):** parent PLP product links used incomplete `path` → `checkProductCategory()` cleared product → «Товар не найден»; `seo_url.php` rebuilds path from `product_to_category` when membership fails; `header.php` uses `!empty(...['has_children'])` (megamenu sets flag on roots only); blog full-path SEO preserved; clear `storage/modification` + `storage/cache` after controller FTP; admin cache button **SAFE UNKNOWN** · [report](../reports/SITE-002-PROD-REGRESSION-HOTFIX-01.md)
- **Mega menu + cache plugin diagnostic (Run 4.283):** clearing `storage/cache` emptied `cat-list-header`; `header.php` previously read-only on miss → empty mega menu until `/katalog`; durable fix = rebuild-on-miss (katalog parity) + `isset(short_description)`; Launch Mode then one left root (`nejtralnoe-oborudovanie`); admin top-bar cleaner = module `oc3x_storage_cleaner` + OCMOD `Cache_Cleaner` — button missing because `storage/modification/` not refreshed after clear; tech root category_id **362** image null — tile automation next · [report](../reports/SITE-002-PROD-MEGAMENU-AND-CACHE-PLUGIN-DIAGNOSTIC-01.md)
- **Admin cache cleaner button restore (Run 4.284):** `oc3x_storage_cleaner` / OCMOD `Cache_Сleaner` (Cyrillic «С» in DB code) injects admin top-bar via compiled `storage/modification/admin/.../header.php` + `header.twig`; wiping `storage/modification/` removes the button until **Modifications → Refresh** (`marketplace/modification/refresh`); safe restore = standard refresh (not FTP/source hack); visual admin confirmation done; public product/blog/megamenu OK after rebuild; catalog `header.php` / `seo_url.php` hotfixes stay in **source** (no mod overlays for those paths) · [report](../reports/SITE-002-PROD-ADMIN-CACHE-CLEANER-BUTTON-RESTORE-01.md)
- **Catalog Section Tiles automation (Run 4.285) + polish (Run 4.286) + mega children (Run 4.287) + elektro image (Run 4.298):** EN **Catalog Section Tiles** / RU **Плитки разделов каталога**; Launch Mode roots **79** + **362**; home + `/katalog` dual section blocks; tech children DB-driven (`getCategories(362)`); tech root name normalized to `Технологическое оборудование`; tile images `catalog/Category-image/` for **368/373/369/371/372/370** (+ regen **373/364/369/368**) + **375** `elektromehanicheskoe.webp`; `.btn.zpm-catalog__all-link` → `/katalog/`; mega menu section-hub children rebuilt via `buildHubChildCards` so tech mega pane matches tiles; baseline later refreshed in Run **4.288** (not in 4.298) · [elektro image](../reports/SITE-002-PROD-ELEKTROMEHANICHESKOE-CATEGORY-IMAGE-APPLY-01.md) · [megamenu children](../reports/SITE-002-PROD-MEGAMENU-CHILDREN-AUTOMATION-01.md) · [image regen](../reports/SITE-002-PROD-TECH-CATEGORY-IMAGES-REGEN-01.md) · [polish](../reports/SITE-002-PROD-CATALOG-TILE-POLISH-01.md) · [tiles](../reports/SITE-002-PROD-CATALOG-TILE-BLOCKS-AUTOMATION-01.md)
- **Blog SEO URL routing (Run 4.278):** active startup is `startup/seo_url` (not `seo_pro` despite `config_seo_url_type`); blog keywords are full paths (`blog/news`, `blog/news/{slug}`) with queries `blog_category_id=N` / `blog_post_id=N`; `/blog/news` **is intended**; patched `seo_url.php` full-path decode + rewrite (no DB row changes); cache clear not required beyond optional `cache.seo_pro*` · [report](../reports/SITE-002-PROD-BLOG-SEO-URL-ROUTING-FIX-01.md)
- **Blog RCK logo + title image (Run 4.271):** post_id **13** hero + body logo attached; schedule unchanged · [report](../reports/SITE-002-PROD-BLOG-RCK-LOGO-AND-TITLE-IMAGE-01.md)
- **Blog publish datetime + reading time (Run 4.272):** admin datetime UI; `reading_time_minutes`; frontend meta output; post 13 schedule unchanged · [report](../reports/SITE-002-PROD-BLOG-PUBLISH-DATETIME-READTIME-01.md)
- **Blog postpublish + related slider meta (Run 4.273):** post **13** published OK; `other_news.twig` uses real `views` + `reading_time_text`; monitor/sitemap delta noted · [report](../reports/SITE-002-PROD-BLOG-POSTPUBLISH-1C-HEALTHCHECK-RELARTICLES-META-01.md)
- **Brand caps + blog slider order (Run 4.276):** `getSliderPosts()` newest-first max **24**; home/category/post controllers; brand `Завод` in full-name phrases; OC mod cache path `/home/a/assum/bzpm.ru/storage/modification/` · [report](../reports/SITE-002-PROD-BRAND-CAPS-AND-BLOG-SLIDER-ORDER-01.md)
- **Blog literal newline cleanup (Run 4.277):** post **13** `content` had **6** literal `\n` (backslash+n) between `</p><p>` — DB `REPLACE` cleanup; root cause **DB_CONTENT_ONLY** (escaped newlines at article paste); no source patch · [report](../reports/SITE-002-PROD-BLOG-LITERAL-NEWLINE-CLEANUP-01.md)
- **Onboarding 04 (Run 4.274):** id **367**/stellazhi-premium-3-vysota-1600 meta+allowlist; runtime file sync; manual 2026-07-16_14-48-00 needs **0**; [report](../reports/SITE-002-PROD-CATALOG-NEW-BRANCH-ONBOARDING-04.md)
- **Onboarding 03 (Run 4.268):** id **366**/stellazhi-premium-vysota-1600 meta+allowlist; runtime file sync; manual 2026-07-15_15-25-30 needs **0**; [report](../reports/SITE-002-PROD-CATALOG-NEW-BRANCH-ONBOARDING-03.md)
- **Onboarding 02 (Run 4.260):** ids **364**/`posuda-i-inventar` + **365**/`stellazhi-standart-vysota-1600` meta+allowlist; runtime file sync; manual `2026-07-12_22-19-55` needs **0**; [report](../reports/SITE-002-PROD-CATALOG-NEW-BRANCH-ONBOARDING-02.md)
- **Monitor baseline refresh (Run 4.261):** storage baseline **1377→1530**; post-refresh `2026-07-12_22-55-45` **`NO_ACTION_REQUIRED`**; checkpoint `SITE-002-STABLE-PROD-POST-1C-MONITOR-BASELINE-1530-01` · [report](../reports/SITE-002-MONITOR-BASELINE-REFRESH-01.md)
- **Runtime checkout pin (Run 4.262):** `git reset --hard 0ab7e9f5`; status **clean**; manual `2026-07-13_00-05-00` **`NO_ACTION_REQUIRED`**; scheduler unchanged · [report](../../mars-infrastructure/reports/MARS-INFRA-RUNTIME-CHECKOUT-PIN-SITE-002-01.md)
...
- UI first-level block implementation **not** performed in this run.

### First-level block scope decision (Run 4.313)

- Verdict: **HYBRID RECOMMENDED**.
- SHOW: **80, 86, 207, 301, 322, 326, 331, 354, 358, 360**.
- HIDE/WAIT: **82, 83, 85, 87, 89** (legacy duplicate names; no 1C map proof).
- Empty copy for future proven empties: `Ожидайте, товары скоро поступят.`
- Apply **not** executed; next = operator confirmation then apply charter.
- Report: [SITE-002-PROD-FIRST-LEVEL-BLOCK-SCOPE-DECISION-01.md](../reports/SITE-002-PROD-FIRST-LEVEL-BLOCK-SCOPE-DECISION-01.md).

### First-level block hybrid apply (Run 4.314)

- Verdict: **COMPLETE — ACCEPTED** (operator visual review closed in Run **4.315**).
- Surfaces: home + `/katalog/` Catalog Section Tiles.
- Helper: `buildNeutralFirstLevelBlockCards` (mega keeps `buildHubChildCards` product gate).
- SHOW/HIDE as Run 4.313; empty copy supported but not currently rendered.
- Tech/deep/sitemap/baseline/importer unchanged; baseline remains **1879**.
- Report: [SITE-002-PROD-FIRST-LEVEL-BLOCK-HYBRID-APPLY-01.md](../reports/SITE-002-PROD-FIRST-LEVEL-BLOCK-HYBRID-APPLY-01.md).

### First-level block hybrid closeout (Run 4.315)

- Verdict: **COMPLETE — ACCEPTED** (historical).
- Docs-only; operator confirmed Run **4.314** COMPLETE at the time.
- Practical UI outcome later corrected by Run **4.316** after operator visual review.
- Report: [SITE-002-PROD-FIRST-LEVEL-BLOCK-HYBRID-CLOSEOUT-01.md](../reports/SITE-002-PROD-FIRST-LEVEL-BLOCK-HYBRID-CLOSEOUT-01.md).

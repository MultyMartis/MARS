#!/usr/bin/env python3
"""BZPM M9.5 — Neutral Root Hub deploy to TEST only (category 79 hub mode)."""
import ftplib
import hashlib
import io
import json
...
BASE = r"C:\AI MARS\projects\ocpilot\sites\site-002\m9.5-root-hub-work"
...
def main():
    ...
    for rel in REMOTE_FILES:
        ...
        try:
            ...
            pre_path = os.path.join(BACKUP_DIR, f"pre-m9.5-{rel.replace('/', '__')}")
            ...
            pre_backup.append({"remote": rel, "local_backup": None, "note": f"download error: {e}"})
    ...
    manifest = {
        "task": "M9.5 Neutral Root Category Hub (category 79)",
        "site": "SITE-002",
        "test_url": "https://zpm.new-site.space/",
        "rollback_source": "SITE-002-STABLE-M8.3-BEFORE-M9-20260615-0159",
        "m9_phase3_rollback": "m9-phase3-remaining-work/backups/m9-phase3-deploy-20260614-200051.json",
    ...
    out = os.path.join(BACKUP_DIR, f"m9.5-root-hub-deploy-{STAMP}.json")

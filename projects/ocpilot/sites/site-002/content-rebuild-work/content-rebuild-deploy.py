#!/usr/bin/env python3
"""SITE-002 PDP content structure rebuild — FTP deploy producttabs.twig only."""
import ftplib
import hashlib
import io
import json
...
ROLLBACK_BAK = os.path.join(BACKUP_DIR, "producttabs.twig.pre-content-rebuild.bak")
...
def main():
    ...
    if not os.path.isfile(ROLLBACK_BAK):
        print("Downloading live twig for rollback backup...")
        twig_live = ftp_download(TWIG_REMOTE)
        with open(ROLLBACK_BAK, "wb") as f:
            f.write(twig_live)
        print("  saved", ROLLBACK_BAK)
    ...
    manifest = {
        "stamp": STAMP,
        "task": "pdp-content-structure-rebuild",
    ...
    manifest_path = os.path.join(BACKUP_DIR, f"content-rebuild-deploy-manifest-{STAMP}.json")

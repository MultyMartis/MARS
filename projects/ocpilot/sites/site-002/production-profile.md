...
**Last updated:** 2026-08-19 (Run 4.325 — SITE-002-PROD-POST-1C-OFFERS-RECOVERY-AND-NEW-SECTIONS-HEALTHCHECK-01 — latest import SUCCESS with offers recovered; sitemap 1887 vs baseline 1879; new sections need onboarding; monitor artifacts semantically inconsistent; no production mutation)
...
| New sections onboarding / placement charter (Production) | **COMPLETE — SAFE APPLY PLAN READY** (Run **4.326**) — `barnoe` = live empty root; `hlebopekarnoe` = live root with products; `holodilnoe` = live root with 1 product; `posuda` = live child under `362` despite XML top-level; `upakovochnoe` = XML-only/404; recommended default = keep DB placement stable, separate UI from structure, no structural moves in onboarding Wave A, mapping charter + monitor diagnostic next · [charter 4.326](reports/SITE-002-PROD-NEW-SECTIONS-ONBOARDING-PLACEMENT-CHARTER-01.md) · [healthcheck 4.325](reports/SITE-002-PROD-POST-1C-OFFERS-RECOVERY-AND-NEW-SECTIONS-HEALTHCHECK-01.md) |

...
**Last updated:** 2026-07-09 (Run 4.234 — category Lari reparent discovery; implementation charter ready; checkpoint unchanged `SITE-002-STABLE-PROD-INFO-PAGE-FORMS-01`)
...
| Category Lari reparent (Production) | **DISCOVERY COMPLETE — CHARTER READY** (Run 4.234) — Лари id **88** currently `parent_id=79` (direct under neutral hub); target `parent_id=358` (Шкафы и лари); old `/katalog/nejtralnoe-oborudovanie/lari` live + sitemapped; nested `/shkafy-i-lari/lari` resolves to old path; hybrid reparent + 301 recommended; 0 Production mutation · [discovery report](reports/SITE-002-PROD-CATEGORY-LARI-REPARENT-DISCOVERY-01.md) |

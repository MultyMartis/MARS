#!/usr/bin/env python3
"""SITE-002 PDP documents block final pass — FTP deploy."""
import ftplib
import hashlib
import io
import json
...
WORK_DIR = os.path.join(BASE, "documents-final-pass-work")
STAMP = datetime.now(timezone.utc).strftime("%Y%m%d-%H%M%S")
ROLLBACK = {
    "producttabs.twig": os.path.join(BACKUP_DIR, "producttabs.twig.pre-documents-final-pass.bak"),
    "style.css": os.path.join(BACKUP_DIR, "style.css.pre-documents-final-pass.bak"),
}
...
def main():
    ...
    manifest_path = os.path.join(BACKUP_DIR, f"documents-final-pass-deploy-manifest-{STAMP}.json")

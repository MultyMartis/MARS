#!/usr/bin/env python3
"""QA + screenshots — SITE-002 CATEGORY V2.1 list card layout fix."""
import asyncio
import json
import os
import sys
...
OUT_DIR = r"C:\AI MARS\projects\ocpilot\sites\site-002\qa\category-v2.1-list-card-layout-fix"
QA_JSON = os.path.join(OUT_DIR, "category-v2.1-list-card-layout-fix-qa-result.json")
...
async def measure_list_card(page):
    return await page.evaluate(
        ...
        const footer = card.querySelector('.p-card__footer');
        const actions = card.querySelector('.p-card__top .p-card__actions');
        ...
        return {
            ...
            has_fad_length: iconClasses.some(c => c.includes('fa-arrows-alt-h')),
            has_fad_width: iconClasses.some(c => c.includes('fa-arrows-alt-v')),
            has_fad_height: iconClasses.some(c => c.includes('fa-arrow-to-top')),
            has_fa_weight: iconClasses.some(c => c.includes('fa-weight-hanging')),
            ...
            wishlist_clickable: !!wishlist,
            compare_clickable: !!compare,
...
async def measure_category(page, mode):
    ...
    if mode == "list":
        base.update(await measure_list_card(page))
        base["pass"] = (
...
async def shot_category(page, width, height, mode):
    ...
    print("saved", path, "pass=", metrics.get("pass"))
    return metrics

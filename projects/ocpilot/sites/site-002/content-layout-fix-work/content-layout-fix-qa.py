...
def analyze(label, url):
    ...
    specs_count = len(re.findall(r'class="product-content__specifications', content))

    main_block = ""
    m_main = re.search(r'<div class="product-content__main">(.*?)</div>\s*(?:<aside|<!--)', content, re.S)
    if m_main:
    ...
    checks = {
        ...
        "product_content_main": "product-content__main" in content,
        "no_product_content_top": "product-content__top" not in content,
        ...
        "has_main": "product-content__main" in content,
        "has_side": "product-content__side" in content,
        "specs_in_main": "product-content__specifications" in main_block,
        ...
        and "Технические характеристики</h3>" in content,
        ...
        "white_bg_rule": "background-color: #fff" in open(
            r"C:\AI MARS\projects\ocpilot\sites\site-002\content-layout-fix-work\style.css",
            encoding="utf-8",
        ).read(),
        "sample_link": (
    ...
    always = [
        ...
        "product_content_grid", "product_content_main", "no_product_content_top",
        "specs_in_main",
    ]
...
def main():
    results = {
        ...
        "twig_branches_verified_static": {
            "case_b": "no documents: product-content__main only, 100% width",
            "case_d": "no description and no documents: specifications only in main",
        },
    }
    ok = True
    ...
    out = r"C:\AI MARS\projects\ocpilot\sites\site-002\content-layout-fix-work\content-layout-fix-qa-result.json"

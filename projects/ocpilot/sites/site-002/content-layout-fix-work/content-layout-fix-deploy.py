#!/usr/bin/env python3
"""SITE-002 PDP content layout fix — FTP deploy."""
import ftplib
import hashlib
import io
import json
...
WORK_DIR = os.path.join(BASE, "content-layout-fix-work")
STAMP = datetime.now(timezone.utc).strftime("%Y%m%d-%H%M%S")
ROLLBACK = {
    "producttabs.twig": os.path.join(BACKUP_DIR, "producttabs.twig.pre-content-layout-fix.bak"),
    "style.css": os.path.join(BACKUP_DIR, "style.css.pre-content-layout-fix.bak"),
}
...
def main():
    ...
    manifest_path = os.path.join(BACKUP_DIR, f"content-layout-fix-deploy-manifest-{STAMP}.json")

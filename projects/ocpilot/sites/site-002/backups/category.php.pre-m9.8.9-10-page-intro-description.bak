...
class ControllerProductCategory extends Controller {
	public function index() {
		...
		if ($category_info) {
			$is_hub = $visibility->isNeutralHubCategory($category_id);
			...
			if ($is_hub) {
				foreach ($visibility->getNeutralHubBranchIds() as $branch_id) {
					$branch = $this->model_catalog_category->getCategory($branch_id);
					...
					if ($branch['image']) {
						$thumb300 = $this->model_tool_image->resize($branch['image'], 300, 300);
					} else {

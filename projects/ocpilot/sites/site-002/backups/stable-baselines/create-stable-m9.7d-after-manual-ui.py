#!/usr/bin/env python3
"""SITE-002 — Stable checkpoint M9.7D after operator manual UI refinement (read-only)."""
import ftplib
import hashlib
import io
import json
...
BACKUP_NAME = "SITE-002-STABLE-M9.7D-AFTER-MANUAL-UI"
...
REMOTE_FILES = [
    ("system/library/zpm/category_visibility.php", ["M7.1", "M9.5", "M9.7C"]),
...
def fetch(url: str) -> dict:
    req = urllib.request.Request(
        url,
        headers={"User-Agent": "SITE-002-Stable-M9.7D/1.0", "Accept": "text/html"},
    )
...
def run_qa() -> dict:
    ...
    return {
        ...
        "baseline_note": "LIVE TEST STATE AFTER MANUAL UI REFINEMENT — supersedes M9.7C visual baseline",
        "checks": checks,
        "summary": summary,
        "megamenu_after": mm,
        "hub_cards": cards,
...
def build_manifest(qa, files, images, missing_files, db_dumps, json_exports, db_errors, data_snapshot) -> dict:
    manifest = {
        ...
        "stable_state": "M7.1 + M8.3 + M9 + M9.5 + M9.7B/C + operator manual UI refinement on TEST",
        "supersedes": "SITE-002-STABLE-M9-COMPLETE-20260615",
        "visual_baseline_note": "LIVE TEST STATE AFTER MANUAL UI REFINEMENT — do NOT use M9.7C or M9-COMPLETE as visual baseline",
        ...
        "rollback_instruction": {
            ...
            "prior_checkpoint": "SITE-002-STABLE-M9-COMPLETE-20260615 for pre-M9.7D rollback (excludes manual UI)",

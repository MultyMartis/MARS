            ...
            foreach ($f['attr'] as $attr_slug => $values) {
                ...
                if ($implode) {
                    if (is_numeric($attr_slug)) {
                        ...
                        AND pa.attribute_id = '" . (int)$attr_slug . "' 
			...
			if (!empty($f['attr']) && is_array($f['attr'])) {
				foreach ($f['attr'] as $attr_slug => $values) {
					...
					if (!empty($implode)) {
						...
						Numeric attr key → pa.attribute_id; slug key → ad.filter_name.
						*/
						if (is_numeric($attr_slug)) {
							$sql .= " AND EXISTS (
							SELECT 1 FROM " . DB_PREFIX . "product_attribute pa 
...
public function getAttributesByCategory($category_ids, $plp_category_id = 0) {
    ...
    require_once(DIR_SYSTEM . 'library/zpm/attribute_filter_visibility.php');
    $filter_visibility = new AttributeFilterVisibility();
    ...
    if (!$attribute_data) {
        ...
        foreach ($query->rows as $result) {
            if ($filter_visibility->isStoreOnly((int)$result['attribute_id'])) {
                continue;
            }
        ...
        foreach ($attribute_data as $key => $attribute) {
            if ($filter_visibility->isStoreOnly((int)$attribute['attribute_id'])) {
                unset($attribute_data[$key]);
            }
        }
    }

    $plp_category_id = (int)$plp_category_id;
    if ($plp_category_id > 0) {
        require_once(DIR_SYSTEM . 'library/zpm/filter_profile_resolver.php');
        $profile_resolver = new FilterProfileResolver($this->db);
        $filter_profile = $profile_resolver->resolveForCategory($plp_category_id);

        if ($filter_profile) {
            $attribute_data = $profile_resolver->applyProfileToAttributes($filter_profile, $attribute_data);
        }
    }

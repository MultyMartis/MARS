#!/usr/bin/env python3
"""SITE-002 CATEGORY V2.1 list card commerce pass — FTP deploy."""
import ftplib
import hashlib
import io
import json
...
ROLLBACK = {
    "product_results.php": os.path.join(
        BACKUP_DIR, "product_results.php.pre-list-card-commerce-pass.bak"
    ),
    "productcard.twig": os.path.join(
        BACKUP_DIR, "productcard.twig.pre-list-card-commerce-pass.bak"
    ),
    "style.css": os.path.join(BACKUP_DIR, "style.css.pre-list-card-commerce-pass.bak"),
}
...
def main():
    ...
    manifest = {
        "pass": "CATEGORY-V2.1-LIST-CARD-COMMERCE-PASS",
        "site_id": "SITE-002",
    ...
    manifest_path = os.path.join(
        BACKUP_DIR, f"category-v2.1-list-card-commerce-deploy-manifest-{STAMP}.json"
    )

...
foreach ($results as $result) {
				...
				if ($result['length'] > 0) {
					$primary_specs[] = array('name' => 'Длина, мм', 'text' => intval($result['length']));
				}

				if ($result['width'] > 0) {
					$primary_specs[] = array('name' => 'Ширина, мм', 'text' => intval($result['width']));
				}

				if ($result['height'] > 0) {
					$primary_specs[] = array('name' => 'Высота, мм', 'text' => intval($result['height']));
				}

				if ($result['weight'] > 0) {
					$primary_specs[] = array('name' => 'Масса, кг', 'text' => round((float)$result['weight'], 2));
				}

<?php
/**
 * BZPM M9 — category filter profile resolver (Phase 1: profile 301 only).
 *
...
class FilterProfileResolver {
	...
	/** @var int[] */
	private $registered_branch_roots = array(301);
	...
	public function loadProfile($profile_id) {
		...
		$file = DIR_SYSTEM . 'library/zpm/filter_profiles/' . $profile_id . '_stoly.php';

		if ($profile_id === 301 && is_file($file)) {
			$profile = require($file);
			$profile['hidden_global_ids'] = $this->getGlobalHiddenIds();
			return $profile;
		}

		// Future: 80_moechnye_vanny.php, 322_podtovarniki.php, 207_zonty.php
		$generic = DIR_SYSTEM . 'library/zpm/filter_profiles/' . $profile_id . '.php';
		if (is_file($generic)) {
			$profile = require($generic);
			$profile['hidden_global_ids'] = $this->getGlobalHiddenIds();
			return $profile;
		}
	...
	public function isHiddenAttribute($profile, $attribute_id) {
		...
		return !in_array($attribute_id, $allowed, true);
	}

...
**Evidence cutoff:** 2026-08-19 (SITE-002 Run **4.328** / `SITE-002-PROD-NEW-SECTIONS-WAVE-B-MAPPING-CHARTER-01` — Wave B charter complete; safe B1 backfill narrowed to `95` and `364`; `186` root mapping deferred due overlap with nested Tech `368`; `171` unmapped on current XML evidence; `upakovochnoe` still blocked; baseline refresh still blocked; next = B1 apply approval + monitor diagnostic)
...
| Current SITE-002 focus | **Wave B mapping charter complete (Run 4.328)** — safe backfill scope is now narrowed to `holodilnoe` (`95`) and `posuda` (`364`) only; `hlebopekarnoe` root mapping remains unresolved because current XML proves nested Tech `Хлебопекарное` overlap with `368`; `barnoe` lacks matching current XML proof; `upakovochnoe` remains XML-only/blocked; baseline refresh still blocked; next = bounded B1 apply after approval + Wave C monitor diagnostic · [charter 4.328](sites/site-002/reports/SITE-002-PROD-NEW-SECTIONS-WAVE-B-MAPPING-CHARTER-01.md) · [apply 4.327](sites/site-002/reports/SITE-002-PROD-NEW-SECTIONS-WAVE-A-ONBOARDING-APPLY-01.md) · [charter 4.326](sites/site-002/reports/SITE-002-PROD-NEW-SECTIONS-ONBOARDING-PLACEMENT-CHARTER-01.md) |

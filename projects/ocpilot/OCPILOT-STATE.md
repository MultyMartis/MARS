...
**Evidence cutoff:** 2026-08-19 (SITE-002 Run **4.329** / `SITE-002-PROD-NEW-SECTIONS-WAVE-B1-MAPPING-BACKFILL-APPLY-01` — Wave B1 mapping backfill complete for `95` and `364`; excluded categories untouched; baseline refresh still blocked; next = Wave C monitor diagnostic + post-B1 import observation)
...
| Current SITE-002 focus | **Wave B1 mapping backfill complete (Run 4.329)** — `holodilnoe` (`95`) and `posuda` (`364`) now GUID-mapped in `oc_mars_1c_category_map`; `364` remains DB-child under `362`; `hlebopekarnoe`/`barnoe`/`upakovochnoe` still unmapped by design; baseline refresh still blocked; next = Wave C monitor diagnostic + observe next natural 1C import · [apply 4.329](sites/site-002/reports/SITE-002-PROD-NEW-SECTIONS-WAVE-B1-MAPPING-BACKFILL-APPLY-01.md) · [charter 4.328](sites/site-002/reports/SITE-002-PROD-NEW-SECTIONS-WAVE-B-MAPPING-CHARTER-01.md) · [apply 4.327](sites/site-002/reports/SITE-002-PROD-NEW-SECTIONS-WAVE-A-ONBOARDING-APPLY-01.md) |

...
**SITE-002 Run 4.329 (2026-08-19):** New sections Wave B1 mapping backfill apply **COMPLETE** — bounded `oc_mars_1c_category_map` upsert for `holodilnoe` (`95`, GUID `95bfa611-898d-11f1-aece-581122cf362c`) and `posuda` (`364`, GUID `9b37b1f1-7c19-11f1-aecc-581122cf362c`); no category/product/importer/baseline changes; excluded `186`/`171`/`upakovochnoe` untouched; public smoke PASS; baseline refresh still blocked; next = Wave C monitor diagnostic. Report: [SITE-002-PROD-NEW-SECTIONS-WAVE-B1-MAPPING-BACKFILL-APPLY-01.md](sites/site-002/reports/SITE-002-PROD-NEW-SECTIONS-WAVE-B1-MAPPING-BACKFILL-APPLY-01.md).
**SITE-002 Run 4.328 (2026-08-19):** New sections Wave B mapping charter **COMPLETE** — fresh XML/DB/map/importer review confirms safe B1 backfill scope only for `holodilnoe` (`95`) and `posuda` (`364`); `hlebopekarnoe` root mapping deferred because current XML proves nested Tech `Хлебопекарное` overlap with `368`; `barnoe` current XML proof absent; `upakovochnoe` XML-only with product evidence but create/map still blocked because auto-create is off; baseline refresh still blocked; next = bounded B1 apply after approval + Wave C monitor diagnostic. Report: [SITE-002-PROD-NEW-SECTIONS-WAVE-B-MAPPING-CHARTER-01.md](sites/site-002/reports/SITE-002-PROD-NEW-SECTIONS-WAVE-B-MAPPING-CHARTER-01.md).
**SITE-002 Run 4.327 (2026-08-19):** New sections Wave A onboarding apply **COMPLETE** — `hlebopekarnoe` and `holodilnoe` verified live; bounded DB mutations only: `186.description`, `95.meta_description`, `95.description`; no image/nav/source changes; `barnoe`, `posuda`, `upakovochnoe` untouched; baseline refresh still blocked; next = Wave B mapping charter + Wave C monitor diagnostic. Report: [SITE-002-PROD-NEW-SECTIONS-WAVE-A-ONBOARDING-APPLY-01.md](sites/site-002/reports/SITE-002-PROD-NEW-SECTIONS-WAVE-A-ONBOARDING-APPLY-01.md).
**SITE-002 Run 4.326 (2026-08-19):** New sections onboarding and placement charter **COMPLETE** — `barnoe` empty/live, `hlebopekarnoe` live with products, `holodilnoe` live with 1 product, `posuda` live but DB-child under `362`, `upakovochnoe` XML-only/404; recommended strategy = keep DB placement stable, separate UI from structure, mapping charter next, baseline refresh still blocked. Report: [SITE-002-PROD-NEW-SECTIONS-ONBOARDING-PLACEMENT-CHARTER-01.md](sites/site-002/reports/SITE-002-PROD-NEW-SECTIONS-ONBOARDING-PLACEMENT-CHARTER-01.md).
**SITE-002 Run 4.325 (2026-08-19):** Post-1C offers recovery and new sections healthcheck **COMPLETE** — latest import `mars_1c_import_2026-08-19_080010.txt` SUCCESS with real offers step; `offers0_1.xml` present/current; sitemap **1887** vs baseline **1879**; new sections require onboarding; monitor effective status **ONBOARDING_REQUIRED** because artifact semantics conflict. Report: [SITE-002-PROD-POST-1C-OFFERS-RECOVERY-AND-NEW-SECTIONS-HEALTHCHECK-01.md](sites/site-002/reports/SITE-002-PROD-POST-1C-OFFERS-RECOVERY-AND-NEW-SECTIONS-HEALTHCHECK-01.md).
...
**Evidence cutoff:** 2026-08-19 (SITE-002 Run **4.330** / `SITE-002-PROD-NEW-SECTIONS-WAVE-C-MONITOR-DIAGNOSTIC-01` — Wave C monitor diagnostic complete; route churn + runner artifact bug explained; baseline refresh still blocked; next = Wave C2 monitor fix after approval + observe next 1C import for `95`/`364`)
...
| Current SITE-002 focus | **Wave C monitor diagnostic complete (Run 4.330)** — route churn explains monitor raw diff; `monitor-classification.json` authoritative; runner merge bug confirmed in `run-summary.json`; raw onboarding 219 inflated; baseline refresh still blocked; next = Wave C2 monitor fix after approval + observe next natural 1C import for `95`/`364` · [diagnostic 4.330](sites/site-002/reports/SITE-002-PROD-NEW-SECTIONS-WAVE-C-MONITOR-DIAGNOSTIC-01.md) · [apply 4.329](sites/site-002/reports/SITE-002-PROD-NEW-SECTIONS-WAVE-B1-MAPPING-BACKFILL-APPLY-01.md) · [charter 4.328](sites/site-002/reports/SITE-002-PROD-NEW-SECTIONS-WAVE-B-MAPPING-CHARTER-01.md) · [apply 4.327](sites/site-002/reports/SITE-002-PROD-NEW-SECTIONS-WAVE-A-ONBOARDING-APPLY-01.md) |
...
| Date | Change |
|------|--------|
| 2026-08-19 | **UPDATED** — SITE-002 new sections Wave B1 mapping backfill apply **01** (Run **4.329**) — bounded `oc_mars_1c_category_map` upsert for `95` / `holodilnoe` and `364` / `posuda`; table **7→9** rows; excluded `186`/`171`/`upakovochnoe` untouched; no category/product/importer/baseline changes; public smoke PASS; baseline refresh remains blocked; next = Wave C monitor diagnostic |
| 2026-08-19 | **UPDATED** — SITE-002 new sections Wave B mapping charter **01** (Run **4.328**) — fresh XML/DB/map/importer review confirms B1 backfill-ready only for `95` / `holodilnoe` and `364` / `posuda`; `186` root mapping deferred due overlap with nested Tech `368`; `171` current XML proof absent; `Упаковочное` XML-only with product evidence but create/map still blocked because auto-create is disabled; baseline refresh remains blocked; production/FTP/DB/import/baseline/Client Ops **0** |
| 2026-08-19 | **UPDATED** — SITE-002 post-1C offers recovery and new sections healthcheck **01** (Run **4.325**) — latest import `mars_1c_import_2026-08-19_080010.txt` SUCCESS with real offers step; `offers0_1.xml` present/current; offers recovery confirmed; sitemap **1887** vs baseline **1879**; effective monitor classification **ONBOARDING_REQUIRED** due artifact inconsistency; next = onboarding + placement + importer mapping charter; production/FTP/DB/import/baseline/Client Ops **0** |
| 2026-08-19 | **UPDATED** — SITE-002 new sections onboarding and placement charter **01** (Run **4.326**) — classification complete for `barnoe`, `hlebopekarnoe`, `holodilnoe`, `posuda`, `upakovochnoe`; safe default = keep DB placement stable, separate UI from structure, mapping/monitor follow-up required; baseline refresh remains blocked; production/FTP/DB/import/baseline/Client Ops **0** |
...
| 2026-07-28 | **UPDATED** — SITE-002 first-level block hybrid closeout **01** (Run **4.315**) — docs-only; operator confirmed Run **4.314** COMPLETE; pending visual review closed; HYBRID **ACCEPTED / COMPLETE**; no further apply needed; normal monitor on baseline **1879**; production/Client Ops **0** |
| 2026-07-28 | **UPDATED** — SITE-002 first-level block hybrid apply **01** (Run **4.314**) — HYBRID live on home+`/katalog/`; SHOW 10 / HIDE **82/83/85/87/89**; empty copy supported not currently rendered; mega/deep/tech unchanged; baseline **1879**; FTP 3 files + cache.*; DB/import/Client Ops **0**; operator visual review **accepted** in Run **4.315** |

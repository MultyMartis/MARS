...
| **Phase 1B-D3 — Controlled sequential producer connection** | **COMPLETE** — synthetic FIRST_SEEN + exact replay; executions 3414/3415; Telegram message_id=6 once; D3 charter consumed; see [PHASE-1B-D3-CONTROLLED-SEQUENTIAL-PRODUCER-CONNECTION-AND-SYNTHETIC-LIVE-POST.md](PHASE-1B-D3-CONTROLLED-SEQUENTIAL-PRODUCER-CONNECTION-AND-SYNTHETIC-LIVE-POST.md) |
| **Phase 1B-D4 — SITE-002 real-source adapter / manual dry-run** | **COMPLETE (offline)** — adapter + sanitized fixtures + live guard; D4B offline evidence baseline committed (not pushed); see [PHASE-1B-D4-SITE002-REAL-SOURCE-ADAPTER-DESIGN-AND-MANUAL-DRY-RUN-INTEGRATION.md](PHASE-1B-D4-SITE002-REAL-SOURCE-ADAPTER-DESIGN-AND-MANUAL-DRY-RUN-INTEGRATION.md) |
| **n8n Client Ops workflow** | **INACTIVE** — auth bound + Pattern B Telegram + durable dedupe gate (nodes=17; executions=31; table rows=2); `DEDUPE_SEQUENTIAL_SAFE_CONCURRENCY_UNPROVEN` |
| **Telegram** | **SANDBOX PATH APPLIED** (`@monitor_bzpm_metacode_bot`; credential `2bIC5376l7ElXb4B` bound; chat `499423375`); D3 controlled one-send message_id=6; durable SENT ledger **DEFERRED**; **not** production-activated |
| **Production** | **UNCHANGED** |
| **Exporter (offline)** | Exists under `src/client_ops_reporting_bridge/` |
| **Exporter (publish/push)** | Generic `push-webhook` remains **blocked** (`NETWORK_DISPATCH_NOT_AUTHORIZED_D2`); D3 gated transport capability exists but one-time charter is **CONSUMED** |
...
**Explicit statement:** Phase 1B-D3 proved one controlled synthetic producer HTTPS path (FIRST_SEEN delivered once; exact replay suppressed). Phase 1B-D4 proved SITE-002 real-source adapter offline (deterministic event identity; firewall; mock responses). Phase 1B-D4B committed the offline evidence baseline (not pushed). Generic live mode remains BLOCKED. Real-source live POST remains BLOCKED (`REAL_SOURCE_LIVE_DISPATCH_NOT_AUTHORIZED_D4`). SITE-002 runtime monitor is **not** connected. Concurrent producers / scheduler / unattended / production activation remain forbidden. Durable post-Telegram SENT ledger remains DEFERRED. Next: **Phase 1B-D5 — First Manual SITE-002 Real-Source Connection Charter and Controlled Live POST** (Pattern B only; separate charter).

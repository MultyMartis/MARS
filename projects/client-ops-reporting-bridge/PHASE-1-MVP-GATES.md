...
**Next phase:** Phase 1B-D5 First Manual SITE-002 Real-Source Connection Charter and Controlled Live POST (Pattern B); real-source live POST and production activation remain blocked until that charter
...
| 2 | Dedicated Telegram bot vs existing internal bot? | **DEDICATED BOT + SANDBOX APPLY DONE** — `@monitor_bzpm_metacode_bot`; Pattern B applied on inactive workflow; one C1 delivery | Bot/credential/chat confirmed; production activation still blocked |
...
| **Telegram credential gate** | Dedicated bot approval + credential placement | **SATISFIED for sandbox apply** — credential `MARS Client Ops Telegram — bzpm.ru` (`2bIC5376l7ElXb4B`) bound to inactive workflow Telegram node (1B-C1); secret remains gitignored local only | Rotate/revoke token externally; production activation still gated |

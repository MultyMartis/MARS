...
# changed-summary.json → envelope metrics
...
ACTION_TEXT: Final[Mapping[str, str]] = {
    "NONE": "none",
    "REVIEW_ONBOARDING": "проверить новые ветки каталога",
    "REVIEW_HYGIENE": "проверить флаги гигиены каталога",
    "REVIEW_SOURCE_FAILURE": "проверить сбой монитора / источника",
    "REVIEW_SCHEDULER_AND_ARTIFACTS": (
        "проверить планировщик и свежесть артефактов"
    ),
    "REVIEW_SOURCE_ARTIFACTS": (
        "состояние сайта не подтверждено — проверить исходные артефакты"
    ),
    "REVIEW_SCHEMA_COMPATIBILITY": (
        "проверить совместимость схемы исходных артефактов"
    ),
    "REVIEW_SOURCE_TIME": "проверить метки времени исходных артефактов",
    "REVIEW_ENVELOPE_SECURITY": (

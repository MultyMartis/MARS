...
SITE-002 monitor/report artifacts
  тЖТ SITE-002 adapter parser + firewall
  тЖТ SITE002MonitorResult
  тЖТ Client Ops producer input
  тЖТ existing producer offline transport (mock/fixture/disabled)
...
def parse_source(source_dir: Path) -> tuple[ParsedArtifacts, dict[str, Any]]:
    ...
    # run.log is evidence/debug only тАФ never load into adapter contract
    redaction_result = {
...
def normalize_status(source_status: str) -> str:
    """Map SITE-002 classification vocabulary; unknown тЖТ raise."""
...
def adapt_source_dir(
    ...
    *,
    ...
    """Parse тЖТ firewall тЖТ normalize тЖТ optional envelope.
...
def run_site002_adapter_dry_run(
    ...
    *,
    ...
    """Manual offline adapter тЖТ producer mock/fixture/disabled path."""

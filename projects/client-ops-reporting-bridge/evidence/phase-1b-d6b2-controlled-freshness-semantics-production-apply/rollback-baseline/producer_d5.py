...
explicit completed SITE-002 monitor artifact
  тЖТ D4 adapter
  тЖТ D5 authorization gate
  тЖТ D3 HTTPS transport (reused)
  тЖТ one bounded POST
...
def apply_d5_real_source_markers(envelope: dict[str, Any]) -> dict[str, Any]:
    """Annotate producer metadata only тАФ does not alter identity-bearing fields."""
    out = dict(envelope)
...
def assess_preview_for_live(preview: Mapping[str, Any]) -> dict[str, Any]:
    """Return preview verdict тАФ does not send."""

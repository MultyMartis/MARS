...
/**
 * CORVONERO Campaign V2.1 — Commander XLSX generation with dynamic expected counts.
 */
...
async function forensicVerifyWorkbook(filePath, payload, campaignId, templateSha, expected) {
  ...
  for (let r = DATA_START_ROW; r <= texts.rowCount; r++) {
    ...
    const h2 = safeCellText(texts, r, FORENSIC_COL_V21.headline_2);
    const adText = safeCellText(texts, r, FORENSIC_COL_V21.text);
  ...
  return {
    status,
    checks,
    counts: { groups: groupsSeen.size, keyword_rows: kwRows.length, ad_rows: adRows.length, max_group_keyword_count: maxGroupKw },
  };
}
...
async function main() {
  ...
  const manifestPath =
    process.argv[2] ??
    path.join(PILOT_ROOT, 'CORVONERO-CAMPAIGN-V2.1-AUTHORITY-MANIFEST-v1.json');
  const outputDir =
    process.argv[3] ??
    path.join('X:', 'AI MARS STORAGE', 'exports', 'corvonero', 'CORVONERO-CAMPAIGN-V2.1-FINAL-2026-06-30');
  const countsPath =
    process.argv[4] ?? path.join(PILOT_ROOT, 'CORVONERO-CAMPAIGN-V2.1-EXPECTED-COUNTS-v1.json');
  ...
  let loaded = enrichLoadedAuthority(await loadAuthority(manifestPath));
  ...
  const generationDoc = {
    run_id: 'CORVONERO-CAMPAIGN-V2.1-FINAL-2026-06-30',
  ...
  fs.writeFileSync(
    path.join(PILOT_ROOT, 'CORVONERO-CAMPAIGN-V2.1-GENERATION-v1.json'),
  ...
  fs.writeFileSync(
    path.join(PILOT_ROOT, 'CORVONERO-CAMPAIGN-V2.1-FORENSIC-VALIDATION-v1.json'),
  ...
  console.log('PASS — 10 V2.1 XLSX files generated and forensically verified');
}

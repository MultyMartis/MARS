...
Handler `/page__FORM.php` field contract: `pf_contact` = method (hidden or select); `pf_phone` = visible phone. **FORM CONTRACT REGRESSION 01 COMPLETE** closed 90 live clones of the webinar misuse (visible `pf_contact` / missing `pf_phone`). `blog.html` still lacks a consent checkbox (WAVE 01 residual; not invented this wave).

Authority: [Form Security Baseline](ISEO-SU-FORM-SECURITY-AND-ANTISPAM-BASELINE-v1.md) · [Consent WAVE 01 evidence](ISEO-SU-FORM-CONSENT-WAVE-01-EVIDENCE-v1.md) · [WAVE 01A evidence](ISEO-SU-FORM-CONSENT-WAVE-01A-CALC-RESULT-PATCH-EVIDENCE-v1.md) · [Form contract regression 01](ISEO-SU-FORM-CONTRACT-REGRESSION-01-EVIDENCE-v1.md).
